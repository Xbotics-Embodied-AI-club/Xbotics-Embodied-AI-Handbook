...
表里没有哪一行是''在回路''与''极高便携''同时占住的------这两列在四行里始终一升一降，这正是选型的实质：不是挑''最好的那个''，而是按预算与场地在这条带子上挑一个付得起的位置。本课选了
SO-101
主从臂（真机、同构），它是这一类里成本最低的入口，轨迹直接可用，代价是采集规模上不去。下面逐类展开。
...
\texttt{lerobot-record} 实际穿过哪几层：(1)起于命令 → (2)读主臂动作 →
(3)从臂执行、相机出图 → (4)拼成一帧写进数据集（本书自绘，层名与目录名按
LeRobot v0.5.1 源码整理）}
