...
## 3.2 在 SO-101 上训练 ACT：把仿真数据和真机数据混成一份

3.1 节用的是 LIBERO —— 别人做好的数据集、别人定好的成功判据。这一节换成我们自己的机器人，
并且要回答第9讲 4.5 节末尾留下的那个问题。

那一节采过两份数据：真机 Leader–Follower 臂遥操一份，仿真里键盘遥操一份。它们**形状一样**
（都是六维动作、两路相机、同样的落盘结构），**口径不一样** —— 真机的 `action` 是主臂读到的
关节目标角度（绝对），仿真那份是归一化到 $[-1,1]$ 的关节增量（相对）。第9讲把话说到了这里：
容器格式一致不等于语义一致，混之前必须逐项核对。

这一节把这件事做完：产一份**从一开始就照着能混来做**的仿真数据，和真机数据合成一个训练集，
训一个 ACT。

### 3.2.1 SO-101 仿真器与它的三个任务

仿真器是 `so101_sim`，跑在 ManiSkill / SAPIEN 上，装的是与实物同一副 SO-101 的 URDF ——
同样五个臂关节加一个夹爪，同样的关节限位与连杆长度。三样东西都公开：

- 仿真器：<https://github.com/Xbotics-Embodied-AI-club/Xbotics-SO101-Sim>
- 本课用的 LeRobot 分支：<https://github.com/Xbotics-Embodied-AI-club/lerobot>
- 仿真数据集与产线源码：<https://huggingface.co/datasets/Harrysunshine/so101-sim-pickplace-v2>

![四层自上而下：SAPIEN / ManiSkill 负责刚体物理与渲染、并行环境；so101_sim 场景包提供与实物同一副 SO-101 的 URDF、三个抓放场景与成功判据；So101SimEnv 是 lerobot 封装，给出 gym 接口并把观测与动作换成真机口径（臂关节用度、夹爪用 0~100 的行程百分比）；最下面是 lerobot 官方的 record / train / eval 三条命令](../../assets/figures/lecture10/fig10-3-sim-stack.png){width=100%}

它是一个**薄封装**，不是另一套训练框架 —— 所以录数据、训练、评测三条官方命令原样可用。它注册了三个抓放场景，三个都是「把桌上的物体放进料箱」，只换物体 —— 40 mm 方块、20 mm 方块、40 mm 易拉罐：
...
![三个仿真场景的初始画面并排，自左向右分别是 40 mm 方块、20 mm 方块、40 mm 易拉罐；每格里机械臂在右侧起始位姿，物体在爪前，料箱在桌面靠前处](../../assets/figures/lecture10/fig10-3-sim-scenes.png){width=100%}

成功判据是**物体最终落在料箱内**：把物体位置换算进料箱自己的坐标系再比半尺寸——料箱每集
随机绕 z 转一个角，按世界轴比会在箱口边界附近判反。

### 3.2.2 数据不是遥操出来的，是脚本化专家产的

第9讲那份仿真数据是人按键盘采的。这一节换成**脚本化专家**：直接用逆运动学算出一条轨迹，
批量跑。理由是量 —— 键盘一集一集地采，采到几百集就是几十个小时；脚本化能在多进程里并行跑，
而且每一集都可复现（给定随机种子，物体位置、料箱朝向、轨迹全都定死）。

轨迹分五段，中间只在真正该停的地方停：

1. **取物** —— 一条落地曲线，从起始位姿斜着下来，下降与平移重叠着走，最后 5 cm 才转成垂直下插
2. **合拢** —— 夹爪按真机速率渐变到底，手臂不动，保持若干帧让堵转建立
3. **搬运** —— 一条弧线，抬起与平移融成一段
4. **松手** —— 渐变张开，物体自由落体进箱
5. **回家** —— 一段回到起始位姿

![一集示教的六条曲线：五个臂关节角（左轴，度）与夹爪行程（右轴，虚线，百分比），背景按五个阶段着色 —— 取物、合拢、搬运、松手、回家；夹爪只在合拢与松手两处动，关节曲线全程平滑没有折角](../../assets/figures/lecture10/fig10-3-demo-phases.png){width=100%}

### 3.2.3 能混成一份的前提：逐项对齐，一项都不能少

第9讲要求核对的那几项，这里逐一给出取值。**每一项配错都不报错，只会安静地训出一个更差的
策略**，所以这张表是这一节最该记住的东西：

| 要对齐的 | 取值 | 配错会怎样 |
|---|---|---|
| 动作口径 | 绝对关节角（`control_mode=pd_joint_pos`） | 相对当绝对训，手臂以包线最大速度朝错误方向走 |
| 臂关节单位 | 度 | 与弧度差 57.3 倍，观测偏小让归一化输出远离训练分布 |
| 夹爪单位 | 行程百分比 $0\sim100$ | 与度数**量级恰好撞车**（物理行程约 0–100 度），看数值看不出来，只表现为抓取这一环学不动 |
| 相机键名 | `observation.images.top` / `.wrist` | 键对不上，策略拿不到那一路画面 |
| 分辨率 | 两路都 $640\times480$ | 与训练时不同分辨率，等于换了个观测 |
| 控制频率 | 30 Hz | 动作块覆盖的物理时长变了 |
| 视频编码 | `h264` | 官方合并第一步逐字比 features，不等直接抛错 |
| `robot_type` | `so_follower` | 同上 |
| 指令文本 | 与真机那份**逐字相同** | 差一个字就落成两个任务号，混不成一份 |

最后一项容易被忽略。真机九个任务里有 `Pick up a cube and place in the bin`，仿真这边的
40 mm 方块场景用的是**同一句话**，于是合并之后它们落在同一个任务号下，模型看到的是
「同一个任务的两种画风」，而不是两个任务。

合并走官方命令（`code/vla/0_dataset_gen/merge_dataset.py` 是它的薄封装），不自己拼 parquet：
自己拼要同时改 `episodes` / `tasks` / `stats` 三张表，漏一张不报错，只在训练时表现为某些集
读不到任务文本。合并的第一步 `validate_all_metadata` 逐字比 `fps` / `robot_type` / `features`,
任一不等就抛错 —— 所以这一步同时也是上面那张表的自动检查。

### 3.2.4 实验：仿真 498 集 + 真机 300 集，一个 ACT

只取方块这一个任务：仿真 498 集，加上公开的 SO-101 真机遥操 300 集，合成 798 集 / 273,889 帧
/ **1 个任务**。训练脚本 `code/vla/3_imitation_learning/3_1_act/train_act_so101_sim.sh`
在合并后断言「只有 1 个任务」—— 多于一个就说明两侧指令文本没对上，直接停下来，不让它安静
地训成两个任务。

ACT 从随机初始化开始训，52M 参数。和 3.1 节 LIBERO 那轮相比，算法、网络、超参一个没动 ——
动作维度、状态维度、相机路数都由数据集自己的 `features` 描述决定，这正是本节要说明的事。
训 50000 步、batch 64，约 11.7 个 epoch。

### 3.2.5 结果

先看它究竟在做什么：

![两行八帧，走完一整个回合：机械臂从起始位姿降向方块、两指合拢夹住、抬起、 端着方块移到料箱上方、松手让方块落进箱、退回、回到起始位姿](../../assets/figures/lecture10/fig10-3-act-rollout.png){width=100%}

最终 checkpoint 在同一个场景上跑 50 局：**成功率 76.0%**。

**这个数字比训练途中看到的低。** 途中每 10000 步评一次、每次 10 局，五个点是
40% → 70% → 80% → 80% → 90%；照最后那个点读，会以为是 90%。差在哪里：**10 局的分辨率
只有 10 个百分点**，9/10 与 7/10 之间隔着两格，而 50 局里 76% 与 90% 只差七局。
途中那些点是用来看趋势的 —— 判断还在不在涨、要不要继续训；**出结论必须另跑一轮足量的**。
这一条在第8讲 3.1 节说过一次（loss 下降不等于任务成功），这里是它的另一半：
**成功率涨了也不等于成功率就是那个数**，要看那个数是几局量出来的。

配套代码在 `code/vla/3_imitation_learning/3_1_act/`：`train_act_so101_sim.sh` 合并数据并训练，
`eval_act_so101_sim.sh` 跑足量验收。
