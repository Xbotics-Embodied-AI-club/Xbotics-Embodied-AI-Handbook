...
![两种执行策略摆在同一条时间轴上：上半是动作队列，$t=0$ 预测的一段依次执行完，$t=4$ 才重新查询策略，两段不重叠；下半是 temporal ensemble，$t=0,1,2,3$ 每一步都重新预测一整段，于是第 3 步那一列（橙色框）被四段预测同时覆盖。底下两行标出这四个权重按 $w_i \propto \exp(-mi)$ 在 $m=0.01$ 时的真实取值（本书自绘）](../../assets/figures/lecture10/ref/fig10-2-chunking-vs-temporal-ensemble.png){width=100%}
...
关键在于：**loss 下降不等于任务成功**（第8讲 3.1 节已经强调过），所以必须靠仿真器内的闭环 rollout 来真正判断策略好不好。LIBERO 这类带成功判定的仿真环境，正好提供了这条闭环验证的标尺。

跑完之后，真正要盯的是 `eval/pc_success` 这条曲线，它的形状本身就在说事。开头一段会长时间贴着 0——这不是训练坏了，而是这个任务要求"抓起碗、移过去、放到盘子上"三步全对才算成功，前面若干次评估里策略往往只做对了前两步，成功率是离散的 0/1 统计，中间没有部分分。等它第一次离开 0，通常会在很短的训练区间里迅速抬起来，然后进入一个上下大幅震荡的高位平台：因为每次评估只跑 10 个 episode，成功率天然只有 0%、10%……100% 这 11 个取值，抖动是采样噪声不是性能反复。判断"训好了没有"要看这个平台的**中位水平**，不能看某一次评估的峰值——正好一次 100% 就宣布收敛，是这一步最常见的自欺。

![这条曲线来自本仓库真跑过的一次 LIBERO ACT 训练：三段背景色对应上面说的三个阶段。灰色段一直贴在 0，到约第 184 步才第一次离开；橙色段在两百多步的区间里迅速抬起；绿色段进入高位平台，此后一直在 40% 与 100% 之间大幅震荡，红色虚线是平台期的中位水平 83%。平台里任何一次 100% 的峰值都不能单独拿来当结论（数据出自 `code/vla/3_imitation_learning/result/3_1_act/eval_result1.png` 这次 run 的 `eval/pc_success` 面板，逐像素还原后用本书配色重绘）](../../assets/figures/lecture10/ref/fig10-3-libero-success-curve.png){width=100%}
...
- **有真机**：用第9讲 4 节采的 SO-101 主从臂遥操作数据（例如"抓方块放盘子"），同样用 `lerobot-train` 训 ACT，训练命令封在 `code/vla/3_imitation_learning/3_1_act/train_act_cuboid.sh` 里；训完把策略部署回 SO-101 做闭环执行，动作维度 $d_a$、状态维度 $d_s$、相机路数都会由数据集自动对上，超参沿用默认；
...
如果没有 `action_is_pad`，episode 尾部那批样本的监督就会被越界部分污染。也就是说，补位掩码不是边角细节，而是动作块监督能正常工作的必要条件。
...
投影层的输入维度不是写死的常数，而是从数据集的 feature 描述里读出来的：状态那一路取
...
![五格从左到右是 $t=0$、0.25、0.5、0.75、1。$t=0$ 是标准正态噪声，$t=1$ 是棋盘格数据分布，中间三格是 $z_t=(1-t)z_0+tz_1$ 的边际分布——注意结构不是最后一步突然出现的，$t=0.75$ 时格子的轮廓已经显形（本书自绘：直接按插值式采样画出解析的概率路径，未训练模型）](../../assets/figures/lecture10/ref/fig10-5-flow-matching-path.png){width=100%}

要说清楚这五格画的到底是什么：它们不是某个训练好的模型跑出来的采样轨迹，而是**概率路径本身**——按 $z_0 \sim \mathcal{N}(0, I)$、$z_1 \sim p_{\text{数据}}$ 各自独立采样、再按插值式算出 $z_t$，把每个 $t$ 上得到的那一堆点画出来。Flow Matching 训练要拟合的正是这条路径：向量场学到位之后，从噪声出发积分，走的就是这五格描述的过程。
...
![左列是采到的噪声样本，右列是数据集里的样本（画成双峰）。同一下标的两个点配成一条**条件路径**，图里画了 7 条：每个噪声点各自连向一个数据点，端点是独立采样再配对的，所以这些直线彼此交叉。红色那条被高亮出来，标出 $z_0$、$z_1$ 与它们之间的插值点 $z_t$（本书自绘）](../../assets/figures/lecture10/ref/fig10-5-linear-interpolation-path.png){width=100%}
...
看这三个方法，最好别按"第一代、第二代、第三代"来记——它们不是一条互相替代的世代链，而是在**两个正交的维度**上各做了选择：
