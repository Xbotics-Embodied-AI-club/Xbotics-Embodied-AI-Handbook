...
![三条实时路线在同一条推理链上的作用位置（自绘）。链路自左向右：观测进来 → VLM prefill → N 步去噪 → 动作块出队 → 控制器逐个消费。绿色括号是系统层，扣在 prefill 与去噪这两段；蓝色括号是算法层，扣在去噪与动作块出队这两段上；橙色括号是学习+投机层，扣在整条链的外侧](../../assets/figures/lecture13/ref/fig13-1-realtime-three-routes.png){width=100%}

三道括号的**位置不同**，就是"三条路线正交"这句话的全部含义：

- **系统层动的是那两段本身**。绿色区域里两条并排的泳道，画的是 1.5 节要讲的 Full Streaming Inference——大模型慢回路几十毫秒看一次场景，Action Expert 快回路约 2 ms 出一次动作，两条并发跑；右边那对长短条是同一件事的另一面，单次推理从 106 ms 压到 27 ms。这一段被压短了，链路的其余部分一个字没改。
- **算法层动的是段与段之间的调度**。左半是 FASTER 的做法：去噪步数沿时间视界分配，近端一步、远端多步（1.4 节），所以那六根柱子越往右越高。右半是 RTC 的做法：新算出来的一块与上一块有重叠，重叠区切成冻结 / 软过渡 / 自由重画三段（1.3 节）。两者都不改模型权重，只改推理期什么时候算什么。
- **学习+投机层根本不在主干上**。橙色那条是 FLASH 的旁路：小模型 7.8 ms 猜一整块，主模型并行验证，猜中就绕开去噪、猜错才回主干走完整的 58 ms（1.6 节）。同一层里的 V2 更靠外——它不绕路，学的是"何时快、何时慢"，调的是整条链的节奏。
...
三条 bullet 用文字最难同时说清的，是这三刀**落在链路的不同维度上**：

![三条线用的是同一个招式，刀口位置各不相同（自绘）。三格共用一套视觉语法——同样的链路样式、同样的红色虚线加菱形刀口符号；变的只是刀切在哪个维度上](../../assets/figures/lecture13/ref/fig13-4-one-cut-three-lines.png){width=100%}

左格那条红线竖着落在时间轴上，切的是**时间**：同一个动作块，刀左边是已经发出去、来不及改的部分，右边是可以重新生成的部分（1.3 节的 RTC）。中格那条线同样是竖的，切开的却是**信息**：当下单帧和记忆库各存各的、各有各的更新节奏，两边再一起汇进决策（第 2 节）。右格那一刀转成了横的，切的是**职责**：上面是懂任务、不学走路的 S2，下面是会走路、不懂任务的 S1，刀口处只留一个窄接口（3.3 节的潜在 verb、3.9 节的分块命令）。

三格的刀口符号是同一个，位置却落在三个互不相干的维度上——这就是"同一个招式"这句话的形状。刀口一律标成红色，是因为接缝同时也是账单的所在：图下那行把这笔账的两面都写了，还要再补一句——切开之后，就再也没有一个模块能看见全局。这是本讲反复出现的那对取舍，也是判断下一篇 VLA 前沿论文的起点。
