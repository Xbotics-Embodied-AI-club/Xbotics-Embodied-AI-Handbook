...
让一台机械臂把桌上的方块放进盘子，有两种截然不同的做法。一种是把''怎么动''这件事一步步算出来，每一环都人写规则；另一种是直接用一个网络从相机画面映射到电机指令。这一讲从这两种机器人的对比讲起，引出后面六讲的共同主角------\textbf{策略（policy）}。
...
传统做法会把''怎么动''\textbf{显式地}用人写的模型一步步算出来：先感知与建图，再用
RRT、MPC
这类算法规划一条轨迹，最后交给控制器执行------每一环都是人设计的算法。

把''生成机器人运动''的方法摊开看，大致分成两支：左边是这种\textbf{显式建模}（运动学、规划、控制），右边是这一讲要讲的\textbf{隐式建模}（从示教中学习、强化学习）：

\begin{figure}[H]

{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/81_generating-motion-explicit-vs-implicit.jpg}

}

\caption{生成机器人运动的两条路：左侧显式建模（运动学、规划、控制），右侧隐式建模（模仿学习、强化学习）}
...
端到端策略给出的答案是：\textbf{不再把任务拆成感知---规划---控制三段分别设计，而是用一个神经网络，从图像与状态直接预测任务级的动作表示}，靠联合训练省掉人工写死的中间接口。相机像素进去，动作出来------但要注意这个''动作''通常还不是电机级命令：它更常是关节目标、末端增量或一段动作块，后面仍要经过坐标变换、控制器和安全约束才真正落到电机上。
...
PilotNet
让摄像头画面直接进一个卷积网络、输出方向盘转角，中间不再有''车道线检测→路径规划→转向控制''的人工流水线：

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/813_pilotnet-end2end.png}}

}

\caption{PilotNet：摄像头图像经一个卷积网络直接输出方向盘转角，端到端反向传播训练}

\end{figure}%

Levine
等人在机械臂上做了同样的事，并把本讲的题眼问了出来------把感知和控制\textbf{联合}端到端地训，会不会比分开设计更好？他们用一个网络把图像直接映射到电机扭矩，并在其视觉运动控制实验中报告：联合训练感知与控制，优于所比较的分阶段基线。要注意这是这批任务、这批数据、这组基线下的结果，并不等于所有任务都该取消模块化设计。

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/813_levine-visuomotor-arch.png}}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/821_agent-env-loop.jpg}

}

\caption{智能体与环境的交互回路：智能体根据观测输出动作，环境接收动作后返回新的观测，循环往复}

\end{figure}%

图里用了强化学习的习惯记号（状态 \(s_t\)、动作 \(a_t\)、奖励
\(r_t\)），本讲先只关注''观测进、动作出''这一条主线，奖励要到第14讲的强化学习才登场。写成符号是
\(a = \pi(o)\)；更一般地写成一个分布
\(\pi(a \mid o)\)------同一个观测下，合理的动作往往不止一个，这件事到第10讲学生成式策略时会成为核心出发点。
...
把''观测里加不加图像、加不加语言''当坐标，策略就排成一条谱系：只看本体状态的
\textbf{state policy} → 加图像的\textbf{视觉运动策略}（如 ACT、Diffusion
Policy）→ 再加语言指令的\textbf{语言条件策略}（如 RT-1）→
把视觉、语言、动作统一进一个大模型的 \textbf{VLA}（如
\(\pi_0\)）。越往右，策略''看懂''的东西越多、能听的指令越泛。这条谱系正好是本课程''端到端策略''这一大块（第8--13讲）的地图，最右端的
VLA 会从第11讲起展开。

谱系最右端的 VLA
到底''统一''了什么？一句话：把''看懂图像''的视觉模型、``看懂文字''的语言模型、``控制行为''的动作模型合成一个------让机器人既能看、又能听，再把这份理解直接变成动作：

\begin{figure}[H]

{\centering \includegraphics[width=0.55\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/823_vla-three-pillars.png}

}

\caption{视觉-语言-动作模型把视觉模型（看懂图像）、语言模型（看懂文字）、动作模型（控制行为）三者合一，让机器人能感知、推理并连贯地行动}
...
策略就是''观测→动作''的那个函数，写作
\(\pi(a\mid o)\)。它的两个接口------观测里放什么、动作怎么表达------决定了它是一个只看状态的
state policy，还是一个能听语言的
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/830_train-deploy-roundtrip.png}}

}

\caption{训练时示教数据经过''统一动作口径、归一化''进入网络；部署时网络输出必须用同一套约定''反归一化、还原口径''逆着走回来，最后由裁剪限位兜底}
...
策略的训练目标通常是''让预测动作接近示教动作''，这是一个\textbf{单步}的监督指标。但任务成功是一个\textbf{闭环}的结果：要一步接一步走到底、把方块真的放进盘子。单步每步差一点点，几十步累积下来可能完全跑偏；反过来，loss
不是最低的那个
checkpoint，真机成功率反而更高，也很常见。也不存在一个通用的数值，说''loss
降到多少就能部署''------离线指标（验证损失、动作误差、覆盖率、时延与校准检查）用来做训练监控、checkpoint
粗筛和故障诊断，最终的任务能力则要在仿真或真机上按统一协议闭环跑一遍才算数。

\begin{quote}
一句话：loss
衡量的是''像不像示教''，成功率衡量的是''任务成没成''，两者经常对不上------所以选模型不能只看
loss 曲线。两类指标是分工，不是互斥：离线筛，闭环定。
\end{quote}
...
注意图里的转折点：轨迹还在绿色带内时，偏差几乎看不出来；\textbf{走出带子的那一刻}，模型面对的是示教里从未出现过的状态，输出开始失去依据，轨迹加速飘走。这个现象叫\textbf{分布漂移}（covariate
shift），是模仿学习的经典难题，Ross 等人提出的 DAgger
就是冲它去的（第10讲会看到生成式策略怎么缓解分布漂移；DAgger
这条交互式纠正的路，留到第16讲讲人在环方法时展开）。
...
\caption{同一步运动的两种记法：相对口径报''动多少''（+0.02），绝对口径报''去哪儿''（1.20）；数字不同，含义也完全不同}
...
Policy
的实验里，仅仅换一种动作表示（速度口径换成位置口径），多个任务的成功率就明显不同。

真正的分水岭在于：\textbf{训练和部署两端，必须用同一种口径去解释同一个数字。}
注意相对口径有个数值特点------一个 30Hz
采集、动作温和的抓取任务，相邻两帧的关节增量常常只有 0.01～0.03
弧度，几乎贴着 0。由此引出两个对称的坑，下图左右各画了一个：
...
\item
  ...
  0.02
  左右的绝对目标''------机械臂径直扎向接近零位的姿态；到那之后，每步目标几乎不变、真实位移只有一个最小单位的量级，还会被预测噪声整个淹没。症状：\textbf{先猛地一动，然后几乎不动}。此时
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/834_normalization-ranges.png}}
...
则稳稳贴住数据主体。所以分位数归一化更抗离群点------\(\pi_0\)、LeRobot
这类框架默认用的就是它。
...
\texttt{asset\_id}）的统计量，那就是一个\textbf{部署错误}，应当当场暴露出来。别指望框架会好心地帮你兜住：不同实现的行为并不一致------有的直接报错，有的用默认值，有的加载另一套资产，也有的真就把
1.0 原样发出去（本该还原成
0.8）。所以启动时就要校验统计量的键、维度、版本和数值范围，\textbf{禁止静默回退}；否则整条动作的尺度会系统性错位，而你只看到下图这两种症状：

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/834_scale-error-symptoms.png}}

}

\caption{左：动作被系统性放大，每步都迈过头、围绕目标来回过冲，看起来在''抖''；右：动作被系统性缩小，每步只挪一点点、迟迟够不到目标，看起来''不动''}

\end{figure}%

尺度被放大，机器人每一步都迈过头、围着目标来回过冲------看起来在\textbf{抖}；尺度被缩小，每一步只挪一点点、迟迟够不到目标------看起来\textbf{不动}。这种错和
3.3
...
就算前面都对，真机上还要留一道兜底闸：把动作\textbf{裁剪}（clip）到合理范围、限制最大速度、加上关节限位。策略偶尔吐出一个离谱的值时，这道闸能把它拦在幅值之内。但要清楚它的能力边界：裁剪只管\textbf{单次}幅值，管不了轨迹整体------连续多个贴着边界的动作、坐标系搞错、绝对/相对口径搞反，照样能做出危险运动。所以它是\textbf{最低层的数值保护}，上面还要叠加加速度/jerk
限制、工作空间约束、碰撞监测、看门狗、急停，以及先低速分阶段验证再放速度。
...
那条线上对应的是 \texttt{pi05\_libero} 配置与
checkpoint。换版本要连着配置、归一化资产、评测脚本一起换，不能只换权重文件。
...
提出的视觉-语言-动作模型：一个视觉-语言骨干网络看懂图像和指令，后面接一个''动作专家''生成连续动作。

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/841_pi0-architecture.png}}
...
\begin{quote}
备注：\(\pi_0\) 内部用 flow matching 生成动作，这套机制留到第10讲（Flow
Matching）和第11讲（\(\pi_0\)
家族）再拆。\textbf{本讲只把它当一个''观测进、动作出''的黑盒}------恰好印证第
2 节对策略的定义。
\end{quote}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/842_libero-task-suites.png}}
...
{[}9{]} LIU B, ZHU Y, GAO C, et al.~LIBERO: benchmarking knowledge
