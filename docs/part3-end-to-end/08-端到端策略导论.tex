...
\includegraphics[width=0.72\linewidth,height=\textheight,keepaspectratio,alt={生成机器人运动的两支：左支显式建模，下挂正运动学、逆运动学、基于控制的规划；右支隐式建模，下挂深度强化学习与从专家示教中学习。本讲以及第9--13讲都站在右边这一支上（图片出自参考文献 1）}]{../../assets/figures/lecture08/ref/fig08-1-motion-taxonomy_lerobot-tutorial.png}\{width=100\%\}
...
\KeywordTok{def}\NormalTok{ replace\_images\_with\_white\_noise(observation\_batch: }\BuiltInTok{dict}\NormalTok{[}\BuiltInTok{str}\NormalTok{, Any]) }\OperatorTok{{-}\textgreater{}} \BuiltInTok{dict}\NormalTok{[}\BuiltInTok{str}\NormalTok{, Any]:}
    \CommentTok{\# 遍历观测字典的全部字段，逐个判断该不该换掉}
    \ControlFlowTok{for}\NormalTok{ key, value }\KeywordTok{in}\NormalTok{ observation\_batch.items():}
        \CommentTok{\# 前缀这一条锁定了"只有相机图会被换掉"；is\_tensor 那一条是保险，}
        \CommentTok{\# 万一某个同前缀的字段不是张量，rand\_like 会当场报错}
        \ControlFlowTok{if}\NormalTok{ key.startswith(}\StringTok{"observation.images."}\NormalTok{) }\KeywordTok{and}\NormalTok{ torch.is\_tensor(value):}
            \CommentTok{\# rand\_like 而不是 rand：形状、dtype、所在设备全部照抄原图，}
            \CommentTok{\# 这样"只改内容、不改口径"，网络那一侧感觉不到任何别的变化}
