...
传统做法会把``怎么动''\textbf{显式地}用人写的模型一步步算出来：先感知与建图，再规划一条轨迹，最后交给控制器执行。规划这一步有多种常用算法，比如
\textbf{RRT}（快速扩展随机树，Rapidly-exploring Random
Tree）在自由空间里随机撒点、连成一条不碰东西的路径；\textbf{MPC}（模型预测控制，Model
Predictive
Control）则用动力学模型往前推演几步，再决定当下这一步怎么动。这条链上的每一环，形态基本都由人事先定下来。
...
{\centering \includegraphics[width=0.72\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/fig08-1-motion-taxonomy_lerobot-tutorial.png}

}

\caption{生成机器人运动的两支：左支显式建模，下挂正运动学、逆运动学、基于控制的规划；右支隐式建模，下挂深度强化学习与从专家示教中学习。本讲以及第9--13讲都站在右边这一支上}

\end{figure}%

图上要看的不只是``分成两支''，还有右支那两片叶子------深度强化学习与从专家示教中学习，正是这一讲以及往后第9--13讲的落点。左支的好处则在于每一步都看得见、查得到，哪里出错就去修哪一段，可解释、可调试。
...
\textbf{DAVE-2}{[}2{]}
让摄像头画面直接进一个卷积网络、输出方向盘转角，中间不再有``车道线检测→路径规划→转向控制''的人工流水线。DAVE-2
是 2016 年那篇论文里用的名字，同一组人 2017 年的后续工作把它改称
\textbf{PilotNet}{[}3{]}，今天更常见的是后一个名字。

\begin{figure}[htbp]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/813_pilotnet-end2end_arxiv-1604.07316.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/813_levine-visuomotor-arch_arxiv-1504.00702.png}}
...
放进机器人和环境的互动里看最清楚：\textbf{智能体}（agent，在这里就是机器人）从环境收到观测，输出一个动作作用回环境，环境再给出新的观测，如此往复。这个回路上只有两条边：动作出去、观测回来，这一讲要的正是这两条。强化学习那套习惯记号里还有第三条边（奖励
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0907\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3982\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5111\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 典型例子 \\
\midrule\noalign{}
...
是人类专家此刻通过 Leader
臂给出的目标关节角。策略要学的，就是``给定上面的观测、回归出下面的动作''：

\begin{figure}[htbp]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/822_observation-action-pairs.jpg}}
...
\begin{quote}
备注：观测和动作的``口径''是策略的接口约定。同一个任务换一种动作表示（比如关节角换成末端位姿、绝对换成相对），对网络来说就是另一个学习问题。后面几讲的很多设计差异都出在这里，本讲第
3 节要拆的那几个部署故障也是。
\end{quote}
...
policy），第10讲要动手训的 ACT（动作分块 Transformer，Action Chunking
with Transformers）{[}5{]}和 Diffusion
Policy（用扩散模型生成动作）{[}6{]}都属于这一档。再往右，观测里加进一句自然语言指令，同一个网络就能按``把碗放到盘子上''还是``把杯子推到左边''做不同的事，这类叫\textbf{语言条件策略}（language-conditioned
policy），Google 的 RT-1{[}7{]}
是代表作。最右端把视觉、语言、动作三件事统一进一个大模型，就是\textbf{视觉-语言-动作模型}（Vision-Language-Action，VLA）{[}8{]}，本讲实验要用的
\(\pi_0\) 就是其中一个。

\begin{quote}
备注：这四档是本课程为了把第8--13讲串起来定的一条坐标轴，取的维度只有一个：观测里放什么。它不是哪篇论文的标准分类法。每一档的代表工作各有论文可查（ACT{[}5{]}、Diffusion
Policy{[}6{]}、RT-1{[}7{]}、VLA
综述{[}8{]}），但换一个维度（比如按动作是怎么生成的）分出来的档位就会不一样。
\end{quote}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/fig08-2-policy-spectrum.png}
...
/ ＋相机图像 / ＋语言指令 /
＋大模型统一表征，下面一行是它换来的能力与代表方法。最右端那条红色虚线标出第11讲起展开的落点}

\end{figure}%

图要读的是那一列加粗的``＋''号：四档是\textbf{同一个 \(\pi(a\mid o)\)
一路往观测里加东西}，先加画面、再加语言、最后加进一个能把两者一起理解的大模型，不是四种并列的方法。观测里多进来一样，能听懂的指令就宽一层，这也是为什么最右端的
VLA
...
VLA，也决定了它好不好训、好不好部署。下一节就来看，我们真把一个策略学出来、搬上机器人，会在哪些地方栽跟头。
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/fig08-3-train-deploy-roundtrip.png}
...
\(\sigma\)、加回
\(\mu\)）、再还原动作口径（相对口径需累加），最后由裁剪限位兜底。中间两条竖虚线标出两处衔接：口径与统计量，两端必须各用同一套}
...
{\centering \includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/832_error-snowball.png}
...
等人提出的 \textbf{DAgger}{[}9{]}（数据聚合，Dataset
Aggregation：让策略先跑，再请专家对跑出来的状态补标正确动作，把这批数据加回训练集重训）就是冲它去的。第10讲
1.2 节会从生成式策略的角度再碰一次这个问题，DAgger
这条交互式纠正的路，则留到第16讲 5.6 节的人在环方法展开。

\textbf{策略到底行不行，得让它在环境里真正跑一遍（rollout）、看任务成没成。离线指标做不了这个判决。}
离线指标仍然有它的位置：粗筛
checkpoint、诊断故障、在上真机之前先把明显的问题挡掉，从而降低真机风险。这也是为什么本讲第
4 节的实验，核心是让策略在仿真里闭环跑起来，不是算 loss。
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/833_abs-vs-delta-axis.png}

}

\caption{同一步运动的两种记法：相对口径报``动多少''（+0.02），绝对口径报``去哪儿''（1.20）；数字不同，含义也完全不同（三个数的来源是上面那个
1.18 转到 1.20 的算例）}
...
Policy
那篇论文专门做过这项对照{[}6{]}：方法完全不变，只把动作表示从速度口径换成位置口径，多个任务上的成功率就明显不同。

两种口径真正的分水岭在于，\textbf{训练和部署两端，必须用同一种口径去解释同一个数字。}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/833_mismatch-failures.png}}
...
\begin{quote}
备注：口径的约定其实从\textbf{采数据}那一刻就开始了。Leader--Follower
遥操作里 Follower 臂跟随 Leader
臂，也分两种方式------\textbf{绝对跟踪}：Follower 臂直接去 Leader
臂当前的绝对姿态，录下的动作是 Leader 臂读数，零点对齐之后它就等于
Follower 臂被要求到达的那个绝对姿态（Follower 臂实测到达的姿态另记在
\texttt{observation.state} 里，两者差一点跟踪误差），前提是
Leader--Follower
标定的零点对齐；\textbf{相对（锚定）跟踪}：接通的第一帧记下``Follower
臂当前姿态减 Leader 臂姿态''的差，之后 Follower 臂只复现 Leader
臂的位移。锚定跟踪不要求零点严格对齐、接通瞬间 Follower
臂不会突然跳动，但录下的动作和 Follower
臂的绝对姿态恒差一个\textbf{每次开机都不同的常数}，动作口径已经被悄悄改写，部署时更容易两端对不上。遥操作采数据的细节，下一讲展开。
\end{quote}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/fig08-3-normalization-ranges.png}
...
左图里关节角和夹爪开合那两条细条几乎看不见，这就是问题所在：\([-3.14, 3.14]\)
和 \([0, 1]\) 摆在 0 到 255
的像素旁边，被压成了两道竖线。右图三条一样长，归一化要做的是把各维放到同一把尺子上，不是把数值改小。
...
第二种是\textbf{分位数（\(q_{01}\) / \(q_{99}\)）}。先说清 \(q_{99}\)
是什么：把这一维的所有数据\textbf{从小到大排队，排在 99\%
位置上的那个数就是 \(q_{99}\)}，99\% 的数据都不超过它；\(q_{01}\)
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/834_q99-normalization.png}}

}

\caption{左：直方图里几个离群点把 max 拉得很远，而 \(q_{01}\) /
\(q_{99}\) 紧贴数据主体；右：把 \([q_{01}, q_{99}]\) 线性映射到
\([-1, +1]\)，超出 \(q_{99}\) 的毛刺被截断到 +1}

\end{figure}%

好处在左图一眼可见：min / max 会被那几个毛刺整个带偏，\(q_{01}\) /
\(q_{99}\) 则稳稳贴住数据主体。所以分位数归一化更抗离群点。
...
\textbf{这套统计量（\(\mu, \sigma\) 或 \(q_{01}\) /
\(q_{99}\)）是从训练集统计出来的，是模型的一部分}，它通常和权重放在一起，存成一个不起眼的小文件。部署时如果只带走了权重、把统计量落下了，或者拿错成另一份数据（另一个
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture08/ref/fig08-3-scale-error-symptoms.png}

}

\caption{左：动作被系统性放大，每步都迈过头、围绕目标来回过冲，看起来在``抖''；右：动作被系统性缩小，每步只挪一点点、迟迟够不到目标，看起来``不动''。两图里绿色虚线都是本该到达的目标}
...
\begin{quote}
模型权重和它的归一化统计量是\textbf{一套}东西；权重对了、统计量丢了或拿错了，输出照样全错。而且观测侧（输入）和动作侧（输出）两头都要归一化，部署时两套统计量一个都不能少，启动时先校验，别等机器人动起来才发现。
\end{quote}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2044\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.7956\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
检查项 & 要确认的问题 \\
\midrule\noalign{}
...
这里有个容易踩的前提要先说清楚。\textbf{用的是一份已经在 LIBERO
上微调过的 checkpoint，不是 \(\pi_0\) 的通用基座权重}。\(\pi_0\)
的基座预训练数据里不含 LIBERO 仿真数据，直接零样本丢进 LIBERO
是跑不出成功率的，必须配上对应的 LIBERO 微调配置与权重。本讲实验固定用
LeRobot 提供的 \texttt{lerobot/pi0\_libero\_finetuned\_v044}（代码见
...
提出的视觉-语言-动作模型{[}10{]}：一个视觉-语言骨干网络看懂图像和指令，后面接一个``动作专家''生成连续动作。

\begin{figure}[htbp]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/841_pi0-architecture_arxiv-2410.24164.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture08/ref/842_libero-task-suites_arxiv-2306.03310.png}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# LIBERO 按「套件 + 任务 id」组织，这里取 libero\_spatial 的第 0 个任务}
\NormalTok{env\_cfg }\OperatorTok{=}\NormalTok{ LiberoEnvConfig(}
\NormalTok{    task}\OperatorTok{=}\StringTok{"libero\_spatial"}\NormalTok{,         }\CommentTok{\# 四个套件之一：空间关系类任务}
\NormalTok{    task\_ids}\OperatorTok{=}\NormalTok{[}\DecValTok{0}\NormalTok{],                  }\CommentTok{\# 套件里的第几个任务，一次可以列多个}
\NormalTok{    obs\_type}\OperatorTok{=}\StringTok{"pixels\_agent\_pos"}\NormalTok{,   }\CommentTok{\# 图像 + 本体状态 两路观测}
\NormalTok{    observation\_height}\OperatorTok{=}\DecValTok{256}\NormalTok{,        }\CommentTok{\# 渲染分辨率：它决定策略拿到的图有多大}
\NormalTok{    observation\_width}\OperatorTok{=}\DecValTok{256}\NormalTok{,}
\NormalTok{)}

\CommentTok{\# make\_env 建的是向量环境（可以并行开多份），n\_envs=1 就是只开一份}
\NormalTok{envs }\OperatorTok{=}\NormalTok{ make\_env(env\_cfg, n\_envs}\OperatorTok{=}\DecValTok{1}\NormalTok{)}
\NormalTok{env  }\OperatorTok{=}\NormalTok{ envs[}\StringTok{\textquotesingle{}libero\_spatial\textquotesingle{}}\NormalTok{][}\DecValTok{0}\NormalTok{]   }\CommentTok{\# 按「套件名 → 任务 id」两级取出这一个环境}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# single\_action\_space 是「一个子环境」的动作空间，不含 n\_envs 那一维}
...
节反复强调的\textbf{闭环
rollout}：策略要在自己造成的后果里一直走下去，不是预测一次就完事。
...
\NormalTok{    policy\_cfg.device }\OperatorTok{=} \StringTok{"cuda"} \ControlFlowTok{if}\NormalTok{ torch.cuda.is\_available() }\ControlFlowTok{else} \StringTok{"cpu"}
    \CommentTok{\# strict=False：权重里有少量 buffer 的命名和当前 lerobot 版本对不上，}
    \CommentTok{\# 这些 buffer 不参与前向计算，跳过它们不影响推理结果。}
...
\NormalTok{    )}
    \CommentTok{\# 归一化统计量也跟着权重一起存，所以这两个 processor 从同一个 policy\_path 读}
...
\begin{Shaded}
\begin{Highlighting}[]
        \ControlFlowTok{for}\NormalTok{ \_ }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(max\_steps):    }\CommentTok{\# 超时上限：任务没做完也得停，否则闭环不会自己结束}
            \CommentTok{\# 四步预处理各修一种对不上：先把环境的嵌套观测摊平成 LeRobot 约定的 key，}
\NormalTok{            observation\_batch }\OperatorTok{=}\NormalTok{ preprocess\_observation(observation)}
            \CommentTok{\# 再补上任务的语言指令（环境的观测里没有它，但 VLA 的输入接口要），}
\NormalTok{            observation\_batch }\OperatorTok{=}\NormalTok{ add\_envs\_task(env, observation\_batch)}
            \CommentTok{\# 然后过环境侧口径对齐，最后过策略侧归一化——顺序不能换。}
...
\NormalTok{            observation\_batch }\OperatorTok{=}\NormalTok{ preprocessor(observation\_batch)}
            ...
            \CommentTok{\# 整个闭环里策略只做这一件事：看一眼观测，给出一个动作。}
            \ControlFlowTok{with}\NormalTok{ torch.inference\_mode():    }\CommentTok{\# 推理不建计算图，省显存也省时间}
\NormalTok{                action }\OperatorTok{=}\NormalTok{ policy.select\_action(observation\_batch)}

            \CommentTok{\# 回程按去路的逆序拆：先反归一化，再还原成环境认的动作口径}
...
\NormalTok{            action }\OperatorTok{=}\NormalTok{ env\_postprocessor(\{}\StringTok{"action"}\NormalTok{: action\})[}\StringTok{"action"}\NormalTok{].cpu().numpy()}
            ...
            \CommentTok{\# 动作作用回环境，换来下一帧观测——闭环的下一圈从这里开始}
\NormalTok{            observation, \_, terminated, truncated, info }\OperatorTok{=}\NormalTok{ env.step(action)}
\NormalTok{            frames.append(env.envs[}\DecValTok{0}\NormalTok{].render())    }\CommentTok{\# 每步单独渲一帧，攒完写成 mp4}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture11/ref/pi0/rollout_keyframes.png}
...
\begin{quote}
端到端策略把``感知---规划---控制''的手写流水线，压成一个``观测→动作''的网络；学会它、跑通它的闭环、并在部署时对齐动作口径与归一化，是后面所有
VLA 的起点。
\end{quote}
