...
4. QLoRA 的显存收益和速度代价都依赖实现。Raschka 的一次 LLaMA-2 7B 对比中，显存约从 21.3 GB 降到 14.2 GB，训练时间从 1.85 小时增加到 2.79 小时；按这组数字换算，显存省了约 33%，训练时间多了约 51%（与本节表格里“约 0.66× 相对吞吐”一致），但这不是一个固定规律
...
2. **单卡放不下？先抠单卡。** 别急着加卡，先上 **4.3 省显存四件套**（混合精度 + 梯度检查点 + 梯度累积 + CPU/NVMe offload），在一张卡内部把显存抠出来。很多时候四件套就够，省去多卡的复杂度。
...
部署后的策略能不能在真实世界继续学？RMA 提供了"在线快速适应"的雏形（靠适应模块在线估计环境隐变量，见 5.1.7）；真机在线纠正这一路的代表是 TRANSIC、DAgger 及 HIL-SERL 一类人在环方法（第16讲会展开）。但持续学习不忘旧、避免奖励 hacking 仍是难题。
...
Sim2Real Bridge 描述真实观测和机器人策略之间的闭环：真实观测先变成数字孪生；数字孪生再用于数据增强、任务生成和示教生成；训练出的策略回到真实机器人；真实执行反馈继续修正仿真域。
...
机器人示教很贵。Real2Sim 的一个直接用途是把少量真实示教转成可编辑的 3D 表示，然后从新视角、新物体摆放、新光照、新背景、新时间轨迹中合成更多训练数据。
...
更进一步，系统可以不只增强已有示教，而是生成新的任务、新的场景、新的示教轨迹。MimicGen 从少量人类示教中重组出大量 object-centric 轨迹；Gen2Sim 和 GenSim2 用 LLM / VLM 生成仿真任务、奖励和示教；R2R2R 从视频恢复物体运动，再把 3DGS 转成 mesh 进入 IsaacLab 之类的并行仿真器。
