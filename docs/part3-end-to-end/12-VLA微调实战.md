...
## 2.4 实验：仿真与真机混训，一个模型两边都能用

这一节回答的是一个更有用的问题：**能不能只训一个模型，让它在仿真器和实物上用同一套权重？**

要让这件事成立，前提不是模型，是**数据口径**。仿真录出来的一帧和真机录出来的一帧，必须是同一种东西：同样的通道顺序、同样的单位、同样的取值栅格。做到了，两边的数据才能物理地合成一份、一起训；做不到，混起来只是把两种方言拌在一起。

**数据。** 仿真侧用与真机同一条命令录制（`lerobot-record`，仿真被注册成一台 lerobot 机器人），产出三个任务共 1498 集；真机侧取公开的 SO-101 遥操数据 9 个任务共 2200 集。两侧物理合并成一份 **3698 集 / 128 万帧 / 10 个任务**的训练集。其中 `Pick up a cube` 与 `Pick up a can` 两个任务两边都有、指令文本逐字相同，因此落在同一个任务号下——策略看不出哪一条来自哪边。

对齐里最容易被忽略的是**取值栅格**。SO-101 用的 STS3215 是 12 位绝对编码器，真机的关节角只能落在 $360/4095 = 0.0879°$ 的格点上；仿真如果直接输出浮点真值，一行 `np.diff(action)/0.087912` 就能把两份数据分开。所以仿真侧也要量化到同一个栅格。

> **不必对齐的那一半。** 格点还有个**相位**：真机部分通道落在半格上而不是整格上。它来自 lerobot 换算时的 `mid = (min + max) / 2`——标定端点之和的奇偶决定 mid 是整数还是 x.5。这是**每台机器标定时的偶然**，没有物理含义，实测三台真机的相位各不相同。要对齐的是量化本身（每台真机都有），不是相位（每台都不一样）。分不清这两者，就会去复刻某一台具体机器的标定，第四台真机来了照样对不上。

**怎么复现。** 数据集与产线源码都公开了：

- 仿真数据集（三个任务 1498 集）与产出它的全部源码：<https://huggingface.co/datasets/Harrysunshine/so101-sim-pickplace-v2>
- 训好的权重：<https://huggingface.co/Harrysunshine/so101-smolvla-sim-real-10task>
- 画本节两张图的脚本：`code/vla/5_vla_finetune/5_2_smolvla_full_sft/plot_sim_real_sft.py`

真机那 9 个任务的数据不在我们的仓里——它来自 ModelScope 上公开的 SO-101 遥操数据集，
按原出处取用即可，我们不转发别人的数据。

**训练配置。** 从 `lerobot/smolvla_base` 出发，6 × A800，`accelerate` 起 DDP，batch 16/进程（effective 96），lr 5e-5、warmup 2000、cosine 退火到 30000 步（2.25 epoch），图像增强六项全开。

这里有个**必须亲眼确认**的地方，正好呼应 2.2 节那张表：`smolvla_base` 的 `config.json` 里 `freeze_vision_encoder` 与 `train_expert_only` **默认都是 `true`**，lerobot 的 `SmolVLAConfig` 默认值也是。也就是说，不显式覆盖这两个开关，跑出来的是 2.2 节说的**模块级**全参数微调，不是全模型全参数微调：
...
判据不是"我传了参数"，而是训练日志第一屏那一行：

```
num_learnable_params=402737376 (403M)
```

不加那两个开关时这个数是 **99,880,992（100M）**，占 4.5 亿总参数的 22%——视觉塔全程冻着。而这个实验要跨的恰恰是仿真渲染与真机图像之间的差异，冻着视觉塔等于让动作专家一个人扛。**这一行数字不看，就无法区分"我以为在做全参微调"和"真的在做"。**

**结果。** 收敛曲线与训练途中的成功率（每 5000 步在 cube40 上跑 10 局）：

![左轴红线是 flow-matching 损失（对数轴），从 0.56 一路降到 0.05；右轴蓝线是训练途中每 5000 步一次的成功率，六个点依次为 60%、100%、90%、90%、90%、90%，灰色虚线标出 70% 的验收线，第一个点就已经在线上方](../../assets/figures/lecture12/ref/fig12-2-sim-real-sft-convergence.png){width=90%}

最终 checkpoint 在三个仿真任务上各跑 50 局：

![三条横向条形，自下而上依次是 Pick up a cube 94%（47/50）、Pick up a small cube 90%（45/50）、Pick up a can 96%（48/50），每条带 95% 精确二项区间的误差棒，条内标出各自的置信下界 83%、78%、86%，红色虚线标出 70% 验收线，三条的下界都在线右侧](../../assets/figures/lecture12/ref/fig12-2-sim-real-sft-success.png){width=90%}
...
**验收按最低那一项判，不按平均。** 三项平均 93.3% 这个数没有意义——平均可以是 100/70/40 拼出来的，它会把某一项的失败藏起来。这里三项的**置信下界**都高于 70%，结论才站得住。

顺带说一个样本量的教训：训练到 10000 步时我们也测过一次，三场景各 30 局，得到 90.0 / 73.3 / 100.0%。点估计看着三项都过线了，但 `small cube` 那项 22/30 的 95% 区间是 **[54.1%, 87.7%]**——下界远在 70% 以下，也就是说真实成功率只有 60% 也能抽出这个结果。**30 局撑不起一个 73% 的结论。** 要么加样本，要么别下结论。

**还没有回答的那一半。** 上面全部是仿真里的数字，**实物一侧本轮没有测**。这个模型具备在实物上跑的条件——它与真机数据共用同一套口径，而且本身就在 2200 集真机数据上一起训过——但"具备条件"不等于"验证过"。实机成功率要在实机上量，这是 2.5 节的事。
...
**核心创新：** 全量 SFT 解冻模型的全部参数，在新机器人的数据上一起更新视觉-语言骨干与动作头，给出最高的参数更新自由度；对 VLA-0、SmolVLA 这类小体量 VLA，单卡就能把整模型适配到新本体上，是最直接、最不绕弯子的微调路线。2.4 节那一轮把这条线跑到底：仿真与真机 3698 集混成一份训练集，一个模型在三个仿真任务上拿到 94 / 90 / 96% 的成功率、三项的置信下界都高于 70%。
...
和 2.5 节一样，真机这条线还没开展，本节先只立题。上面那份默认注入名单、`--peft.*` 那几个参数、以及“先确认可训练参数量降下来”这个检查，在真机上都不变；要现场定的是秩取多少、以及动作专家之外还要不要覆盖别的层。
