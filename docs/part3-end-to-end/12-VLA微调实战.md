...
数据来源主要有三种：遥操作采集（人类操控机器人录制）、仿真环境生成、跨机器人迁移。其中“仿真环境生成”这一路，除了在仿真里遥操作，还能**完全不靠人**：由仿真器自己跑出成功轨迹当数据。本讲用来微调 SO-101 的那批仿真数据随课程一并提供，不必自己采集。
...
数据全部出自我们自己的 SO-101 仿真器：仿真里跑出来的成功轨迹转成 LeRobot 数据集，这批数据随课程一并提供。配套代码在 `code/vla/5_vla_finetune/5_2_smolvla_full_sft/`。微调从社区预训练权重 `lerobot/smolvla_base` 出发，全参数更新，不挂任何适配器。三个入口分别是：
...
**该盯什么。** loss 下降只说明拟合在进行，不说明任务做得成，真正要读的是评测报出的 `pc_success`。上面这条流程跑通了训练，但训练与评测不在同一个任务上（数据出自已下线的 ReachCube，只能放进 PickPlaceCube40 场景里评），`pc_success` 接近 0 是任务不匹配的预期结果，读不出微调的好坏。新版 SO-101 仿真器就位之后，我们把这条线重做了一遍，让训练与评测落在同一批任务上——那一轮的结果就是下一节。

## 2.4 实验：把 SmolVLA 全量微调到 SO-101

2.3 节把流程跑通了，但训练与评测不在同一个任务上，读不出成功率。这一节补上那个数字。

**数据。** 仿真三个抓放任务 1498 集，加上公开的 SO-101 真机遥操数据 9 个任务 2200 集，
合成一份 3698 集 / 128 万帧 / 10 个任务的训练集。

仿真器长什么样、这份数据怎么产出来的、以及**两侧要逐项对齐哪九项才混得成一份**，
都在第10讲 3.2 节讲过了，这里只用不重述。要点只有一条：两侧共有的 `Pick up a cube` 与
`Pick up a can` 指令文本逐字相同，于是落在同一个任务号下 —— 模型看到的是「同一个任务的
两种画风」，而不是两个任务。

**训练。** 从 `lerobot/smolvla_base` 出发，全参数更新，不挂适配器。

### 2.4.1 一个必须亲眼确认的地方

`smolvla_base` 的 `config.json` 里 `freeze_vision_encoder` 与 `train_expert_only`
**默认都是 `true`**，lerobot 的 `SmolVLAConfig` 默认值也是。不显式覆盖，跑出来的是
2.2 节说的**模块级**全参数微调，不是全模型全参数微调：
...
不加那两个开关时这个数是 **99,880,992（100M）**，占 4.5 亿总参数的 22%——视觉塔全程冻着。
微调要跨的正是新机器人带来的视觉差异，冻着视觉塔等于让动作专家一个人扛。
**这一行不看，就分不清“我以为在做全参微调”和“真的在做”。**
...
![左轴红线是 flow-matching 损失（对数轴），从 0.56 降到 0.05；右轴蓝线是训练途中每 5000 步一次的成功率，六个点依次为 60%、100%、90%、90%、90%、90%](../../assets/figures/lecture12/ref/fig12-2-sim-real-sft-convergence.png){width=100%}

两条线的走向不一样：损失一路降到底，成功率在第二个评测点就到顶、之后一直在 90% 附近横着。**训练损失不是验收指标**——它衡量的是动作序列拟合得多准，而任务成不成只看物体最后在不在箱里。靠 loss 判断“还该不该继续训”会一直得到“该”，得看右边那条线。

最终 checkpoint 在三个仿真任务上各跑 50 局：

| 任务 | 成功率 |
|---|---|
| `Pick up a cube and place in the bin` | 94.0%（47/50） |
...
**核心创新：** 全量 SFT 解冻模型的全部参数，在新机器人的数据上一起更新视觉-语言骨干与动作头，给出最高的参数更新自由度；对 VLA-0、SmolVLA 这类小体量 VLA，单卡就能把整模型适配到新本体上，是最直接、最不绕弯子的微调路线。2.4 节那一轮把这条线跑到底：仿真与真机 3698 集混成一份训练集，一个模型在三个仿真任务上拿到 94 / 90 / 96% 的成功率。

这两节各留下一个不写下来就会重犯的教训。一是**观测特征对齐**：微调命令本身只有一行，真正把人挡在门外的是相机键对不上（2.3 节）。二是**默认值不等于你以为的那档**：`smolvla_base` 默认冻结视觉编码器，不显式关掉这两个开关，跑出来的是模块级全参微调而非全模型全参微调，可训参数 100M 与 403M 之差全在这里——判据是训练日志里的 `num_learnable_params`，不是命令行里传了什么（2.4 节）。
...
## 3.10 实验：把 SmolVLA 用 LoRA 微调到 SO-101

和 2.4 节**同一份训练集、同一套评测**：仿真三个抓放任务 1498 集加真机 9 个任务 2200 集，
合成 3698 集 / 10 个任务；30000 步、warmup 2000、六个图像增强、六卡 effective batch 96、
评测环境参数逐字相同。两轮之间**唯一的差别就是怎么改权重** —— 只有这样，差出来的东西才
归因得到 LoRA 本身。

### 3.10.1 挂在哪些层：默认名单不够用

3.3.6 节说过 LoRA 通常只挂注意力的 $W_q$ 与 $W_v$。上游 SmolVLA 的默认名单正是这么定的
（`_get_default_peft_targets`），但它只挂**动作专家**那一族，外加五个具身投影：

照默认跑，可训练参数是 742,656，占全模型 4.5 亿的 **0.16%**。跑到 8000 步，
三个途中评测点**全是 0.0%**，而全参微调在第一个点就有 60%。

判据不是成功率，是 loss 对读：**LoRA 在 7000 步是 0.198，全参在 1000 步已经 0.194**。
差的不是方向，是进度 —— 欠拟合，不是学歪。
...
而 2.4.1 节的结论恰恰是「解冻视觉塔才跨得过新机器人的视觉差异」。默认名单把视觉塔和
文本塔整个冻着，等于让动作专家一个人扛同一件事。三族一起挂、$r$ 提到 64、五个具身投影
改成全量训练（低秩近似一个本来就不大的投影没有意义）、学习率随之提到 3e-4 —— 可训练参数
变成 9,851,728，占 **2.1%**。

> **秩要按基座大小取，不能两个模型共用一个数。** 同样 $r=64$ 挂到 $\pi_0$（40 亿参数、
> 180 个 $q/v$）上只摊到 0.58%，比这里薄了近四倍。参数占比才是要对齐的量。

### 3.10.2 结果

![两条 flow-matching 损失曲线（对数轴）与两组途中成功率：红色是全参 403M、橙色是 LoRA 10M，
LoRA 的损失全程高一截、末点 0.099 对 0.052；深浅两色的方点与圆点是每 5000 步一次的途中成功率，
两者最后都落在 80~90% 一带](../../assets/figures/lecture12/ref/fig12-3-lora-vs-full.png){width=100%}

先看它在仿真里跑成什么样 —— 三个任务各一集成功回合，每个任务两行八帧：

![三个任务各占两行（自上而下：方块、小方块、易拉罐），每个任务八帧走完一整个回合：初始位姿、夹爪降向物体、合拢夹住、抬起进入搬运、停在料箱上方、松手让物体落进箱、手臂退回、回到起始位姿](../../assets/figures/lecture12/ref/fig12-3-lora-rollout.png){width=92%}

最终 checkpoint 在三个仿真任务上各跑 50 局，与 2.4 节那轮并排：

| 任务 | LoRA 10M | 全参 403M |
|---|---|---|
| `Pick up a cube and place in the bin` | 94.0%（47/50） | 94.0%（47/50） |
| `Pick up a small cube and place in the bin` | 84.0%（42/50） | 90.0%（45/50） |
| `Pick up a can and place in the bin` | 98.0%（49/50） | 96.0%（48/50） |

可训练参数是全参的 1/40，三项里一项低 6 个点、一项高 2 个点、一项持平。

**这张图里最该记住的是那两条线的分叉。** 损失全程差着一截、到最后也没收敛到一处，
成功率却追平了。**损失差不等于能力差** —— 2.4.2 节说过训练损失不是验收指标，
那里是纵向的（同一轮里 loss 还在降而成功率早已横住），这里是横向的：两个不同的模型，
loss 差了近一倍，任务表现一样。要判断一个微调方案行不行，只能去跑任务。

适配器权重公开在
<https://huggingface.co/Harrysunshine/so101-smolvla-lora-sim-real-10task>，
配套代码在 `code/vla/5_vla_finetune/5_3_smolvla_lora/`。

## 3.11 实物实验：把适配器部署到 SO-101 真机

真机这条线上，LoRA 与全参的**部署路径完全一样** —— 2.5 节那张分工图原样适用：板子只读舵机、
开相机、下发动作，模型推理放 x86 GPU 机上，中间过 gRPC。唯一的差别在客户端握手时告诉服务端
加载什么：全参给的是一份完整权重，LoRA 给的是**适配器仓**，这时才轮到 3.9 节那个
`--policy.use_peft` 出场 —— 它的用途正是「加载一个已经训好的适配器」，而不是开启 LoRA 训练。
...
3.10 节把这条线跑到底：同一份 3698 集训练集、同一套评测，LoRA 用全参 1/40 的可训练参数拿到 94 / 84 / 98% 的成功率，与全参的 94 / 90 / 96% 基本打平。

**局限性：** 低秩是一种近似，当任务与预训练分布差异极大、或秩 $r$ 取得过低时，旁路的表达力可能不够，效果不及全量微调 —— 3.10.1 节那次 0.16% 的默认名单就是活的反例，三个评测点全 0；而且 $r$、$\alpha$、学习率都需要按任务调。QLoRA 用量化与 offload 换显存，代价是训练变慢，精度则多出一层不确定（3.6.5 节：可能略有损失，但不是必然，要用验证集确认）。显存够时优先用 LoRA，显存紧张才上 QLoRA。具体怎么选，可对照下表：
...
第 2、3 节把 VLA 微调到了自己的机器人上。小模型全量微调、或者给大模型挂 LoRA，单张卡大多还扛得住；可一旦要全量微调 $\pi_0$ 这种 3B 量级、甚至更大的 VLA，单卡立刻开始吃紧：配套代码里那条真机微调脚本（跑的是一批**公开的** SO-101 遥操作数据，我们自己的真机数据见 2.5 节，还没采齐）默认还是单卡在训 $\pi_0$，代价是权重压到 bf16、批量只敢给 8；想把批量放大、想再换大一档的模型，或者手上的卡再小一号，就会直接撞墙。这一节把多卡训练从头讲到尾：先算清训练显存被谁吃掉，再依次讲数据并行、省显存四件套、ZeRO/FSDP 分片、模型并行，最后落到双卡实战。
...
关键的反直觉点在于：**真正撑爆显存的是优化器状态（84 GB），参数只占 14 GB**。优化器状态一个人就占了大头。这也解释了为什么后面省显存的技术里，“对优化器状态动刀”是收益最大的一类。
...
AMP 并不把一切都换成 fp16，它**该用低精度的地方用低精度、该保高精度的地方保高精度**，三件事配合起来：
...
先看清一件事：训练时的**显存峰值，主要由一次前向反向里那批数据的激活值顶起来**。批量越大，同时存在显存里的激活就越多，峰值越高。所以“放不下”常常卡在**这一批的激活**上，而不是参数本身放不下。
...
### 4.4.3 ZeRO Stage 1：先切最大的那块（优化器状态那一块）
...
这里最反直觉的一点要先说透：**这两个进程跑的是一模一样的代码。** 同一份脚本被原样跑了两遍，没有一个当“主”、另一个当“从”那样各执行不同脚本。那它们怎么做到“各算各的数据子集、只在一处打日志”而不互相打架？靠的是 torchrun 给每个进程注入的一组**环境变量**，让每个进程知道“我是第几个”。
...
前面 torchrun 和 Lightning 讲的是“自己写训练循环时怎么上多卡”。但 VLA 这门课真正要训的 ACT、$\pi_0$ 这些策略，跑的是 **LeRobot** 这套现成框架。它的多卡入口是 Hugging Face 的 **`accelerate launch`**，既非 torchrun、也非 Lightning 的 strategy。
...
配套代码里那条真机微调脚本 `code/vla/5_vla_finetune/5_4_so101_real_sft/train_so101_real.sh` 里就有这个分支（提醒一句：它用的是公开的 SO-101 遥操作数据，本讲的实验主线在仿真那一侧，见 2.3~2.4 节）：脚本顶部一个 `NUM_GPUS` 变量，填 1 时直接调 `lerobot-train`，大于 1 时换成上面这条 `accelerate launch --multi_gpu` 的写法，训练参数一个字都不用改——单卡与多卡的差别只在最外面那层启动器。
...
部署后的策略能不能在真实世界继续学？RMA 提供了“在线快速适应”的雏形（靠适应模块在线估计环境隐变量，见 5.1.7 节）；真机在线纠正这一路的代表是 TRANSIC[38]、DAgger 及 HIL-SERL 一类人在环方法（第16讲第 5 节会展开）。但持续学习不忘旧、避免奖励黑客仍是难题。
