...
![图：一条从左到右的链——大规模 Web 数据经预训练得到 Base LLM，再经监督微调得到 fine-tuned LLM；微调那一步上方另有一支箭头，把特定的（私有）知识库喂进来](../../assets/figures/lecture12/files/66bb667edb61a05af03b25e6_65b7a67b97500a346be929a1_how-is-fine-tuning-performed.png){width=75%}
...
![图：同一个医学问题下基座模型与微调后模型的回答对照，左侧只答 "Allergy?"，右侧给出 systemic lupus erythematosus (SLE) 及后续检查建议](../../assets/figures/lecture12/files/66bb667edb61a05af03b265e_65b7a5e7f00d1466ff618b26_what-does-fine-tuning-do-for-the-model.png){width=75%}
...
![图：In-Context Learning 的 prompt 结构——先给两个 German → English 的示例，再让模型翻译一句没见过的新句子](../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimagesfea460ea-84d5-4973-9bc7-dc0e53a13ae0_1340x680.png){width=60%}

做法很简单：在 prompt 里塞几个示例，模型就能"临时"学会新任务。注意"临时"二字——上图那两个 German → English 的示例只活在这一次对话里，模型权重一个字节都没变，下一次提问还得把它们重新贴一遍。
...
这两个维度是**正交关系**：前者描述"更新哪些参数"，后者描述"用什么数据和 loss 更新"。例如，SFT 可以用全量微调做，也可以用 LoRA 做；DPO 可以全量训练，也可以只训练 LoRA 适配器。它们不是互斥分类，而是可以组合的两个坐标轴——记住这一点，后面遇到"LoRA DPO"这类看起来像新方法的名字，就能一眼拆成两个已知的坐标。

![图：一张三行三列的方法地图。列是参数更新范围——全模型全参数 / 模块级全参数 / PEFT（LoRA、QLoRA），图下注明越往右被更新的参数越少；行是训练信号——自下而上为 SFT、离线偏好 DPO 等、在线奖励 PPO / GRPO。九个格子里有六格填着本讲提到的组合：左列是全量 SFT 与全量 DPO，中列只有"冻视觉编码器＋全参训语言骨干"一格，右列自下而上是 LoRA SFT 与 QLoRA SFT（上下叠在同一格）、LoRA DPO、标着第15讲的 LoRA GRPO；红框那格 LoRA SFT 标着"本讲主线"。三个空格不是不能组合，只是本讲没走那条路（本书自绘）](../../assets/figures/lecture12/ref/fig12-1-two-axes-map.png){width=100%}
...
![图：不同层解冻方案的对照——上半部分是训练/验证/测试准确率，下半部分是训练时间，横轴从只训练最后一层排到解冻全部层](../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimages6088451d-43e7-43a7-a615-270b1867cfea_1814x1316.png){width=80%}
...
![图：从 Feature-based 到 Finetuning I、Finetuning II 三档的性能与成本权衡](../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimagesae8e84db-16f9-485d-a0cb-0392fc8aca56_1454x536.png){width=60%}
...
**怎么做的？** QLoRA 是"量化基座模型 + LoRA 训练"的组合：把冻结的基座模型用 4-bit 精度加载，计算时再反量化到较高精度，LoRA 适配器仍以较高精度训练。所以它不是和 LoRA 并列的另一种适配器结构，而是把 LoRA 挂在一个被量化加载的大模型上。
...
![图：左侧是一个 Transformer 层，两个 Adapter 分别插在多头注意力之后和前馈层之后，各自紧接一个 Layer Norm；右侧把其中一个 Adapter 放大——输入先经前馈下投影压到很窄的中间层（图上只画了两个圆点），过非线性，再经前馈上投影回到原宽度，并有一条跳连从入口直接加到出口。（图取自参考文献 [3]）](../../assets/figures/lecture12/adapter_arxiv-1902.00751.png){width=60%}
...
![图：指令微调的数据格式——每条样本由 PROMPT（指令或任务描述）与 COMPLETION（希望模型学会的标准回答）配对组成](../../assets/figures/lecture12/files/66bb667edb61a05af03b265b_65b7a73f7ee95b6c9e261863_using-prompts-to-fine-tune-llms-with-instruction.png){width=75%}
...
数据来源主要有三种：遥操作采集（人类操控机器人录制）、仿真环境生成、跨机器人迁移。其中"仿真环境生成"这一路，除了在仿真里遥操作，还能**完全不靠人**——让视觉强化学习策略在仿真里从零学会任务、自动跑出成功轨迹当数据。本讲用来微调 SO-101 的那批仿真数据就是这么来的，第16讲 6.3 节会把这套"视觉 RL 生成数据"的流程从头讲透。
...
## 2.3 实验：把 SmolVLA 全量微调到 SO-101

配套代码给了两条路线，一条不需要真机、一条用公开真机数据。

**路线一：纯仿真闭环**（`code/vla/5_vla_finetune/5_2_smolvla_full_sft/`）。数据是第16讲 6.3 节那条"视觉 RL 生成数据"链路的产物——SAC 专家在 SO-101 仿真里自己练成，再把成功推演转成 LeRobot 数据集；微调从社区预训练权重 `lerobot/smolvla_base` 出发，全参数更新，不挂任何适配器。三个入口分别是：
...
这条线上真正卡人的不是训练本身，而是**观测特征对齐**：`smolvla_base` 是按三路相机 `camera1/2/3` 预训练的，而这份仿真数据只有一路 `base_camera`，不改名直接跑会抛 `Feature mismatch`。`train_smolvla.sh` 里那个 `--rename_map` 就是干这件事的——把数据集的相机键映射到权重期望的键。缺 `camera2/3` 是允许的，特征校验只要求数据提供的相机是权重期望相机的**子集**。

**路线二：公开真机数据**（`code/vla/5_vla_finetune/5_4_so101_real_sft/train_so101_real.sh`）。同一个脚本按 `MODEL` 变量在 $\pi_0$ / SmolVLA / ACT 之间切换，对一批公开的 SO-101 遥操作数据做全参微调；`code/vla/5_vla_finetune/5_4_so101_real_sft/verify_checkpoint.py` 负责在每个存点上验一遍"这个 checkpoint 到底学没学到东西"。它的判据值得单独看一眼：不用 `corr(pred, action)`，因为这批数据里 `action` 是主臂目标位姿、`state` 是从臂实测位姿，两者天然贴在一起——什么都不学、原样照抄 `state`，逐关节相关系数就能到 0.9 以上。换成"模型误差 / 照抄误差"这个比值，照抄基线恒为 1，模型必须明显低于 1 才算学到了东西。

**该盯什么。** loss 下降只说明拟合在进行，不说明任务做得成；先看评测里的 `pc_success`，再看上面那个验收脚本报出的比值有没有稳定压到 1 以下。但这个比值本身也有边界，脚本的说明里写得很清楚：它做的是**开环逐帧比对**——每一帧都先把策略状态清空再独立预测一次。真机上动作是连着下发的，误差会一步步累积，所以比值压到 1 以下只证明模型学到了非平凡的东西，**不等于它在真机上能把任务做完**；闭环那一步得有真机才能验。仿真那条线还有一个前提要先知道：训练数据所在的 ReachCube 任务在 `so101_sim` 重构后已经下线，`smolvla_eval.sh` 只能把这个 checkpoint 放进现存的 PickPlaceCube40 场景里评，属于任务不匹配，`pc_success` 接近 0 是预期结果、不代表微调失败——脚本注释与 `5_2` 的 README 都写明了这一点。想拿到一个真能读的成功率，得让训练与评测落在同一个现存任务上。
...
![图：左半标注"训练中"——输入 $x$ 兵分两路，一路过冻结的 $W \in \mathbb{R}^{d \times d}$，一路经下投影 $A$（图上标 $\mathcal{N}(0,\sigma^2)$）压到 $r$ 维、再由上投影 $B$（图上标 $B=0$）抬回原维度，两路输出相加；右半标注"训练后"——$W$ 与 $BA$ 已合并成单块 $W_{merged}$，旁路不再出现](../../assets/figures/lecture12/files/Pasted-image-20241120171738-1024x434.png){width=80%}
...
![图：一个 Transformer 层里可以挂 LoRA 旁路的线性层——注意力侧的 Query / Key / Value 与输出投影，以及 MLP/FFN 侧的各个投影矩阵](../../assets/figures/lecture12/files/lora_transformer_targets.png){width=90%}

名字会随架构变——LLaMA/Qwen 这类门控 MLP 是 gate、up、down 三个，传统 Transformer FFN 只有 up、down 两个——但挂法完全一样：**每一个独立的线性层都是一个可以单独决定挂不挂的旁路点**。所以"LoRA 加在哪"不是一个二选一的开关，而是一张按层勾选的清单。
...
QLoRA 论文[2]把 LoRA 加到所有线性层上，并在附录中报告：当覆盖所有层时，rank 对最终表现的影响并不总是显著。实际项目中要结合数据量、过拟合风险和显存选择目标层。像 Hugging Face PEFT 这类框架会按模型类型匹配常见模块，也支持通过 `target_modules` 显式指定，必要时可以设为 `"all-linear"` 覆盖所有线性层。
...
用 LoRA 微调一个模型一共五步：加载并冻结基座、注入旁路、训练、保存适配器、推理时合并回去。
...
2. Rank 不是越大越稳定地更好。QLoRA 论文[2]报告：当 LoRA 覆盖所有线性层时，最终效果和 rank 的关系并不强；Raschka 的博客实验中也有高 rank 表现较好的案例，但这取决于模型、数据和训练预算
...
![图：输入 $x$ 分成两条支路——上支是冻结基座 $W_0$，标注为 NF4 / 4-bit 存储、计算时反量化到 BF16；下支是 LoRA 旁路，标注为 BF16/FP16 的 $A$、$B$ 矩阵，并注明"只有这里接收梯度"；两支输出相加得到 $h = W_0x + \frac{\alpha}{r}BAx$。右下角另有一个分页优化器方框，用虚线指向 LoRA 旁路，标注"显存峰值时 CPU 分页"](../../assets/figures/lecture12/files/qlora_architecture.png){width=85%}
...
![图：基座模型权重显存的粗略对比，四根柱子分两组——7B 在 fp16 下约 14 GB、压到 4-bit 后约 3.5 GB；65B 在 fp16 下约 130 GB、压到 4-bit 后约 32.5 GB（本书自绘；四个数的来源是按 fp16 每参数 2 字节、4-bit 每参数 0.5 字节直接折算）](../../assets/figures/lecture12/files/qlora_memory.png){width=85%}
...
![图：标准微调、LoRA、QLoRA 三种方式的显存构成对照——基座权重、适配器与优化器状态各占多少、各处在什么精度](../../assets/figures/lecture12/files/lora_vs_qlora.png){width=90%}
...
第 2、3 节把 VLA 微调到了自己的机器人上。小模型全量微调、或者给大模型挂 LoRA，单张卡大多还扛得住；可一旦要全量微调 $\pi_0$ 这种 3B 量级、甚至更大的 VLA，单卡立刻开始吃紧——2.3 节那个真机微调脚本默认还是单卡在训 $\pi_0$，代价是权重压到 bf16、批量只敢给 8；想把批量放大、想再换大一档的模型，或者手上的卡再小一号，就会直接撞墙。这一节把多卡训练从头讲到尾：先算清训练显存被谁吃掉，再依次讲数据并行、省显存四件套、ZeRO/FSDP 分片、模型并行，最后落到双卡实战。
...
第二条轴叫**模型并行（model parallelism）**[18]。当一份模型副本本身就装不进一张卡时，数据并行无能为力，这时只能把**模型本身切开**，不同部分放到不同卡上。
...
**ZeRO / FSDP** 做的就是把这份浪费切掉：它仍然站在**数据并行这条轴上**（每张卡还是处理不同的数据），但不再让每张卡都存完整的一份，而是把参数、梯度、优化器状态**沿数据并行的若干张卡切开分摊**，用到哪片临时聚合哪片。于是它既保留了数据并行"切数据、易扩展"的好处，又把"复制掉的显存"省了回来——相当于在数据并行轴上，顺手解决了一部分"放不下"。这一块是后面 4.4 节的主角，这里只先把它在地图上的位置钉住：**它不是第三条轴，而是数据并行轴上的省显存升级。**
...
把这张表读顺，数据并行这条轴的故事就闭环了：4.2 节的 DDP 是这条轴的起点，用"复制整份"换来最简单的近线性提速；ZeRO/FSDP 是这条轴的终点，用"分片 + 临时拼回"把 DDP 复制掉的 N 份冗余一层层切回来，让单卡显存随卡数摊薄。两者不是二选一的对立，而是同一条轴上"零分片"到"完全分片"的两端——DDP 就是 ZeRO 的 Stage 0。
...
![图：Megatron 把 MLP 子层做张量并行的切法。第一个权重矩阵 $A$ 按列切成 $A_1$、$A_2$，输入 $X$ 完整复制给两条支路，各算 $XA_1$、$XA_2$ 并各过一个 GeLU（逐元素激活，无需通信）；第二个权重矩阵 $B$ 按行切成 $B_1$、$B_2$，两条支路各算 $Y_1B_1$、$Y_2B_2$ 得到部分和，最后那个 $g$ 把两个部分和规约（求和）成完整输出。整层只在末尾汇总处发生一次跨卡通信](../../assets/figures/lecture12/ref/megatron-tensor-parallel-mlp_arxiv-1909.08053.png){width=92%}
...
2.3 节那条真机微调脚本 `code/vla/5_vla_finetune/5_4_so101_real_sft/train_so101_real.sh` 里就有这个分支：脚本顶部一个 `NUM_GPUS` 变量，填 1 时直接调 `lerobot-train`，大于 1 时换成上面这条 `accelerate launch --multi_gpu` 的写法，训练参数一个字都不用改——单卡与多卡的差别只在最外面那层启动器。
...
改的时候要走对参数。`lerobot-train` 的 `use_policy_training_preset` 默认是 `true`，启动校验那一步会用**策略自带的预置优化器整个覆盖** `--optimizer.*`——也就是说 `--optimizer.lr` 写了也不生效，学习率仍是预置值。真正管用的入口是策略自己的 `--policy.optimizer_lr`（2.3 节那个真机微调脚本用的就是它）。ACT 的预置学习率是 `1e-5`（`lerobot/policies/act/configuration_act.py` 里的 `optimizer_lr`），双卡有效 batch 翻倍，先试线性缩放的 `2e-5`：
...
![图：一根 7B 模型的单卡显存柱，在五种方案下怎么变。柱子按 4.1.2 节的四类吃货分段：参数 fp16（蓝）、梯度 fp16（绿）、优化器状态 fp32（橙）、激活值（斜纹灰）；红色虚线是标着 40 GB 的单卡容量线。①单卡基线 124 GB，高高在线上；②开省显存四件套后橙色的优化器状态整段消失（搬到卡外）、斜纹的激活段被梯度检查点压扁，落到 31 GB；③DDP 那根柱子仍标着 124 GB，一条双向箭头从 ① 拉到 ③、上面写着"加卡只提速，单卡显存一点没降"，右侧几根灰柱注着"另外 7 张卡各存一模一样的一份"；④ZeRO-2 把梯度段和优化器段各切成 1/8，降到 38 GB；⑤ZeRO-3 把三段全部切成 1/8，降到 26 GB。图下一行注明：斜纹那段是激活值，随批量与序列长度变，图上只画一个示意高度，其余三段按 16 字节/参数精确折算（本书自绘；分片按 8 卡算，与 4.4.6 节表里的公式一致）](../../assets/figures/lecture12/ref/fig12-4-vram-under-each-strategy.png){width=100%}

图上有两处正好解释了这条决策线为什么是这个顺序。**第一处是 ③ 那根柱子**：它和 ① 一模一样高，因为 DDP 只是把数据分给多张卡，每张卡仍然要完整地放下参数、梯度和优化器状态——所以第 1 步只解决"慢"，不解决"放不下"，单卡放不下时加多少张卡都没用。**第二处是 ② 和 ④ 的高低**：省显存四件套（31 GB）比 ZeRO-2（38 GB）还低，而且不需要多卡、不增加通信。这就是第 2 步要排在第 3 步前面的原因——最贵的那一段（优化器状态 84 GB）用 offload 直接搬走，比切成 8 份还划算。真正非分片不可的，是四件套之后仍然顶破容量线的那些情况。
...
Aljalbout 等人的综述《The Reality Gap in Robotics: Challenges, Solutions, and Best Practices》[26]系统总结了这个问题：现实差距不是一个单点误差，而是视觉、动力学、传感器、执行器和软件栈多层误差共同叠加的结果。

Sim2Real 就是研究"如何让仿真训练好的策略在真实世界依然能工作"的子领域。它在过去十年里发展出了一整套方法论，并且在 VLA（Vision-Language-Action）时代继续演化。本节就是要把这套方法论梳理清楚。

> 小结：仿真不是奢侈品而是必需品；但要把仿真里学到的东西真正落地，必须正面应对现实差距。Sim2Real 这一整套方法就是为这一步存在的。
...
![图：左侧"训练"栏是九张仿真渲染图，同一张桌子和同一批几何体在每张里换了一套颜色、纹理、光照和相机角度，桌面从粉到绿到紫无所不有；右侧"测试"栏只有一张真机照片——普通木桌上摆着彩色积木。（图取自参考文献 [27]）](../../assets/figures/lecture12/ref/figures/wri-005-tobin2017-domain-randomization-figure1-crop_arxiv-1703.06907.png){width=78%}
...
这里 $\xi$ 是仿真环境的参数（质量、摩擦、纹理种子等），$p(\xi)$ 是我们设计的随机化分布，$\pi_\theta$ 是参数为 $\theta$ 的策略（这个 $\pi$ 是强化学习里表示策略的通用记号，和本讲反复出现的模型名 $\pi_0$ 没有关系），$\tau$ 是它跑出来的一条轨迹，$\mathcal{L}$ 是这条轨迹上的损失。模型不是在某一个仿真上最优，而是在整个 $\xi$ 分布上的**平均最优**。如果真实世界的参数被这个 $p(\xi)$ "覆盖"了，那么真机就只是其中一个采样。
...
![图：上下两行结构完全相同，差别只在最左边那一格。上行"训练"从高度随机化的仿真图出发，下行"测试"从真机照片出发；两者都先过同一个生成器 $G$，被翻译成同一套规范仿真外观（灰底、纯色积木、彩色机械臂），再送进同一个智能体输出动作。（图取自参考文献 [34]）](../../assets/figures/lecture12/ref/figures/wri-003-rcan-method-summary.png){width=75%}
...
![图：论文的方法总览，分三格。左格是输入输出示例——上排是机械臂的图像观测序列，下排三张图叠着彩色散点，画的是同一个观测下采出的多条候选动作序列。中格是通用形式：观测 $\mathbf{O}_t$ 送进去噪网络 $\varepsilon_\theta(\mathbf{O},\mathbf{A},k)$，一次吐出预测时域 $T_p$ 内的一整段动作 $\mathbf{A}_t$，只执行开头几步再重新预测。右两格是两种网络实现：CNN 版用 FiLM 把观测条件注入一维卷积，Transformer 版用交叉注意力。（图取自参考文献 [36]）](../../assets/figures/lecture12/ref/figures/wri-003-diffusion-policy-overview_arxiv-2303.04137.png){width=80%}

左格那三张彩色散点图是这张图里最该看的：同一个观测下，动作不是收敛到一个点，而是散成几团——**"多模态"在这里是画得出来的**，而这正是下面三条特点里第一条要说的事。
...
所以 DrEureka 属于"自动化 Sim2Real 配置"这一类。它和 VLA 的关系很弱：LLM 在这里不是低层控制器，而是奖励函数与域随机化范围的设计助手。
...
Sim2Real 关心的是：策略已经在仿真里学好，怎样迁移到真实机器人。Real2Sim 关心的是更靠前的一步：真实世界本身怎样变成一个机器人仿真器能使用的世界。这里的目标不是做一个好看的三维模型，而是做一个可以被加载、碰撞、推动、控制、采样、评估的仿真环境。
...
![图：Ye 等人综述的全文外框，自上而下五层：预备知识（三维表示 / 生成模型 / 仿真平台与资产 / 具身智能基础）、三个角色（资产生成器、仿真环境、Sim2Real 桥接）、数据集与评估、挑战与未来方向。三个角色一层被虚线框单独圈出。（图出自参考文献 [39] 图 1）](../../assets/figures/lecture12/ref/figures/wri-018-ye2026-fig1-outline_arxiv-2604.26509.png){width=90%}
...
服装生成相对成熟一些，因为裁剪样片（sewing pattern，衣服在裁剪台上摊平后的二维片形）可以充当仿真原生的中间表示：它不是单纯的视觉网格，而是布料仿真器能直接理解的结构。一般软体物体还没有类似统一的中间表示。PhysGaussian、PhysDreamer、GarmentDreamer、DressCode 代表了早期路线，但离"任意可形变资产都能自动生成并稳定仿真"还有明显距离。
...
早期具身场景多来自程序化生成规则（procedural rules）或已有的真实扫描数据。ProcTHOR、RoboCasa、OmniGibson、BEHAVIOR、Habitat 这类平台提供大量房间、厨房、家居任务。它们的价值在于：场景不是静态图片，而是可以被智能体走动、抓取、打开、移动和重置。
...
![图：中间橙色框是生成式仿真域，内含三列——数字孪生列（自动场景重建 / 运动学重建 / 物理感知与对齐）、数据增强列（视角与几何增强 / 动态与时序增强 / 物理一致性）、任务与示教生成列；左端真实观测（RGB-D 扫描、视频示教、遥操数据）经一支粗箭头进入，右端合成示教喂给真实机器人部署，底部一条虚线箭头从部署折返回真实观测。（图出自参考文献 [39] 图 9）](../../assets/figures/lecture12/ref/figures/wri-018-ye2026-fig9-bridge_arxiv-2604.26509.png){width=90%}
...
![图：左下角方框是 PhysTwin 的核心表示——一只软玩具，放大处画出连接质点的弹簧拓扑（红点为控制点），旁边挂着刚度、接触和三维高斯三组参数；左上角两台相机拍下真实视频，经重建与跟踪得到上排的伪轨迹，下排是仿真渲染出的几何与运动；两排之间的代价函数由几何项、运动项、渲染项三项相加，红色虚线箭头是回传的优化方向。（图出自参考文献 [41]）](../../assets/figures/lecture12/ref/figures/wri-003-phystwin-framework.png){width=85%}
...
Real2Sim 要交付的不是"把真实世界重建得很像"，而是"把真实世界转换成机器人仿真器可执行的三维内容"——资产要同时满足几何有效、物理参数化、运动学可执行和格式兼容四项。Ye 等人[39]的三段式结构可以概括为：
