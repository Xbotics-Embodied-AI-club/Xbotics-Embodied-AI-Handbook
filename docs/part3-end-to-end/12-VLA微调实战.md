...
数据全部出自我们自己的 SO-101 仿真器：仿真里跑出来的成功轨迹转成 LeRobot 数据集，这条生成流程见第16讲 6.3 节。配套代码在 `code/vla/5_vla_finetune/5_2_smolvla_full_sft/`。微调从社区预训练权重 `lerobot/smolvla_base` 出发，全参数更新，不挂任何适配器。三个入口分别是：
