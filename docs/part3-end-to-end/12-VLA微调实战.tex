...
第11讲走读了从 OpenVLA 到 \(\pi_0\) 家族的各类 VLA 模型，看清了一个 VLA
长什么样、动作是怎么被生成出来的。但那时加载的都是别人训练好的权重，跑的是通用能力。真正要把一个
VLA 用到你自己的机器人、你自己的任务上，还差关键一步：微调。
...
一个大模型从诞生到上线，要走完四步：先在海量无标注语料上\textbf{预训练}，拿到一个只会续写文字的基座；再用任务数据\textbf{微调}；然后\textbf{对齐}到人类偏好；最后\textbf{部署}上线。微调卡在这条链的中段：预训练给的是``通识教育''，微调给的是``专业培训''。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/66bb667edb61a05af03b25e6_65b7a67b97500a346be929a1_how-is-fine-tuning-performed.png}

}

\caption{一条从左到右的链：大规模 Web 数据经预训练得到 Base
LLM，再经监督微调得到 fine-tuned
LLM；微调那一步上方另有一支箭头，把特定的（私有）知识库喂进来}

\end{figure}%

两条数据流的体量差得很远：左边那条是整个互联网量级的语料，右边那条常常只有几万条。规模差着好几个数量级，可决定模型在\textbf{你的}任务上好不好用的，是小的那条。
...
{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/66bb667edb61a05af03b265e_65b7a5e7f00d1466ff618b26_what-does-fine-tuning-do-for-the-model.png}

}

\caption{同一个医学问题下基座模型与微调后模型的回答对照，左侧只答
``Allergy?''，右侧给出 systemic lupus erythematosus (SLE)
及后续检查建议}

\end{figure}%

左边把三个症状当成互不相干的线索，只敢回一个最泛的猜测；右边把它们当成一组\textbf{同时出现}的证据，于是收敛到一个具体的诊断，还知道接下来该查什么。差别不在于右边``看过更多资料''------把同一份资料塞进
prompt
也做不到这一步；差别在于参数本身吸收了这类数据的分布，遇到同类输入时它默认就往那个语境上靠。
...
做法很简单：在 prompt
里塞几个示例，模型就能``临时''学会新任务。一个德译英的 prompt 长这样：

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Translate the following German sentences into English:}

\NormalTok{Example 1:}
\NormalTok{German: "Ich liebe Eis."}
\NormalTok{English: "I love ice cream."}

\NormalTok{Example 2:}
\NormalTok{German: "Draußen ist es stürmisch und regnerisch."}
\NormalTok{English: "It\textquotesingle{}s stormy and rainy outside."}

\NormalTok{Translate this sentence:}
\NormalTok{German: "Wo ist der nächste Supermarkt?"}
\end{Highlighting}
\end{Shaded}
...
可以全量训练，也可以只训练 LoRA
适配器。它们是可以组合的两个坐标轴。记住这一点，后面遇到``LoRA
DPO''这类看起来像新方法的名字，就能一眼拆成两个已知的坐标。

\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-1-two-axes-map.png}

}

\caption{一张三行三列的方法地图。列是参数更新范围：全模型全参数 /
模块级全参数 /
PEFT（LoRA、QLoRA），图下注明越往右被更新的参数越少；行是训练信号：自下而上为
SFT、离线偏好 DPO 等、在线奖励 PPO /
GRPO。九个格子里有六格填着本讲提到的组合：左列是全量 SFT 与全量
DPO，中列只有``冻视觉编码器＋全参训语言骨干''一格，右列自下而上是 LoRA
SFT 与 QLoRA SFT（上下叠在同一格）、LoRA DPO、标着第15讲的 LoRA
GRPO；红框那格 LoRA SFT
标着``本讲主线''。三个空格不是不能组合，只是本讲没走那条路}

\end{figure}%

这张图要兑现的就是上面那句``正交、可组合''。看\textbf{列}：最右一列从下往上是
LoRA SFT → LoRA DPO → LoRA
GRPO：参数更新范围一个字没变，变的只是训练信号。看\textbf{行}：最下面一行从左往右是全量
SFT → 冻视觉编码器的模块级训练 → LoRA/QLoRA
SFT：训练信号一个字没变，变的只是动多少参数。所以``LoRA
DPO''从来不是一个新方法，它就是这张图上的一个格子；同理，任何一个你没见过的组合词，先在这两条轴上各定一个坐标，基本就拆明白了。
...
\textbf{代价是什么？} 以 7B 模型为例，如果用 fp16（半精度，每个数 2
字节）训练，仅模型权重就约 14
GB；再加上梯度、优化器状态、激活值、缓存和显存碎片，实际显存常常是数十
GB 到 100 GB 级别，需要梯度检查点（4.3 节展开）、ZeRO / FSDP（4.4
节展开）等技术，通常也需要多张高端 GPU
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimages6088451d-43e7-43a7-a615-270b1867cfea_1814x1316.png}

}

\caption{不同层解冻方案的对照：上半部分是训练/验证/测试准确率，下半部分是训练时间，横轴从只训练最后一层排到解冻全部层}

\end{figure}%

两半张图的趋势形状完全不同：准确率解冻到某一档之后就基本走平，训练时间却一路往上抬。部分微调要找的，就是这两条线\textbf{拉开最大}的那一档。再往右加层，买到的准确率越来越少、付出的时间越来越多。

把视野再放宽一格，从``完全不动预训练模型''到``全量更新''，是一条连续的成本-性能带：

\begin{figure}[htbp]

{\centering \includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimagesae8e84db-16f9-485d-a0cb-0392fc8aca56_1454x536.png}

}

\caption{从 Feature-based 到 Finetuning I、Finetuning II
三档的性能与成本权衡}

\end{figure}%

图上三档正好对应这条带的三个位置。\textbf{Feature-based} 冻结整个预训练
Transformer，只取它的输出 embedding 训一个分类器；\textbf{Finetuning I}
仍冻结 Transformer，但训练顶部一到多个任务层；\textbf{Finetuning II} 连
Transformer
一起更新，也就是全量微调。越往右效果通常越好，代价是越慢越贵。这条带上没有免费的位置，只有适合你手头资源的位置。
...
的缩写，中文常译作``参数高效微调''。它是一大类方法的总称，涵盖多种具体算法：目标是在尽量少更新参数的情况下，让大模型适配新任务。
...
\item
  \textbf{降低遗忘风险}：原始权重不被覆盖，适配器卸载后可以恢复到基座模型行为
\item
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimages89599158-c5bf-4e73-9d31-a388b625e4d2_2262x622.png}

}

\caption{三个并排的紫色标题条把常见 PEFT 方法分成三栏。Prompt
modifications 栏下列着 ``Hard'' prompt tuning、``Soft'' prompt tuning 与
Prefix-tuning；Adapter methods 栏下列着 Adapters 与
LLaMA-Adapter，并有一条连线把 Prefix-tuning 与 LLaMA-Adapter
连起来；Reparameterization 栏下只有一项 Low rank adaptation (LoRA)}

\end{figure}%

三堆分别对应``新参数进输入端''\,``新参数进层内''``不插新模块、改写权重更新量的表示形式''。要留神的是\textbf{第一堆里混着一个不属于
PEFT 的成员}：
...
\item
  \textbf{提示类（Prompt
  modifications）}：都是从输入端下手让模型适配任务，但要分清哪一类真的在训练参数。\textbf{硬提示（hard
  prompt）}
  是人工写的离散文本提示，\textbf{没有任何可训练参数}，严格说属于提示工程，不是
  PEFT；\textbf{软提示（soft prompt）与 Prefix Tuning}
  学的是连续向量提示，有可训练参数，属于参数高效适配。
\item
  \textbf{适配器类（Adapter methods）}：在 Transformer
  内部插入小模块，冻结原始模型，只训练这些适配器。图上那条把
  Prefix-tuning 连到 LLaMA-Adapter
  的线说明分栏不是硬边界：同一个方法可以同时带着两栏的性质。
\item
  \textbf{重参数化类（Reparameterization）}：不直接训练完整的权重变化量，而是用一个更小的参数化形式来表示它。LoRA
  用低秩矩阵近似 \(\Delta W\)，走的就是这条路。
\end{itemize}

下面先讲 LoRA / QLoRA，再回看更早的 Adapter 和软提示方法。

\paragraph{（1）LoRA（Low-Rank
Adaptation）：低秩适配方法}\label{loralow-rank-adaptationux4f4eux79e9ux9002ux914dux65b9ux6cd5}

\textbf{解决什么问题？} LoRA{[}1{]}
要在尽量保留效果的前提下，把可训练参数量降低到原始模型的很小一部分，常见配置只有
0.1\%\textasciitilde1\% 量级。

\textbf{核心直觉：} 许多微调任务的有效更新量 \(\Delta W\)
可以被一个\textbf{较低秩的矩阵很好地近似}。虽然 \(\Delta W\)
本身是个巨大的矩阵，但真正起作用的方向不多，于是可以用两个小得多的矩阵相乘（\(\Delta W = BA\)）来参数化它。这是一个有效的建模假设，也有经验证据支持（3.2
节会讲``内在维度''这条线），但它不是对所有任务、所有层、任意 rank
都成立的事实。

打个比方：原始权重矩阵 \(W\) 是一本百科全书，微调的变化量 \(\Delta W\)
就像是一张勘误表。LoRA
的做法是，不直接写这张勘误表（太大了），而是用两个小表格相乘来生成它。结构长什么样、\(A\)
和 \(B\) 各在哪一步起作用，3.3.1 节会配图逐条走一遍。

\textbf{适用语境：} LoRA 常被用作 PEFT
的对照基线。相对全量微调，它通常能用低得多的显存接近全量微调效果，但具体差距取决于任务、数据质量和
LoRA rank。
...
\paragraph{（2）QLoRA（Quantized
LoRA）：显存友好的微调方案}\label{qloraquantized-loraux663eux5b58ux53cbux597dux7684ux5faeux8c03ux65b9ux6848}

\textbf{解决什么问题？} LoRA
减少了可训练参数，但基座模型本身仍要占显存。对于 33B、65B、70B
这种大模型，光是加载模型权重就要几十 GB 甚至上百 GB 显存。QLoRA{[}2{]}
进一步压缩基座模型，让大模型微调的硬件门槛明显下降。

\textbf{怎么做的？} QLoRA 是``量化基座模型 + LoRA
训练''的组合：把冻结的基座模型用 4-bit
精度加载，计算时再反量化到较高精度，LoRA
适配器仍以较高精度训练。所以它是把 LoRA 挂在一个被量化加载的大模型上，和
LoRA 是叠加关系。
...
的先驱}\label{adapter-tuningpeft-ux7684ux5148ux9a71}

\textbf{解决什么问题？} 这是最早的 PEFT 方法之一{[}3{]}（2019
...
{\centering \includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/adapter_arxiv-1902.00751.png}

}

\caption{左侧是一个 Transformer 层，两个 Adapter
分别插在多头注意力之后和前馈层之后，各自紧接一个 Layer
Norm；右侧把其中一个 Adapter
放大：输入先经前馈下投影压到很窄的中间层（图上只画了两个圆点），过非线性，再经前馈上投影回到原宽度，并有一条跳连从入口直接加到出口。（图取自参考文献
{[}3{]}）}

\end{figure}%

关键在那个残差连接：Adapter
初始化时输出接近零，加回主干等于什么都没改，所以插进去的一瞬间模型行为和原来一样。这和后面
LoRA 把 \(B\)
初始化为零是同一个思路，都是让训练从预训练状态起步而不是从一次随机扰动起步。中间那个瓶颈维度就是它的容量旋钮，越窄参数越少、表达力越弱。

\textbf{适用语境：} 现在用得比 LoRA
少了，但在多任务场景下仍有优势：每个任务一套 Adapter，切换任务就是切换
Adapter。原始论文{[}3{]}报告，在它测的那批 NLP 任务上，仅训练 3.6\%
的参数就能接近全量微调性能。

\paragraph{（4）Prefix Tuning / Prompt
Tuning：用``软提示''来微调}\label{prefix-tuning-prompt-tuningux7528ux8f6fux63d0ux793aux6765ux5faeux8c03}
...
Tuning
更简单，只在输入层加。这些向量是模型自己学出来的``暗号''，人类读不出内容。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2206\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1804\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5990\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
条件 & 常见方法 & 说明 \\
\midrule\noalign{}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1875\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.8125\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
名称 & 含义 \\
\midrule\noalign{}
...
\subsubsection{1.4.1
监督微调（SFT）：教模型``怎么做任务''}\label{ux76d1ux7763ux5faeux8c03sftux6559ux6a21ux578bux600eux4e48ux505aux4efbux52a1}

\textbf{解决什么问题？} 预训练模型只会续写文本，SFT
教它遵循指令、完成任务。

\textbf{怎么做的？}
准备一批（指令，回答）对，让模型看着指令学习生成对应的回答。
本质上就是监督学习：给正确答案，让模型去拟合。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/66bb667edb61a05af03b265b_65b7a73f7ee95b6c9e261863_using-prompts-to-fine-tune-llms-with-instruction.png}

}

\caption{指令微调的数据格式：每条样本由 PROMPT（指令或任务描述）与
COMPLETION（希望模型学会的标准回答）配对组成}
...
图下面那两行 summarization、translation
容易看岔：它们只是同一种数据格式下的两类任务示例，\textbf{不是两个新的微调阶段}。

训练过程和预训练类似：前向传播算
loss，反向传播更新权重，多轮迭代直到收敛。
区别在于数据是标注好的（指令，回答）对，而不是无标注的文本。

\textbf{SFT 的几种常见组织方式：}

下面这些是从不同角度描述 SFT
数据，可以叠加。例如，一个医疗助手数据集可以同时是``指令微调''、``领域适配''和``多任务微调''。

\textbf{指令微调（Instruction Tuning）：} 最常见的 SFT 形式。
用各种各样的指令-回答对来训练，让模型学会遵循指令。 InstructGPT{[}4{]}
...
\textbf{SFT 的局限在哪？} SFT
能教模型``做什么''，但很难教``什么更好''。比如，对于同一个问题，可能有多个语法正确但质量不同的回答，而
SFT
...
\subsubsection{1.4.2
基于人类反馈的强化学习（RLHF）：教模型``什么更好''}\label{ux57faux4e8eux4ebaux7c7bux53cdux9988ux7684ux5f3aux5316ux5b66ux4e60rlhfux6559ux6a21ux578bux4ec0ux4e48ux66f4ux597d}
...
{\centering \includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimages7dfa415c-da9c-4d6f-8de8-ffc9f92272db_1602x952.png}

}

\caption{InstructGPT
的三步流程，图上分三栏。第一栏：从提示集抽一条提示（``Explain the moon
landing to a 6 year old''），标注员写出期望回答，用它监督微调出 SFT
模型。第二栏：同一条提示采样出 A--D 四个模型输出，标注员排出 D
\textgreater{} C \textgreater{} A = B
的顺序，用这批比较数据训练奖励模型。第三栏：抽一条新提示（``Write a
story about frogs''），策略生成输出，奖励模型给出分数 \(r_k\)，再用 PPO
回过头更新策略。（图取自参考文献 {[}4{]}）}
...
\item
  ...
  模型、奖励模型、正在训练的策略模型、参考模型；实际工程会通过共享权重、offload（卸载，4.3.5
  节展开）、缓存等方式降低显存压力
\item
  \textbf{奖励黑客（Reward
...
\subsubsection{1.4.3
直接偏好优化（DPO）：不要奖励模型，直接学偏好}\label{ux76f4ux63a5ux504fux597dux4f18ux5316dpoux4e0dux8981ux5956ux52b1ux6a21ux578bux76f4ux63a5ux5b66ux504fux597d}

\textbf{解决什么问题？} RLHF 工程复杂度高。DPO
的目标是：在离线偏好数据场景中，用更简单的监督式目标表达偏好优化。

\textbf{核心突破：} DPO 论文{[}5{]}指出，在特定偏好建模和 KL
约束假设下，可以把 RLHF 中``奖励建模 +
策略优化''的目标改写成一个简单的偏好分类损失。也就是说，在很多离线偏好数据场景里，你可以跳过显式奖励模型和
PPO，直接用偏好数据来优化语言模型。

\textbf{怎么做的？}
准备三元组数据：（prompt，好的回答，差的回答）。训练目标很直觉：让模型在``好的回答''上的生成概率升高，在``差的回答''上的生成概率降低，同时不要偏离参考模型太远。
...
\textbf{KTO（Kahneman-Tversky Optimization）：} DPO
需要成对的偏好数据（好回答 vs 差回答）， 但实际中收集成对数据很麻烦。
KTO
只需要``这个回答好''或``这个回答差''的二元标签就行，数据收集成本大幅降低。
名字来源于行为经济学的前景理论：人对损失比收益更敏感。

\textbf{ORPO（Odds Ratio Preference Optimization）：} 它把 SFT
和偏好优化合并到同一个训练目标里， 无需额外参考模型。

\textbf{SimPO：} 它进一步简化 DPO，
用序列的平均对数概率来代替参考模型的作用，减少一个模型的显存开销。

\textbf{IPO（Identity Preference Optimization）：} 它修正了 DPO
在偏好数据上容易过拟合的问题， 理论上更鲁棒。

\textbf{GRPO（Group Relative Policy Optimization）：} 它让模型对同一个
prompt 生成一组回答，用组内相对分数来估计优势（advantage）， 从而省掉
PPO 里常见的价值网络（value / critic）。 注意：GRPO
省掉的主要是价值网络，不是所有奖励信号；
奖励仍然可以来自规则、验证器或奖励模型。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1413\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2006\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3327\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3253\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
阶段 & 方法 & 训练信号 & 解决的问题 \\
\midrule\noalign{}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1243\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2607\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2920\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3229\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
阶段 & 做什么 & 用什么方法 & 参数更新方式 \\
\midrule\noalign{}
...
表里``用什么方法''和``参数更新方式''这两列可以自由配对：``用 LoRA 做
SFT''``用 LoRA 做 DPO''这类说法，``用 LoRA''那半交代的是只训练 LoRA
适配器，``做 SFT / 做 DPO''那半交代的是训练目标取监督学习还是偏好优化。

但 VLA 的流程要单独看。VLA
微调的核心通常落在``让策略在新机器人、新场景或新任务上更会动作''这一侧：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1060\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1231\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5191\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2518\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
阶段 & 做什么 & 常见训练信号 & 关注点 \\
\midrule\noalign{}
...
微调是一个循环迭代的过程：微调 → 评估 → 发现问题 → 调整数据或超参 →
再微调。每一轮迭代都在逼近你想要的效果。
...
\item
  ...
  来预测，有些模型使用连续动作头、扩散模型或 flow
  matching（流匹配，把一段噪声沿着学出来的``流''连续变形成动作，第11讲第
  3
  节讲过）模块。不同机器人本体、动作维度和控制频率不同，这部分往往最需要适配。
\end{itemize}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1803\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2978\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5219\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
模型 & 动作表示 & 微调时要关注什么 \\
\midrule\noalign{}
\textbf{RT-2}{[}6{]} & 把机器人动作离散化成文本 token，由 VLM 直接生成 & 更像把网页规模的视觉问答（VQA）数据和机器人轨迹混在一起共同微调（co-fine-tuning）；不适合说成有一个独立动作头。 \\
\textbf{OpenVLA}{[}7{]} & 基于 VLM 输出离散化动作 token & 官方支持端到端微调和 LoRA 微调；小数据适配时 LoRA 更常用。 \\
\textbf{\(\pi_0\)}{[}8{]} & 连续动作块，由 flow matching 动作专家生成 & 重点在动作专家（action expert）与机器人数据的适配，而不是只调语言模型。 \\
\textbf{GR00T N1}{[}9{]} & 多模态骨干 + 连续动作策略模块 & 更像机器人基础模型的策略适配；具体冻结哪些模块取决于任务、数据量和实现配置。 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
VLA
适配到某个新机器人或新任务时，常见数据是几百到几千条机器人操作轨迹。每条轨迹包含一系列的（图像观测，语言指令，动作）三元组。

数据来源主要有三种：遥操作采集（人类操控机器人录制）、仿真环境生成、跨机器人迁移。其中``仿真环境生成''这一路，除了在仿真里遥操作，还能\textbf{完全不靠人}：由仿真器自己跑出成功轨迹当数据。本讲用来微调
SO-101 的那批仿真数据随课程一并提供，不必自己采集。

正因为下游适配数据通常较少，LoRA 等 PEFT 方法在 VLA
微调中经常出现：可训练参数少通常能降低过拟合风险（但仍要靠验证集和早停把住，见
1.3.4 节的备注），也便于在同一基座模型上维护多个任务或机器人适配器。
...
全量微调解冻模型的\textbf{所有参数}，在任务数据上继续训练（机制见 1.3.1
...
对 VLA 来说，第三条尤其关键：像 VLA-0（第11讲第 6
节）、SmolVLA（第11讲第 7
节）这类小体量模型，全参数微调的显存压力有限，往往可以直接全量
...
实践中 VLA 各模块的处理方式常常不一刀切。这里要注意术语（口径见 1.3.4
节）：\textbf{一旦冻结了某个模块，严格说就不再是``全模型全参数微调''，而是``模块级全参数微调''}。两者复现起来差别很大，所以配置表要逐模块写清
trainable / frozen：
...
\subsection{2.3 实验：在仿真里把 SmolVLA 全量微调到
SO-101}\label{ux5b9eux9a8cux5728ux4effux771fux91ccux628a-smolvla-ux5168ux91cfux5faeux8c03ux5230-so-101}

数据全部出自我们自己的 SO-101 仿真器：仿真里跑出来的成功轨迹转成 LeRobot
数据集，这批数据随课程一并提供。配套代码在
\texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/}。微调从社区预训练权重
\texttt{lerobot/smolvla\_base}
出发，全参数更新，不挂任何适配器。三个入口分别是：

\begin{itemize}
\tightlist
\item
  \texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/train\_smolvla.sh}：微调本体，一条
  \texttt{lerobot-train}；
\item
  \texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/smolvla\_eval.sh}：\texttt{lerobot-eval}
  跑多局，报 \texttt{pc\_success} 并自动录像；
\item
  \texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/plot\_loss.py}：把训练日志里的
  loss 抓出来画成收敛曲线。
\end{itemize}

这条线上真正卡人的是\textbf{观测特征对齐}：\texttt{smolvla\_base}
是按三路相机 \texttt{camera1/2/3} 预训练的，而这份仿真数据只有一路
\texttt{base\_camera}，不改名直接跑会抛
\texttt{Feature\ mismatch}。\texttt{train\_smolvla.sh} 里那个
\texttt{-\/-rename\_map}
就是干这件事的：把数据集的相机键映射到权重期望的键。缺
\texttt{camera2/3}
是允许的，特征校验只要求数据提供的相机是权重期望相机的\textbf{子集}。

\textbf{该盯什么。} loss
下降只说明拟合在进行，不说明任务做得成，真正要读的是评测报出的
\texttt{pc\_success}。上面这条流程跑通了训练，但训练与评测不在同一个任务上（数据出自已下线的
ReachCube，只能放进 PickPlaceCube40 场景里评），\texttt{pc\_success}
接近 0 是任务不匹配的预期结果，读不出微调的好坏。新版 SO-101
仿真器就位之后，我们把这条线重做了一遍，让训练与评测落在同一批任务上------那一轮的结果就是下一节。

\subsection{2.4 实验：把 SmolVLA 全量微调到
SO-101}\label{ux5b9eux9a8cux628a-smolvla-ux5168ux91cfux5faeux8c03ux5230-so-101}

2.3
节把流程跑通了，但训练与评测不在同一个任务上，读不出成功率。这一节补上那个数字。

\textbf{数据。} 仿真三个抓放任务 1498 集，加上公开的 SO-101 真机遥操数据
9 个任务 2200 集， 合成一份 3698 集 / 128 万帧 / 10 个任务的训练集。

仿真器长什么样、这份数据怎么产出来的、以及\textbf{两侧要逐项对齐哪九项才混得成一份}，
都在第10讲 3.2 节讲过了，这里只用不重述。要点只有一条：两侧共有的
\texttt{Pick\ up\ a\ cube} 与 \texttt{Pick\ up\ a\ can}
指令文本逐字相同，于是落在同一个任务号下 ------
模型看到的是「同一个任务的 两种画风」，而不是两个任务。

\textbf{训练。} 从 \texttt{lerobot/smolvla\_base}
出发，全参数更新，不挂适配器。

\subsubsection{2.4.1
一个必须亲眼确认的地方}\label{ux4e00ux4e2aux5fc5ux987bux4eb2ux773cux786eux8ba4ux7684ux5730ux65b9}

\texttt{smolvla\_base} 的 \texttt{config.json} 里
\texttt{freeze\_vision\_encoder} 与 \texttt{train\_expert\_only}
\textbf{默认都是 \texttt{true}}，lerobot 的 \texttt{SmolVLAConfig}
默认值也是。不显式覆盖，跑出来的是 2.2
节说的\textbf{模块级}全参数微调，不是全模型全参数微调：

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{{-}{-}policy.freeze\_vision\_encoder=false} \DataTypeTok{\textbackslash{}}
\NormalTok{{-}{-}policy.train\_expert\_only=false}
\end{Highlighting}
\end{Shaded}

判据不是``我传了参数''，是训练日志第一屏那一行：

\begin{verbatim}
num_learnable_params=402737376 (403M)
\end{verbatim}

不加那两个开关时这个数是 \textbf{99,880,992（100M）}，占 4.5 亿总参数的
22\%------视觉塔全程冻着。
微调要跨的正是新机器人带来的视觉差异，冻着视觉塔等于让动作专家一个人扛。
\textbf{这一行不看，就分不清``我以为在做全参微调''和``真的在做''。}

\subsubsection{2.4.2 结果}\label{ux7ed3ux679c}

微调后的策略在仿真里执行任务：

\begin{figure}[htbp]

{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-2-rollout-strip.png}

}

\caption{三个任务各占两行（自上而下：方块、小方块、易拉罐），每个任务八帧走完一整个回合：初始位姿、夹爪降向物体、合拢夹住、抬起进入搬运、停在料箱上方、松手让物体落进箱、手臂退回、回到起始位姿}

\end{figure}%

最后两帧值得单看：成功判据只问``物体在不在箱内''，松手那一刻就已经满足了，回家那一段一分不给。策略照样把它走完了------模仿学习照单全收示教里的一切，包括与判据无关的部分。这也意味着\textbf{想看完整回合得自己录}：评测程序一判成功就结束这一集，录出来的像永远停在松手那一帧。

训练收敛与途中成功率：

\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-2-sim-real-sft-convergence.png}

}

\caption{左轴红线是 flow-matching 损失（对数轴），从 0.56 降到
0.05；右轴蓝线是训练途中每 5000 步一次的成功率，六个点依次为
60\%、100\%、90\%、90\%、90\%、90\%}

\end{figure}%

两条线的走向不一样：损失一路降到底，成功率在第二个评测点就到顶、之后一直在
90\%
附近横着。\textbf{训练损失不是验收指标}------它衡量的是动作序列拟合得多准，而任务成不成只看物体最后在不在箱里。靠
loss 判断``还该不该继续训''会一直得到``该''，得看右边那条线。

最终 checkpoint 在三个仿真任务上各跑 50 局：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.6719\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2500\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
任务 & 成功率 \\
\midrule\noalign{}
\texttt{Pick\ up\ a\ cube\ and\ place\ in\ the\ bin} & 94.0\%（47/50） \\
\texttt{Pick\ up\ a\ small\ cube\ and\ place\ in\ the\ bin} & 90.0\%（45/50） \\
\texttt{Pick\ up\ a\ can\ and\ place\ in\ the\ bin} & 96.0\%（48/50） \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

数据集与权重都公开了，配套代码在
\texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/}：

\begin{itemize}
\item
  仿真数据集与产线源码：\url{https://huggingface.co/datasets/Harrysunshine/so101-sim-pickplace-v2}
\item
  微调后的权重：\url{https://huggingface.co/Harrysunshine/so101-smolvla-sim-real-10task}
\item
  真机 9
  个任务的数据：\url{https://modelscope.cn/datasets/zhuzhuangtian/so101-pick-place-tasks}
\end{itemize}

\subsection{2.5 实物实验：把 SmolVLA 全量微调到 SO-101
真机}\label{ux5b9eux7269ux5b9eux9a8cux628a-smolvla-ux5168ux91cfux5faeux8c03ux5230-so-101-ux771fux673a}

真机走的是同一套流程：同一个 \texttt{lerobot-train} 入口，同样把
\texttt{freeze\_vision\_encoder} 与 \texttt{train\_expert\_only}
关掉，同样拿 \texttt{num\_learnable\_params}
核对。换到真机之后有三件事不一样。

\textbf{一是数据来路。} 训练用的是公开的 SO-101
遥操数据，九个任务。它出自别人的机位，光照与白平衡跟自己这条臂必然不同，所以图像增强里的光度扰动要开得比官方默认宽------亮度与对比度从
0.8--1.2 放到 0.7--1.3，白平衡靠 hue 与 saturation
模拟色温漂移（torchvision 没有独立的白平衡算子，这是标准做法）。

\textbf{二是验收判据不能用相关系数。} 这批数据里 \texttt{action}
是主臂的目标位姿、\texttt{observation.state}
是从臂的实测位姿，两者天然贴在一起。实测过：什么都不学、把
\texttt{state} 原样抄成预测，逐关节 \texttt{corr(pred,\ action)} 就有
0.936--0.995，全部能过
0.9------这条门槛没有任何区分力。换成三个相对量，基线由数据自己定义，不用外部拍阈值：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1675\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6207\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2118\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
指标 & 定义 & 它抓什么 \\
\midrule\noalign{}
\texttt{ratio} & \texttt{MAE(pred,\ action)\ /\ MAE(state,\ action)} & 照抄基线恒为 1，模型要明显低于 1 \\
\texttt{delta\_\allowbreak corr} & \texttt{corr(pred\ −\ state,\ action\ −\ state)} & ``下一步往哪挪''学到没有 \\
\texttt{delta\_\allowbreak std} & \texttt{std(pred\ −\ state)\ /\ std(action\ −\ state)} & 抓``输出≈state''这种坍缩 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

这三个量算的都是\textbf{开环逐帧比对}：每帧独立预测，不接上一帧的结果。真机上动作是连续下发的，误差会一步步累积，所以它们过了只说明模型学到了非平凡的东西，闭环还得真跑。

\textbf{三是推理与执行分家。}
大多数边缘端设备都没有足够的算力和内存把一个 VLA 跑起来------\(\pi_0\)
权重就有 3 GB 多、SmolVLA 也有 900
MB，板上加载完再推理，内存和延迟都撑不住。所以让板子只干必须在现场干的事，模型推理放
x86 GPU 机上，中间过 gRPC：

\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-2-real-deploy-split.png}

}

\caption{左边是边缘端部署板，负责读舵机、开相机、把观测打包发出去、收到动作块下发舵机；右边是
x86 GPU 机，负责加载
checkpoint、跑策略推理、回一个动作块；两者之间一条蓝色箭头向右标注观测（gRPC），一条红色箭头向左标注动作块}

\end{figure}%

服务端起的时候\textbf{不指定模型}：加载哪个 checkpoint
是客户端握手时告诉它的，所以同一个服务端可以先后接 \(\pi_0\) / ACT /
SmolVLA 三种客户端，换模型不用重启。相机与舵机的设备名由 udev
规则固定，不靠 \texttt{/dev/video0} 这种会随插拔重排的号。

真机九任务的权重也公开了：\url{https://huggingface.co/Harrysunshine/so101-smolvla-9task}

\subsection{2.6 本节小结}\label{ux672cux8282ux5c0fux7ed3-1}

\textbf{核心创新：} 全量 SFT
解冻模型的全部参数，在新机器人的数据上一起更新视觉-语言骨干与动作头，给出最高的参数更新自由度；对
VLA-0、SmolVLA 这类小体量
VLA，单卡就能把整模型适配到新本体上，是最直接、最不绕弯子的微调路线。2.4
节那一轮把这条线跑到底：仿真与真机 3698
集混成一份训练集，一个模型在三个仿真任务上拿到 94 / 90 / 96\% 的成功率。

这两节各留下一个不写下来就会重犯的教训。一是\textbf{观测特征对齐}：微调命令本身只有一行，真正把人挡在门外的是相机键对不上（2.3
节）。二是\textbf{默认值不等于你以为的那档}：\texttt{smolvla\_base}
默认冻结视觉编码器，不显式关掉这两个开关，跑出来的是模块级全参微调而非全模型全参微调，可训参数
100M 与 403M 之差全在这里------判据是训练日志里的
\texttt{num\_learnable\_params}，不是命令行里传了什么（2.4 节）。
...
上一节的全量 SFT
把所有参数都解冻了，代价是显存、训练和存储成本都顶到最高。1.3.3 节提过的
PEFT 家族就是冲着这三项成本去的，而其中被用得最多的是
\textbf{LoRA（Low-Rank Adaptation）}，Hugging Face
PEFT、Unsloth、LLaMA-Factory、LeRobot 都为它提供了成熟支持。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.4810\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5190\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
项目 & 显存占用 \\
\midrule\noalign{}
模型权重（fp16） & \textasciitilde14 GB \\
梯度 & \textasciitilde14 GB \\
优化器状态（Adam，一阶 + 二阶两份 fp32 矩） & \textasciitilde56 GB \\
激活值 / 缓存 & 数 GB \textasciitilde{} 数十 GB \\
\textbf{合计} & \textbf{前三项 \(14+14+56 = 84\) GB，再叠上激活值只会更多} \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

这意味着你需要多张 A100/H100 才能舒服地全量微调一个 7B
模型。而且每个任务都要存一份完整的模型副本：如果你有 10 个任务，就要存
10 份 7B 模型，光存储就是 140 GB。
...
LoRA 的理论根基来自 Aghajanyan 等人 2020 年的一篇论文《Intrinsic
...
10 维的子空间里移动，那你根本不需要在全部 1000 个维度上搜索，只需要在那
10 个维度上调整就够了。
...
LoRA
的全部内容就是这一步替换：把``更新一个大矩阵''换成``学两个小矩阵''。
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/Pasted-image-20241120171738-1024x434.png}

}

\caption{左半标注``训练中''：输入 \(x\) 兵分两路，一路过冻结的
\(W \in \mathbb{R}^{d \times d}\)，一路经下投影 \(A\)（图上标
\(\mathcal{N}(0,\sigma^2)\)）压到 \(r\) 维、再由上投影 \(B\)（图上标
\(B=0\)）抬回原维度，两路输出相加；右半标注``训练后''：\(W\) 与 \(BA\)
已合并成单块 \(W_{merged}\)，旁路不再出现}

\end{figure}%

这张图一次交代了三件后面要分开讲的事。\textbf{旁路两个矩阵的初始值不对称}：\(A\)
取小随机数、\(B\)
取全零，所以训练第一步的旁路输出恰好是零，模型从``和原模型完全一样''起步（3.3.4
节）。\textbf{右半那块合并后的方块}说明训练结束后旁路可以被吸收进权重，推理时不多一条分支（3.3.7
节）。最后一点是画法上的：图上为了对称把权重画成了方阵
\(W \in \mathbb{R}^{d \times d}\)，一般线性层里它可以是
\(W_0 \in \mathbb{R}^{d \times k}\)，不影响这条旁路的结构。
...
注意占比是随 \(r\) 线性涨的，而效果不是：\(r\) 从 8 加到
64，参数量翻了八倍，收益却远不成比例，这正是 3.5.1 节要讨论``\(r\)
取多少''的原因。
...
这是 Eckart-Young 定理保证的：在所有秩为 \(r\) 的矩阵中，\(W_r\) 是与
\(W\) 最接近的（在 Frobenius 范数意义下）。

LoRA 的 \(BA\) 本质上就是在学习一个秩为 \(r\)
的矩阵，只不过这个矩阵是靠梯度下降学出来的，而 SVD
是直接算出来的。这给了它更大的灵活性：它不需要近似整个
...
\item
  \textbf{矩阵 \(A\)}：原始 LoRA
  论文{[}1{]}使用高斯随机初始化；很多工程实现会采用 Kaiming
  初始化一类的随机初始化
\item
  \textbf{矩阵 \(B\)}：初始化为全零
\end{itemize}
...
\item
  ...
  \(\alpha/r\) 保持为 2
\item
  也有不少设置固定 \(\alpha\)：原始 LoRA 论文{[}1{]}提到可以把
  \(\alpha\) 设为最先尝试的 \(r\)，QLoRA 论文{[}2{]}的部分实验则使用
  \(\alpha=16\)
\item
...
原始 LoRA 论文{[}1{]}在 GPT-3 上的实验发现，只对注意力层的
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-3-lora-injection-targets.png}

}

\caption{一个 Transformer 层里可以挂 LoRA 旁路的线性层：注意力侧的 Query
/ Key / Value 与输出投影，以及 MLP/FFN 侧的各个投影矩阵}

\end{figure}%

名字会随架构变：LLaMA/Qwen 这类门控 MLP 是 gate、up、down 三个，传统
Transformer FFN 只有 up、down
两个，但挂法完全一样：\textbf{每一个独立的线性层都是一个可以单独决定挂不挂的旁路点}。所以``LoRA
加在哪''是一张按层勾选的清单。
...
已经合并到同精度基座权重中。如果推理时动态挂载适配器、频繁切换多个适配器，或者基座模型是
4-bit 量化格式而不能直接合并，仍可能存在额外开销或工程限制。
...
用 LoRA
微调一个模型一共五步：加载并冻结基座、注入旁路、训练、保存适配器、推理时合并回去。写成伪代码是这样：

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\# 1 加载并冻结}
\NormalTok{model ← 预训练基座权重}
\NormalTok{model 的每个参数.requires\_grad ← false}

\NormalTok{\# 2 在目标层注入旁路}
\NormalTok{r       ← 16          \# 秩}
\NormalTok{alpha   ← 32          \# 缩放因子，实际缩放量是 alpha / r}
\NormalTok{dropout ← 0.05        \# 旁路输入前先 dropout，防过拟合}
\NormalTok{targets ← \{q\_proj, k\_proj, v\_proj, o\_proj, gate\_proj, up\_proj, down\_proj\}}
\NormalTok{对 targets 里的每个 W0：}
\NormalTok{    新建可训练的 A、B，把该层前向改成 h = W0·x + (alpha / r)·B·A·x}

\NormalTok{\# 3 训练}
\NormalTok{按平常的方式训练 model}
\NormalTok{    \# 反向只有 A、B 收到梯度，W0 的梯度既不计算也不存储}

\NormalTok{\# 4 保存}
\NormalTok{只写出全部 A、B        \# 几 MB 到数百 MB，取决于 r 与覆盖的层数}

\NormalTok{\# 5 推理}
\NormalTok{model ← 预训练基座权重}
\NormalTok{挂上第 4 步存下的 A、B}
\NormalTok{合并 W ← W0 + (alpha / r)·B·A    \# 合并后旁路消失，推理不再有额外开销}
\end{Highlighting}
\end{Shaded}

落到具体框架上，这五步依次对应 Hugging Face Transformers 的
\texttt{AutoModelForCausalLM.from\_pretrained}、PEFT 的
\texttt{LoraConfig} +
\texttt{get\_peft\_model}、常规训练循环、\texttt{save\_pretrained}、以及
\texttt{PeftModel.from\_pretrained} +
\texttt{merge\_and\_unload}。参数名和默认值以官方文档为准：\href{https://huggingface.co/docs/transformers/en/model_doc/auto\#transformers.AutoModelForCausalLM}{Transformers
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2237\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2081\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5682\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
Adam 优化器状态 & 56 GB & \textasciitilde320 MB（一阶 + 二阶两个 fp32 矩） \\
\textbf{静态小计} & \textbf{\textasciitilde84 GB+} & \textbf{\textasciitilde14.5 GB+} \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

真正训练时还要加上激活值和实现开销；如果使用混合精度 Adam 并额外保留一份
fp32 主副本，全量微调还会再多一份权重拷贝。所以 24 GB
显卡能否跑起来取决于 batch
...
不同任务对 \(r\)
的需求不同。下面这张表是社区常见的起手档位，\textbf{不是论文结论}，真正的取值要用验证集扫出来：
...
\(\alpha = 16\)，然后只调 \(r\)。这样 \(r\) 增大时，\(\alpha / r\)
会自动减小，起到正则化的效果。

原始 LoRA 论文{[}1{]}并没有要求 \(\alpha\) 必须随 \(r\) 同比增长；QLoRA
论文{[}2{]}在一组实验中使用了 \(\alpha=16\)、\(r=64\)，并把 LoRA
...
Raschka 的结论均来自《Practical Tips for Finetuning LLMs Using
LoRA》{[}11{]}，原文基于 LLaMA-2 7B、Alpaca/LIMA 等设置做了多组实验。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  原始 LoRA 论文{[}1{]}多数实验只在注意力层上做 LoRA，常用
  \(W_q/W_v\)；QLoRA 论文{[}2{]}则把 LoRA
  加到所有线性层上，作为大模型指令微调的默认设置
\item
  Rank 不是越大越稳定地更好。QLoRA 论文{[}2{]}报告：LoRA
  覆盖所有线性层时，最终效果和 rank 的关系并不强；Raschka
  的博客实验中也有高 rank 表现较好的案例，但这取决于模型、数据和训练预算
\item
  LoRA 应用位置很重要。Raschka 的博客实验{[}11{]}建议把 LoRA
  应用到所有层，而不只是 Query/Value
  那两个矩阵；但他也明确说明自己只比较了``Q/V''与``all
  layers''两个设置，其他组合没有系统穷举
\item
  QLoRA 的显存收益和速度代价都依赖实现。Raschka{[}11{]} 的一次 LLaMA-2
  7B 对比中，显存约从 21.3 GB 降到 14.2 GB，训练时间从 1.85 小时增加到
  2.79 小时；按这组数字换算，显存省了约 33\%，训练时间多了约 51\%（与
  3.6.5 节那张表里``约 0.66× 相对吞吐''一致），但这不是一个固定规律
\item
  优化器和学习率调度没有统一最佳。原始 LoRA 论文{[}1{]}常用 AdamW
  配线性衰减（linear decay）；QLoRA
  论文{[}2{]}在指令微调实验中使用常数调度（constant schedule）；Raschka
  的博客实验{[}11{]}观察到余弦调度（cosine scheduler）对 SGD
  的帮助更明显，对 Adam/AdamW 影响较小
\item
  小数据指令微调中要谨慎增加 epoch：训练过久更容易过拟合或削弱原模型能力
\end{enumerate}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-3-qlora-architecture.png}

}

\caption{输入 \(x\) 分成两条支路：上支是冻结基座 \(W_0\)，标注为 NF4 /
4-bit 存储、计算时反量化到 BF16；下支是 LoRA 旁路，标注为 BF16/FP16 的
\(A\)、\(B\) 矩阵，并注明``只有这里接收梯度''；两支输出相加得到
\(h = W_0x + \frac{\alpha}{r}BAx\)。右下角另有一个分页优化器方框，用虚线指向
LoRA 旁路，标注``显存峰值时 CPU 分页''}

\end{figure}%

图上要盯住的是两条分支各自处在什么精度：基座那条以 4-bit NF4
躺在显存里、冻结不动，只在前向经过它时临时反量化到 bf16（另一种 16
位半精度，动态范围比 fp16 宽得多，4.3.2 节展开）参与计算；LoRA
那条旁路从头到尾都是
bf16/fp16，因为它是唯一要接收梯度的部分。两条分支的输出最后相加。量化只发生在不训练的那一侧，这正是
QLoRA 敢压到 4-bit 的底气。
...
QLoRA 论文{[}2{]}从信息论角度说明：在权重近似服从正态分布的假设下，NF4
是非常合适的 4-bit 量化方案。
...
训练过程中，优化器状态（如 Adam
的一阶和二阶矩估计）会占用大量显存。batch size
较大或序列较长时，可能出现显存峰值（memory spike）导致 OOM（Out of
Memory）。
...
\item
  优化器状态默认存储在 GPU 显存中
\item
  GPU 显存不足时，自动将部分优化器状态\textbf{卸载到 CPU 内存}
\item
...
\item
  \textbf{注入 LoRA}：在目标层添加 bf16 精度的 LoRA 旁路
\item
  \textbf{前向传播}：NF4 权重临时反量化为 bf16 → 与输入相乘 → 加上 LoRA
  旁路输出
\item
  \textbf{反向传播}：梯度只流过 LoRA 参数（bf16），基座模型不更新
\item
...
\begin{Highlighting}[]
\NormalTok{量化设置：}
\NormalTok{    以 4{-}bit 加载      ← true}
\NormalTok{    量化类型           ← NF4}
\NormalTok{    双重量化           ← 开}
\NormalTok{    计算 dtype        ← bf16     \# 反量化后按 bf16 算}

\NormalTok{model ← 按上面这组设置加载预训练基座}
\NormalTok{之后完全走 3.4.1 那五步：注入 A、B → 训练 → 存适配器 → 推理}
\end{Highlighting}
\end{Shaded}

框架侧对应 Hugging Face Transformers 的 bitsandbytes
量化接口，四个设置项就是 \texttt{BitsAndBytesConfig} 的
\texttt{load\_in\_4bit}、\texttt{bnb\_4bit\_quant\_type}、\texttt{bnb\_4bit\_use\_double\_quant}、\texttt{bnb\_4bit\_compute\_dtype}。官方来源：\href{https://huggingface.co/docs/transformers/en/quantization/bitsandbytes}{Transformers
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-3-qlora-memory.png}

}

\caption{基座模型权重显存的粗略对比，四根柱子分两组，纵轴取对数：7B 在
fp16 下约 14 GB、压到 4-bit 后约 3.5 GB；65B 在 fp16 下约 130 GB、压到
4-bit 后约 32.5 GB（四个数按 fp16 每参数 2 字节、4-bit 每参数 0.5
字节直接折算）}

\end{figure}%

要注意这张图量的\textbf{只有基座权重那一块}，不含梯度、优化器状态和激活值。即便如此，落差已经很可观：65B
那一组从约 130 GB 掉到约 32.5 GB，正好跨过单张 48 GB
卡的容量线。本节后面那个``单卡微调
65B''的结论，起点就是这一格。同样的账换成 3.6.1 节那个 70B 的例子，是从
140 GB 降到约 35 GB；再加上双重量化省下的 0.37 bit/参数，才腾出了放 LoRA
参数、激活值和框架开销的空间。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1272\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2872\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2961\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2894\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
QLoRA 论文{[}2{]}的标志性成果就落在这张表的最后一行：\textbf{单张 48 GB
的卡微调 65B 参数的模型}。同一个模型走全量微调要 780 GB 以上。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/lora_vs_qlora.png}

}

\caption{标准微调、LoRA、QLoRA
三种方式的显存构成对照：基座权重、适配器与优化器状态各占多少、各处在什么精度}

\end{figure}%

三行摆在一起，能看清每一步省的是哪一块：标准微调整条都要训、都要存优化器状态；LoRA
把基座那一大块冻在 fp16/bf16，只留一小条适配器可训；QLoRA
再把已经冻住的那一大块压成 4-bit。图上的 \texttt{Adapter\ (16-bit)}
指的是 LoRA 的低秩旁路，不是 1.3.3 节讲的传统 Adapter
层，两者名字撞了，结构不是一回事。
...
bf16，这个额外步骤会拖慢训练，但具体幅度取决于模型、batch size、kernel
实现和硬件。下面是一组第三方博客实验中的 LLaMA-2 7B 对比数据，来源为
Raschka{[}11{]}：
...
对近似正态分布的权重很友好，QLoRA 论文{[}2{]}在多组实验中报告它能匹配
16-bit LoRA 或全量微调的效果；但在具体业务任务上，QLoRA
仍可能略逊于未量化 LoRA，需要用验证集确认。

\textbf{经验法则：显存够用 LoRA 就用
LoRA，少一层量化就少一处不确定；显存不够再上
QLoRA，它是这种情况下最成熟的一条退路。}

\subsection{3.7 LoRA
的变体与演进}\label{lora-ux7684ux53d8ux4f53ux4e0eux6f14ux8fdb}

LoRA 自 2021
年提出以来，陆续暴露出一些局限，也因此长出了一批变体。下面五个各针对一处：\(BA\)
只管方向管不了幅度、所有层共用一个 \(r\)、\(A\) 与 \(B\)
的更新步长不匹配、高秩时 \(\alpha/r\)
把梯度压得太狠、以及零初始化让训练从头学起。

本节提到的变体结论分别来自
DoRA{[}12{]}、AdaLoRA{[}13{]}、LoRA+{[}14{]}、rsLoRA{[}15{]} 和
PiSSA{[}16{]}
五篇论文。这些提升都发生在各自的实验设置下，不能直接当作所有任务的固定收益。
...
\textbf{效果如何？} AdaLoRA 论文{[}13{]}报告，在若干 NLU/NLG
...
\textbf{怎么做的？} LoRA+ 给 \(A\) 和 \(B\) 设不同的学习率，两者的比值
\(\lambda = \eta_B / \eta_A\) 是要选的超参。这个比值没有万能取值：论文在
RoBERTa 上找到的好取值在 16 量级，换到 LLaMA 上最优只剩
2\textasciitilde4 量级。

\textbf{效果如何？} LoRA+
论文{[}14{]}报告，在他们的实验设置里不对称学习率改善了收敛，效果提升
1\%\textasciitilde2\%、微调速度最高约 2
倍，且不增加计算开销。这些幅度不是通用常数，换模型换任务都要重新扫这个比值。

\subsubsection{3.7.4
rsLoRA：秩稳定化缩放}\label{rsloraux79e9ux7a33ux5b9aux5316ux7f29ux653e}

\textbf{解决什么问题？} 原始 LoRA 的缩放因子 \(\alpha / r\)
在高秩时会导致学习速度过慢。\(r\) 增大时，\(\alpha / r\)
减小，梯度信号被过度衰减。

\textbf{怎么做的？} rsLoRA 将缩放因子从 \(\alpha / r\) 改为
\(\alpha / \sqrt{r}\)，保证不同秩下的梯度范数和输出范数保持稳定。

\textbf{效果如何？} rsLoRA 论文{[}15{]}报告，在较高 rank
...
\textbf{效果如何？} PiSSA
论文{[}16{]}报告，这种初始化通常能加速收敛，并在一些任务上优于标准
...
\item
  ...
  可训练参数少、容量受限，通常能压低过拟合风险，但不是免疫，1.3.4
  节那条备注仍然成立。
\item
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1671\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1976\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3014\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3340\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
模型 & 动作表示 & LoRA 应用位置 & 常见做法 \\
\midrule\noalign{}
\textbf{OpenVLA}{[}7{]} & 离散动作 token & 论文把 LoRA 加到\textbf{所有线性层}上，不是只挂语言骨干 & 默认 \(r=32\)；论文实测 rank 取多少对最终成功率影响很小 \\
\textbf{SmolVLA} & 连续动作（flow matching） & 动作专家 / 语言骨干按需选择 & 小模型通常优先全量训练动作模块，必要时对 VLM 加 LoRA \\
...
\textbf{骨干这一侧通常靠 LoRA 适配。} 对语言 / 多模态骨干的注意力层和
FFN 层加
LoRA，让模型学会把视觉-语言理解映射到动作输出。但``到底覆盖哪几层''各家差别很大：上表的
OpenVLA 索性挂满所有线性层，3.9 节会看到 LeRobot 给 \(\pi_0\)
的默认名单又是另一种选法。

\textbf{动作头通常全量训练。} 对于连续动作 VLA，动作头（action
head）或动作专家通常是新增模块，参数量相对较小，常见做法是直接全量训练而不用
LoRA。对于 OpenVLA 这类把动作离散化为 token
的模型，动作输出往往复用语言模型的 token 预测头，不一定存在单独的
\texttt{action\_head}。

\subsection{\texorpdfstring{3.9 在 LeRobot 里给 \(\pi_0\) 挂
LoRA}{3.9 在 LeRobot 里给 \textbackslash pi\_0 挂 LoRA}}\label{ux5728-lerobot-ux91ccux7ed9-pi_0-ux6302-lora}

\(\pi_0\) 体量大、全量微调成本高，正是 LoRA
的典型场景。好消息是不用自己写注入代码：LeRobot v0.5.1 已经把 PEFT
接进了训练入口，2.3 节那条 \texttt{lerobot-train} 命令加几个
\texttt{-\/-peft.*} 参数就切换成 LoRA 微调。

命令面上只多两样东西：\texttt{-\/-peft.method\_type=LORA}
选方法，\texttt{-\/-peft.r} 给秩（默认 16，正好落在 3.5.1
节说的那个常用区间）。触发适配器包装的判据是
\texttt{cfg.peft\ is\ not\ None}（\texttt{lerobot\_train.py:246}），所以\textbf{给了
\texttt{-\/-peft.*} 就已经挂上了}，日志里会打印一行
\texttt{Using\ PEFT!\ Wrapping\ model.}。

\begin{quote}
⚠️ \textbf{别把 \texttt{-\/-policy.use\_peft=true} 加进训练命令。} 它和
\texttt{-\/-peft.*} 不是一回事，名字像而已：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3034\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5740\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1226\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
参数 & 含义 & 什么时候用 \\
\midrule\noalign{}
\texttt{-\/-peft.method\_\allowbreak type} / \texttt{-\/-peft.r} \ldots{} & 给一个干净基座\textbf{挂上新的适配器} & \textbf{开始 LoRA 训练} \\
\texttt{-\/-policy.use\_\allowbreak peft=true} & \texttt{-\/-policy.path} 指向的是一份\textbf{已经训好的适配器目录}，先读适配器 config、再据此加载它的基座 & 推理、续训 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

判据在 \texttt{policies/factory.py:550-567}：\texttt{use\_peft=true} 时
LeRobot 会把 \texttt{pretrained\_path}
当成\textbf{适配器}去解析。训练时若写成
\texttt{-\/-policy.use\_peft=true\ -\/-policy.path=lerobot/smolvla\_base}，它会去把
\texttt{smolvla\_base} 当适配器目录读------那是基座，不是适配器。

\texttt{use\_peft}
在训练流程里唯一的作用点是收尾推送（\texttt{lerobot\_train.py:548}，\texttt{push\_to\_hub}
那一支）。
\end{quote}

真正值得读一眼的是 LoRA 挂在哪。LeRobot
给每个策略预置了默认的注入位置，\texttt{lerobot/policies/pi0/modeling\_pi0.py}
里 \(\pi_0\) 那份写成一个正则：
...
拆开看是两段。第一段是\textbf{动作专家内部注意力的 \(W_q\) 与
\(W_v\)}：正好是 3.3.6 节说的、原始 LoRA
论文最早验证的那两个投影矩阵；第二段是\textbf{状态与动作的几个投影层}。视觉塔和语言骨干整个不在名单里，一个适配器都不挂。SmolVLA
的默认名单（\texttt{lerobot/policies/smolvla/modeling\_smolvla.py}）是同一个模子，只把第一段换成它自己的
\texttt{lm\_expert}。

3.8.3
节说过``覆盖哪几层各家差别很大''，这份名单就是一个具体答案，而且答案有点反直觉：\textbf{LeRobot
的默认配置并没有把 LoRA
加到语言骨干上}，而是压在动作专家那一侧，毕竟需要适配到新机器人的是动作，不是语言理解。另外这两份默认配置都把
\texttt{modules\_to\_save}
设成空列表，也就是说连动作投影层都走低秩旁路、不做全参保存；想让某些新增层全参训练，得自己用
\texttt{-\/-peft.full\_training\_modules} 指定。

想改注入范围就覆盖它：\texttt{-\/-peft.target\_modules=\textquotesingle{}all-linear\textquotesingle{}}
是把所有线性层都挂上（QLoRA
论文{[}2{]}那套做法），也可以给一个自己的模块名列表。改完之后先确认可训练参数量确实降下来了，这是
LoRA 有没有真正生效的最直接证据。

\subsection{3.10 实验：把 SmolVLA 用 LoRA 微调到
SO-101}\label{ux5b9eux9a8cux628a-smolvla-ux7528-lora-ux5faeux8c03ux5230-so-101}

和 2.4 节\textbf{同一份训练集、同一套评测}：仿真三个抓放任务 1498
集加真机 9 个任务 2200 集， 合成 3698 集 / 10 个任务；30000 步、warmup
2000、六个图像增强、六卡 effective batch 96、
评测环境参数逐字相同。两轮之间\textbf{唯一的差别就是怎么改权重} ------
只有这样，差出来的东西才 归因得到 LoRA 本身。

\subsubsection{3.10.1
挂在哪些层：默认名单不够用}\label{ux6302ux5728ux54eaux4e9bux5c42ux9ed8ux8ba4ux540dux5355ux4e0dux591fux7528}

3.3.6 节说过 LoRA 通常只挂注意力的 \(W_q\) 与 \(W_v\)。上游 SmolVLA
的默认名单正是这么定的
（\texttt{\_get\_default\_peft\_targets}），但它只挂\textbf{动作专家}那一族，外加五个具身投影：

照默认跑，可训练参数是 742,656，占全模型 4.5 亿的 \textbf{0.16\%}。跑到
8000 步， 三个途中评测点\textbf{全是 0.0\%}，而全参微调在第一个点就有
60\%。

判据不是成功率，是 loss 对读：\textbf{LoRA 在 7000 步是 0.198，全参在
1000 步已经 0.194}。 差的不是方向，是进度 ------ 欠拟合，不是学歪。

回到基座去数，能挂 \(q/v\) 的一共三族：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.4844\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1562\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1875\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
位置 & \(q/v\) 个数 & 默认挂了吗 \\
\midrule\noalign{}
动作专家 \texttt{lm\_\allowbreak expert} & 32 & 挂了 \\
文本塔 \texttt{vlm.model.text\_\allowbreak model} & 32 & \textbf{没有} \\
视觉塔 \texttt{vlm.model.vision\_\allowbreak model} & 48 & \textbf{没有} \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

而 2.4.1
节的结论恰恰是「解冻视觉塔才跨得过新机器人的视觉差异」。默认名单把视觉塔和
文本塔整个冻着，等于让动作专家一个人扛同一件事。三族一起挂、\(r\) 提到
64、五个具身投影
改成全量训练（低秩近似一个本来就不大的投影没有意义）、学习率随之提到
3e-4 ------ 可训练参数 变成 9,851,728，占 \textbf{2.1\%}。

\begin{quote}
\textbf{秩要按基座大小取，不能两个模型共用一个数。} 同样 \(r=64\) 挂到
\(\pi_0\)（40 亿参数、 180 个 \(q/v\)）上只摊到
0.58\%，比这里薄了近四倍。参数占比才是要对齐的量。
\end{quote}

\subsubsection{3.10.2 结果}\label{ux7ed3ux679c-1}

\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-3-lora-vs-full.png}

}

\caption{两条 flow-matching
损失曲线（对数轴）与两组途中成功率：红色是全参 403M、橙色是 LoRA 10M，
LoRA 的损失全程高一截、末点 0.099 对 0.052；深浅两色的方点与圆点是每
5000 步一次的途中成功率， 两者最后都落在 80\textasciitilde90\% 一带}

\end{figure}%

先看它在仿真里跑成什么样 ------
三个任务各一集成功回合，每个任务两行八帧：

\begin{figure}[htbp]

{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-3-lora-rollout.png}
...
最终 checkpoint 在三个仿真任务上各跑 50 局，与 2.4 节那轮并排：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.6587\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1707\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1707\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
任务 & LoRA 10M & 全参 403M \\
\midrule\noalign{}
\texttt{Pick\ up\ a\ cube\ and\ place\ in\ the\ bin} & 94.0\%（47/50） & 94.0\%（47/50） \\
\texttt{Pick\ up\ a\ small\ cube\ and\ place\ in\ the\ bin} & 84.0\%（42/50） & 90.0\%（45/50） \\
\texttt{Pick\ up\ a\ can\ and\ place\ in\ the\ bin} & 98.0\%（49/50） & 96.0\%（48/50） \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

可训练参数是全参的 1/40，三项里一项低 6 个点、一项高 2 个点、一项持平。

\textbf{这张图里最该记住的是那两条线的分叉。}
损失全程差着一截、到最后也没收敛到一处，
成功率却追平了。\textbf{损失差不等于能力差} ------ 2.4.2
节说过训练损失不是验收指标， 那里是纵向的（同一轮里 loss
还在降而成功率早已横住），这里是横向的：两个不同的模型， loss
差了近一倍，任务表现一样。要判断一个微调方案行不行，只能去跑任务。

适配器权重公开在
\url{https://huggingface.co/Harrysunshine/so101-smolvla-lora-sim-real-10task}，
配套代码在 \texttt{code/vla/5\_vla\_finetune/5\_3\_smolvla\_lora/}。

\subsection{3.11 实物实验：把适配器部署到 SO-101
真机}\label{ux5b9eux7269ux5b9eux9a8cux628aux9002ux914dux5668ux90e8ux7f72ux5230-so-101-ux771fux673a}

真机这条线上，LoRA 与全参的\textbf{部署路径完全一样} ------ 2.5
节那张分工图原样适用：板子只读舵机、 开相机、下发动作，模型推理放 x86
GPU 机上，中间过 gRPC。唯一的差别在客户端握手时告诉服务端
加载什么：全参给的是一份完整权重，LoRA
给的是\textbf{适配器仓}，这时才轮到 3.9 节那个
\texttt{-\/-policy.use\_peft} 出场 ------
它的用途正是「加载一个已经训好的适配器」，而不是开启 LoRA 训练。

要现场定的仍是两件：秩取多少（按基座大小定占比，见 3.10.1 节末那条），
以及动作专家之外还要覆盖哪些层。

\subsection{3.12 本节小结}\label{ux672cux8282ux5c0fux7ed3-2}
...
\(B\)，可训练参数压到基座的 1\%
以内，推理前还能把旁路合并回权重、省掉运行时的额外分支（前提见 3.3.7
节）；QLoRA 再叠加 NF4 量化、双重量化、分页优化器三件套，把基座以 4-bit
装下，让大模型微调走进消费级硬件。

3.10 节把这条线跑到底：同一份 3698 集训练集、同一套评测，LoRA 用全参
1/40 的可训练参数拿到 94 / 84 / 98\% 的成功率，与全参的 94 / 90 / 96\%
基本打平。

\textbf{局限性：} 低秩是一种近似，当任务与预训练分布差异极大、或秩 \(r\)
取得过低时，旁路的表达力可能不够，效果不及全量微调 ------ 3.10.1 节那次
0.16\% 的默认名单就是活的反例，三个评测点全 0；而且
\(r\)、\(\alpha\)、学习率都需要按任务调。QLoRA 用量化与 offload
换显存，代价是训练变慢，精度则多出一层不确定（3.6.5
节：可能略有损失，但不是必然，要用验证集确认）。显存够时优先用
LoRA，显存紧张才上 QLoRA。具体怎么选，可对照下表：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2739\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1776\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5486\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
你的情况 & 推荐方法 & 理由 \\
\midrule\noalign{}
...
\textbf{一句话总结：} LoRA
用一对小矩阵接管了``微调该改什么''，把可训练参数压到基座的百分之一量级，QLoRA
再用量化把基座本身塞进消费级显存；落到 LeRobot 上，这一整套只是
\texttt{lerobot-train} 后面多挂几个 \texttt{-\/-peft.*}
参数，而值得你花时间的是\textbf{看清适配器被挂在了哪几层}。
...
量级、甚至更大的
VLA，单卡立刻开始吃紧：配套代码里那条真机微调脚本（跑的是一批\textbf{公开的}
SO-101 遥操作数据，我们自己的真机数据见 2.5
节，还没采齐）默认还是单卡在训 \(\pi_0\)，代价是权重压到
bf16、批量只敢给
8；想把批量放大、想再换大一档的模型，或者手上的卡再小一号，就会直接撞墙。这一节把多卡训练从头讲到尾：先算清训练显存被谁吃掉，再依次讲数据并行、省显存四件套、ZeRO/FSDP
分片、模型并行，最后落到双卡实战。

\textbf{先说清这一节的定位和读法。}
它是本讲篇幅最大的一节，接近全讲的三分之一，而且到 4.6
节之前讲的都是\textbf{与 VLA 无关的通用分布式训练知识}：同一套
DDP、ZeRO、FSDP，训语言模型、训视觉模型、训 VLA
完全一样。这么安排是有意的：全量微调 \(\pi_0\)
是本书第一次真正撞上显存墙，而撞墙之后要用的那套工具与 VLA
本身无关，索性在这里一次讲透。相应地，读法可以分两种：\textbf{只想把
\(\pi_0\) 在两张卡上跑起来的，直接跳到 4.6 节}，那里是完整的
\texttt{accelerate\ launch}
命令和必须手动处理的坑；\textbf{想搞懂命令背后每个参数在做什么的}，从
4.1 节的显存账本按顺序读下来。
...
到目前为止，本书训过的策略大多还能塞进一张卡：行为克隆、ACT、扩散策略（第10讲）的参数量从几百万到几千万，单张消费级显卡轻松拿下。但
VLA 这条线一路走下来，模型越做越大：
...
\item
  \textbf{\(\pi_0\)} 这一代 flow matching 策略，主干带上 VLM
  后参数量也到了 3B 量级；
\item
  \textbf{GR00T N1} 这类通用人形基础模型，规模还在继续往上走。
\end{itemize}
...
解决这两堵墙，靠的就是\textbf{把训练摊到多张卡上}。但``多卡''是一整套互相配合的技术：有的专门省显存，有的专门切模型，有的专门提速度。名词也多------DDP、ZeRO、FSDP、张量并行、流水线并行------初学者很容易被淹没。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1779\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.0842\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2092\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5287\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
吃货 & 精度 & 每参数字节数 & 口径 \\
\midrule\noalign{}
参数 & fp16 & 2 & 权重一份 \\
...
字节}。这正是 ZeRO 论文{[}17{]}里反复用的那个``16 字节/参数''基准账本。

\begin{quote}
备注（\textbf{和 3.1.1 节的口径对一下}）：3.1.1 与 3.4.2 节那两张表把 7B
的优化器状态记作 56 GB，这里记作 84 GB，差的正是\textbf{那份 fp32
参数主副本}。那两张表只算了 Adam 的两个 fp32 矩（\(8\) 字节/参数
\(\times\) 7B = 56 GB），本节按 ZeRO
论文的完整口径把主副本也算进来（\(12\) 字节/参数 \(\times\) 7B = 84
GB）。两个数都对，只是口径不同；本节之后一律用 16 字节/参数这套账。
\end{quote}
...
\paragraph{（3）为什么 24 GB / 40 GB
的卡轻易就爆}\label{ux4e3aux4ec0ux4e48-24-gb-40-gb-ux7684ux5361ux8f7bux6613ux5c31ux7206}
...
关键的反直觉点在于：\textbf{真正撑爆显存的是优化器状态（84
GB），参数只占 14
GB}。优化器状态一个人就占了大头。这也解释了为什么后面省显存的技术里，``对优化器状态动刀''是收益最大的一类。
...
parallelism）}{[}18{]}。思路非常朴素：每张卡上放一份\textbf{完整的模型副本}，把一个大批量的数据\textbf{切成几份}，每张卡各算一份，最后把各卡的梯度汇总（all-reduce）对齐，保证所有副本始终一致。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.52\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/data-parallel-overview_colossalai-docs.png}

}

\caption{数据并行总览：每张 GPU 上都放一份完整的模型副本（Model
...
parallelism）}{[}18{]}。一份模型副本本身就装不进一张卡时，数据并行无能为力，这时只能把\textbf{模型本身切开}，不同部分放到不同卡上。
...
{\centering \includegraphics[width=0.68\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/model-parallel-tensor-overview_colossalai-docs.png}

}

\caption{模型并行（张量并行）示意：上方是非分布式的整块矩阵乘
...
N 份。这其实是巨大的浪费：明明有 N 张卡，同一份优化器状态却存了 N 遍。
...
4.4
节的主角，这里只先把它在地图上的位置钉住：\textbf{它是数据并行轴上的省显存升级，没另开一条轴。}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1418\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.8582\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
小节 & 在地图上的位置 \\
\midrule\noalign{}
...
4.6 双卡实战 & 把上面这些落到实操：先讲通用启动器 \texttt{torchrun}，再落到 LeRobot 真正用的 \texttt{accelerate\ launch}，在两张卡上把一次完整训练跑起来。 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
PyTorch 早期提供过一个非常好用的接口{[}19{]}：把模型包一层
...
{\centering \includegraphics[width=0.62\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/ring-allreduce_uber-horovod-blog.jpg}
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/ddp-gradient-bucketing-allreduce_arxiv-2006.15704.png}
...
all-reduce
已经基本传完了。整场梯度同步的时间，被尽可能多地塞进了反向计算的缝隙里，几乎不额外占用墙钟时间。这就是
DDP 能做到``多卡近线性加速''的工程底气。

\begin{quote}
备注：桶的大小是个权衡：桶太小，发起的 all-reduce
次数多、每次启动有固定开销；桶太大，又要等很久才凑齐一桶、重叠的机会变少。框架给了默认桶大小（PyTorch
的 \texttt{DistributedDataParallel} 默认 25
MB{[}19{]}），一般不用手动调。
\end{quote}
...
两节的活）；但在动用多卡分片这种重武器之前，还有一整套\textbf{单卡就能用、每张卡都能用}的省显存工具，门槛低、即插即用，往往先把它们用满，问题就解决了一大半。这一节讲的就是这套工具箱里最常用的四件：\textbf{混合精度、梯度累积、梯度检查点、offload}。

为什么是这四件、它们各自管用在哪？还是回到 4.1
节立下的那本\textbf{显存账本}。训练显存有四类吃货：
...
第一件，也是最该默认打开的一件：\textbf{混合精度训练（Automatic Mixed
Precision，AMP）}。
...
{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fp32-fp16-bf16-numeric-range_google-cloud-blog.png}

}

\caption{fp32 / fp16 / bf16 三种浮点格式的位宽与数值范围对比。fp32 用 8
位指数 + 23 位尾数，范围极宽、精度高；fp16 把指数压到 5
位，能表示的最小正数只到约 \(6\times10^{-8}\)，范围窄得多；bf16 保留 8
位指数（范围和 fp32 一样宽）、只砍尾数到 7 位，用精度换范围}

\end{figure}%

指数位宽决定的是\textbf{能表示多小}，而训练里恰好有两个地方会撞上 fp16
那个 \(6\times10^{-8}\) 的下限：
...
\item
  ...
  能表示的最小值，就被直接四舍五入成 0，梯度没了，这部分参数永远不更新。
\item
...
AMP 并不把一切都换成
fp16，它\textbf{该用低精度的地方用低精度、该保高精度的地方保高精度}，三件事配合起来：
...
\item
  ...
  的半精度算力，是省与快的主要来源。
\item
  \textbf{保留一份 fp32 主副本（master
  copy）}：优化器手里始终攥着一份高精度的参数副本，每一步参数更新都作用在它身上，半精度副本只是从它派生出来供前向用。这样``微小更新被尾数吃掉''的问题就被挡在
  fp32 这一侧。这正是 4.1 节账本里那份 fp32 主副本的来历。
\item
...
\(S\)。根据链式法则，所有梯度也会跟着同比例放大 \(S\) 倍。本来要下溢成 0
的小梯度，被抬到 fp16 能表示的范围里活了下来。等梯度安全地回到 fp32
...
主副本解决的是\textbf{精度}问题（微小更新别被尾数吃掉）。两件事分别由两个机制兜住，缺一不可。
\end{quote}
...
Floating Point）}{[}21{]}，它换了个权衡思路。

回到上面那张图最下面一行：bf16 同样是 16 位、同样每个数 2
字节，但它把位宽\textbf{重新分配}了：保留\textbf{完整的 8 位指数}（和
fp32 一样宽的动态范围），代价是尾数只剩 7 位（精度比 fp16 的 10
位还低）。
...
先看清一件事：训练时的\textbf{显存峰值，主要由一次前向反向里那批数据的激活值顶起来}。批量越大，同时存在显存里的激活就越多，峰值越高。所以``放不下''常常卡在\textbf{这一批的激活}上，而不是参数本身放不下。
...
checkpointing）}{[}22{]}，直接冲着激活值这块最大的可压缩项下手，思路是\textbf{用算力换显存}。
...
{\centering \includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/gradient-checkpointing-recompute-graph_cybertronai.png}

}

\caption{梯度检查点的前向计算图：一条很长的前向链上，绝大多数中间节点的激活都不保存，只在少数选定的``检查点（checkpoint，图中双圈节点）''处保留激活。其余节点的激活在前向阶段算完即丢，等反向需要时再临时重算}

\end{figure}%

图上那条长链就是一次前向：每个节点是一层的中间输出。标准训练里这一整条都得留着等反向来取，链有多长、显存就压多少。图里只有双圈那几个节点是被保留的检查点，其余单圈节点算完就丢，\textbf{丢掉的部分才是这一招省下来的显存}。
...
{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/gradient-checkpointing-backprop-graph_cybertronai.png}

}

\caption{梯度检查点下反向传播的执行：\(f\) 是前向（重算）、\(b\)
是反向。反向是分段走的：每要算一段的梯度，就先从上游检查点重做一次局部前向
\(f\) 补出该段激活，紧接着做这一段的反向
...
\paragraph{\texorpdfstring{（3）\(\sqrt{N}\)
的甜点}{（3）\textbackslash sqrt\{N\} 的甜点}}\label{sqrtn-ux7684ux751cux70b9}

那省多少、亏多少？有一个很经典的结论{[}22{]}：如果把一个 N 层（或 N
...
4.1 节那本账里最大的一块：\textbf{优化器状态（84 GB
...
\begin{quote}
备注：offload
解决的是优化器状态/参数``放不下''，靠的是把它们挪出显存。这和后面 4.4 节
ZeRO
...
则补上前三件没碰的优化器状态和参数。所以它们可以\textbf{层层叠加}：一份单卡训练里同时开混合精度、梯度检查点、梯度累积，再视情况加
...
\tightlist
\item
  \textbf{混合精度 AMP}：前向反向用 fp16/bf16 算、保留 fp32 主副本、用
...
也就是说，4.1 节那笔``7B 模型前三类就要 112 GB''的账，DDP
...
那一套通信骨架，模型本身没有被切成几段塞到不同卡（那是后面 4.5 节张量 /
流水线并行的活）。它动的只是``每张卡到底要不要存全份冗余状态''这一件事。所以可以把
...
这里是往\textbf{横向}切：同一批冗余状态，offload 把它挪到 CPU/盘，ZeRO
把它分摊到各卡。两者其实是 DeepSpeed ZeRO
体系里互补的两条腿，本节先讲``横向切''。
\end{quote}
...
\textbf{all-gather（全收集）}{[}23{]}。它和 DDP 的 all-reduce
是一对近亲：all-reduce
...
{\centering \includegraphics[width=0.96\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fsdp-sharding-allgather-reducescatter_pytorch.png}

}

\caption{同一批要在各卡间对齐的数据，三种集合通信的差别：左边 All-Reduce
把各卡的整份数据规约（求和）后让人手一份相同结果，是 DDP
...
\item
  ...
  all-gather（把各片拼成完整梯度）。梯度本来就要分片存时，第二步
...
1：先切最大的那块（优化器状态那一块）}\label{zero-stage-1ux5148ux5207ux6700ux5927ux7684ux90a3ux5757ux4f18ux5316ux5668ux72b6ux6001ux90a3ux4e00ux5757}

切谁先切？回到 4.1 节的账本：混合精度 + Adam
下，\textbf{优化器状态是前三类里最大的一块}。

把一个参数量为 \(\Psi\) 的模型摊开（以下都按每个参数算）：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2232\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1837\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5931\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
这一类状态 & 每参数字节 & 说明 \\
\midrule\noalign{}
...
{\centering \includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/zero-three-stage-partitioning_arxiv-1910.02054.png}

}

\caption{ZeRO 三阶段分片对比。每一行是一种方案，横向从 \(\text{gpu}_0\)
到 \(\text{gpu}_{N-1}\)
...
\(K{=}12\)），\(N\) 为数据并行度（正文写作 \(N_d\)）；图上把参数量记成
\(M\)，就是正文的 \(\Psi\)}

\end{figure}%

\begin{quote}
备注：ZeRO 论文{[}17{]}给的直觉例子：一个 7.5B 参数的模型，混合精度 Adam
下基线每卡约
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2375\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2672\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4953\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
\item
  ...
  同量级（不显著增加），却把每卡显存从 \(16\Psi\) 一路压到趋近
  \(2\Psi\)。所以只要在用 DDP，开到 Stage 2 通常是默认值得做的。
\item
  ...
  都装不下时才动用它。
\item
  \textbf{省显存的来源是``分摊''，不是``消失''。} 那 \(16\Psi\)
...
后来把同一思路做成了\textbf{原生}功能：\textbf{FSDP（Fully Sharded Data
Parallel，完全分片数据并行）}{[}23{]}。名字里的``Fully Sharded''就是
ZeRO Stage 3 的精神：参数、梯度、优化器状态\textbf{全部分片}，每张卡只存
1/N。
...
{\centering \includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fsdp-workflow_pytorch.png}

}

\caption{FSDP 的一步训练工作流。模型被切成若干 FSDP 单元（图中 FSDP
...
这类策略）；放到完全分片就是 Stage 3。FSDP 因此能在 Stage 2/3
之间按显存与通信的权衡取舍。

\begin{quote}
备注：DeepSpeed ZeRO 和 PyTorch FSDP
解决的是同一个问题、给的是同一类答案，差别主要在工程实现与生态（配置方式、与
offload/混合精度的衔接、和训练框架的集成）。4.6 节的实战里会看到，在
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2015\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3761\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4223\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
把这张表读顺，数据并行这条轴的故事就闭环了：4.2 节的 DDP
是这条轴的起点，用``复制整份''换来最简单的近线性提速；ZeRO/FSDP
是这条轴的终点，用``分片 + 临时拼回''把 DDP 复制掉的 N
份冗余一层层切回来，让单卡显存随卡数摊薄。两者是同一条轴上``零分片''到``完全分片''的两端，DDP
就是 ZeRO 的 Stage 0。
...
流水线并行。本节的目的是让你懂它的原理，更要懂它的\textbf{边界}------到底什么时候才轮得到它上场（见第
4.5.5 小节）。
\end{quote}
...
Megatron-LM{[}24{]} 里把 Transformer 的两个核心子层（MLP
...
GeLU 激活，再乘 \(B\)（这里的 \(A\)、\(B\) 沿用 Megatron 原图的记号，指
MLP 自己那两个权重矩阵，和 3.3 节 LoRA 的低秩矩阵 \(A\)、\(B\)
只是重名）。关键的切法是把第一个权重矩阵 \(A\) \textbf{按列}切成
\([A_1, A_2]\)，分给两张卡：
...
{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/megatron-tensor-parallel-mlp_arxiv-1909.08053.png}

}

\caption{Megatron 把 MLP 子层做张量并行的切法。第一个权重矩阵 \(A\)
按列切成 \(A_1\)、\(A_2\)，输入 \(X\) 完整复制给两条支路，各算
\(XA_1\)、\(XA_2\) 并各过一个
GeLU（逐元素激活，无需通信）；第二个权重矩阵 \(B\) 按行切成
\(B_1\)、\(B_2\)，两条支路各算 \(Y_1B_1\)、\(Y_2B_2\)
得到部分和，最后那个 \(g\)
...
{\centering \includegraphics[width=0.92\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/megatron-tensor-parallel-attention_arxiv-1909.08053.png}

}

\caption{注意力子层的张量并行：多头注意力按头切到两张卡，卡 1
...
{\centering \includegraphics[width=0.96\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/gpipe-naive-model-parallel-idle_arxiv-1811.06965.png}

}

\caption{朴素按层切的严重空转。横轴是时间，四行是四张卡。一个 batch
进来，前向 \(F_0\) 从卡
...
救法是 \textbf{micro-batch（微批）}{[}25{]}：别让一整个 batch
作为一个不可分的整体串行流过，把它切成若干个
micro-batch，一份接一份地连续喂进流水线。这样当卡 0 算完第 1 个
micro-batch、把它交给卡 1 时，卡 0 立刻就能接着算第 2 个
...
{\centering \includegraphics[width=0.98\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/gpipe-pipeline-bubble_arxiv-1811.06965.png}

}

\caption{micro-batch 把流水线填满的调度。一个 batch 被切成 4 个
...
把这张图和上一张并排看，差别一眼就出来：上一张每一列时间片里只有一个方块（一张卡在算、三张在等），这一张中段每一列都被填满了（四张卡各在处理不同的
micro-batch）。变的不是硬件，只是喂进去的数据被切细了。
...
\item
  ...
  \textbf{1F1B}（one-forward-one-backward）调度------它出自
  PipeDream{[}49{]} 那条流水线并行线，不是 GPipe
  原文的调度------让每张卡一旦能做反向就\textbf{前向、反向交替着做}（做一个前向紧接做一个能做的反向），把前向和反向更紧密地咬合在一起，从而减少各卡积压的激活、也把气泡进一步压平。直觉上：1F1B
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0774\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5789\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3438\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
顺序 & 先试什么 & 它解决的瓶颈 \\
\midrule\noalign{}
...
\item
  \textbf{触发条件}：数据并行（含
  FSDP）的底线是``每卡都能完整跑一次前向''；\textbf{单独一层}都放不下、这条底线被突破时，才需要模型并行。
\item
...
\begin{quote}
一句话总结：单独一层都放不下、数据并行救不了时，才转到模型并行轴------张量并行把一层的大矩阵横切到多卡（机内
...
前面几节，把多卡训练的道理从头到尾铺了一遍：4.1 节算清显存账本、4.2 节讲
DDP 怎么靠 all-reduce 近线性提速、4.3
节用省显存四件套在单卡里抠空间、4.4 节用 ZeRO/FSDP
沿各卡分片冗余状态、4.5
节把模型本身横切纵切到多卡。这些都是``它为什么省、为什么快''的原理。
...
Hugging Face 的
\texttt{accelerate\ launch}，给出双卡命令并逐参数拆解（第 4.6.4
小节）。收尾给一棵选型决策树（第 4.6.5
...
假设你手上有一份自己写的单卡训练脚本（下面的命令里记作
train.py，是个占位名，本书仓库里没有这个文件），平时就是
...
node = 每台机器，这里就一台）。torchrun
会\textbf{把同一份脚本同时启动两遍}，得到两个独立的 Python
进程，分别绑到 GPU 0 和 GPU 1 上。

这里最反直觉的一点要先说透：\textbf{这两个进程跑的是一模一样的代码。}
同一份脚本被原样跑了两遍，没有一个当``主''、另一个当``从''那样各执行不同脚本。那它们怎么做到``各算各的数据子集、只在一处打日志''而不互相打架？靠的是
torchrun
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1824\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.8176\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
变量 & 含义 \\
\midrule\noalign{}
...
\item
  ...
  节讲的``各算各的数据''）。PyTorch 的 \texttt{DistributedSampler}
  就是读 \texttt{RANK} / \texttt{WORLD\_SIZE} 自动切分的。
\item
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1860\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4297\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3843\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
strategy & 它是什么（对应小节） & 什么时候用 \\
\midrule\noalign{}
\texttt{"ddp"} & 数据并行，每卡一份完整副本（4.2 节） & 模型单卡\textbf{放得下}、只想靠多卡提速 \\
\texttt{"fsdp"} & 完全分片数据并行，参数/梯度/优化器全切（4.4 节） & 模型单卡\textbf{放不下}，要沿各卡分片 \\
\texttt{"deepspeed"} & DeepSpeed ZeRO 各 stage + offload（4.4 节） & 要 ZeRO 分阶段、或要把分片 offload 到 CPU/盘 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
\item
  \textbf{\texttt{fsdp}：放不下就分片。} 对应 4.4 节的 FSDP（PyTorch
  原生 ZeRO-3），把参数/梯度/优化器状态沿各卡切开，每卡只存
...
可以这样记：\textbf{这三个开关，就是把 4.2 节（DDP）和 4.4
节（ZeRO/FSDP）的原理，落到了 \texttt{Trainer(strategy=...)} 这一行上。}
...
这套现成框架。它的多卡入口是 Hugging Face 的
\textbf{\texttt{accelerate\ launch}}，既非 torchrun、也非 Lightning 的
strategy。

下面的命令与参数名都以 \textbf{LeRobot v0.5.1} 为准（本书
\texttt{code/pyproject.toml}
钉的就是这个版本）。这个框架迭代很快，换版本前先对一遍
\texttt{lerobot-train\ -\/-help}。
...
\ExtensionTok{accelerate}\NormalTok{ launch }\DataTypeTok{\textbackslash{}}
  ...
  \AttributeTok{{-}{-}policy.push\_to\_hub}\OperatorTok{=}\NormalTok{false }\DataTypeTok{\textbackslash{}}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3614\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6386\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
参数 & 含义 \\
\midrule\noalign{}
...
\texttt{-\/-dataset.repo\_\allowbreak id=...} & 训练用的数据集（Hugging Face 仓库 id） \\
\texttt{-\/-policy.type=act} & 训哪种策略，这里是 ACT；换 \(\pi_0\) 改成对应类型 \\
\texttt{-\/-policy.push\_\allowbreak to\_\allowbreak hub=false} & 不往 Hugging Face Hub 推权重。这一项\textbf{不能省}：它默认是 \texttt{true}，而一旦为真又没给 \texttt{-\/-policy.repo\_\allowbreak id}，\texttt{lerobot-train} 启动时就直接报``\texttt{policy.repo\_\allowbreak id} argument missing''退出 \\
...
配套代码里那条真机微调脚本
\texttt{code/vla/5\_vla\_finetune/5\_4\_so101\_real\_sft/train\_so101\_real.sh}
里就有这个分支（提醒一句：它用的是公开的 SO-101
遥操作数据，本讲的实验主线在仿真那一侧，见 2.3\textasciitilde2.4
节）：脚本顶部一个 \texttt{NUM\_GPUS} 变量，填 1 时直接调
\texttt{lerobot-train}，大于 1 时换成上面这条
\texttt{accelerate\ launch\ -\/-multi\_gpu}
的写法，训练参数一个字都不用改------单卡与多卡的差别只在最外面那层启动器。

\paragraph{（3）混合精度交给 accelerate
控}\label{ux6df7ux5408ux7cbeux5ea6ux4ea4ux7ed9-accelerate-ux63a7}

这里有个容易记反的地方：\textbf{\texttt{lerobot-train}
的混合精度从头到尾都由 accelerate 接管，单卡也一样。} LeRobot v0.5.1 的
\texttt{lerobot/scripts/lerobot\_train.py} 里，训练步整个包在
\texttt{accelerator.autocast()} 下（源码注释就写着 \emph{Let accelerator
handle mixed precision}），而策略配置里那个 \texttt{-\/-policy.use\_amp}
在这个文件里一次都没被读到------它只在推理侧的
\texttt{lerobot-eval}、\texttt{lerobot-record}
里生效。所以不论单卡多卡，训练精度都用启动器的
\texttt{-\/-mixed\_precision} 控：
...
\ExtensionTok{accelerate}\NormalTok{ launch }\AttributeTok{{-}{-}multi\_gpu} \AttributeTok{{-}{-}num\_processes}\OperatorTok{=}\NormalTok{2 }\AttributeTok{{-}{-}mixed\_precision}\OperatorTok{=}\NormalTok{bf16 }\DataTypeTok{\textbackslash{}}
  \VariableTok{$(}\FunctionTok{which}\NormalTok{ lerobot{-}train}\VariableTok{)} \AttributeTok{{-}{-}dataset.repo\_id}\OperatorTok{=}\VariableTok{$\{HF\_USER\}}\NormalTok{/my\_dataset }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}policy.type}\OperatorTok{=}\NormalTok{act }\AttributeTok{{-}{-}policy.push\_to\_hub}\OperatorTok{=}\NormalTok{false}
\end{Highlighting}
\end{Shaded}

\texttt{-\/-mixed\_precision} 取 \texttt{fp16} 或 \texttt{bf16}（对应
4.3 节讲的两种半精度）。不填就是 fp32
全精度训练------\texttt{accelerator.autocast()}
在没有指定精度时是个空壳，不会自动帮你开半精度。
...
改的时候要走对参数。\texttt{lerobot-train} 的
\texttt{use\_policy\_training\_preset} 默认是
\texttt{true}，启动校验那一步会用\textbf{策略自带的预置优化器整个覆盖}
\texttt{-\/-optimizer.*}------也就是说 \texttt{-\/-optimizer.lr}
写了也不生效，学习率仍是预置值。真正管用的入口是策略自己的
\texttt{-\/-policy.optimizer\_lr}（上面那条真机微调脚本用的就是它）。ACT
的预置学习率是
\texttt{1e-5}（\texttt{lerobot/policies/act/configuration\_act.py} 里的
\texttt{optimizer\_lr}），双卡有效 batch 翻倍，先试线性缩放的
\texttt{2e-5}：
...
\ExtensionTok{accelerate}\NormalTok{ launch }\AttributeTok{{-}{-}num\_processes}\OperatorTok{=}\NormalTok{2 }\VariableTok{$(}\FunctionTok{which}\NormalTok{ lerobot{-}train}\VariableTok{)} \DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}policy.type}\OperatorTok{=}\NormalTok{act }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}policy.optimizer\_lr}\OperatorTok{=}\NormalTok{2e{-}5 }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}policy.push\_to\_hub}\OperatorTok{=}\NormalTok{false }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}dataset.repo\_id}\OperatorTok{=}\NormalTok{lerobot/pusht}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  samples/s 实测一遍}再定预期。这对应 4.2 节讲的 DDP
  近线性提速及其前提。
\item
...
\item
  ...
  节的省显存四件套}（混合精度 + 梯度检查点 + 梯度累积 + CPU/NVMe
  offload），在一张卡内部把显存抠出来。很多时候四件套就够，省去多卡的复杂度。
\item
  \textbf{四件套还不够？分片。} 上 \textbf{4.4 节的
  ZeRO/FSDP}（Lightning \texttt{strategy="fsdp"} 或
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/fig12-4-vram-under-each-strategy.png}

}

\caption{一根 7B 模型的单卡显存柱，在五种方案下怎么变。柱子按 4.1.2
节的四类吃货分段：参数 fp16（蓝）、梯度 fp16（绿）、优化器状态
fp32（橙）、激活值（斜纹灰）；红色虚线是标着 40 GB
的单卡容量线。(1)单卡基线 124
GB，高高在线上；(2)开省显存四件套后橙色的优化器状态整段消失（搬到卡外）、斜纹的激活段被梯度检查点压扁，落到
31 GB；(3)DDP 那根柱子仍标着 124 GB，一条双向箭头从 (1) 拉到
(3)、上面写着``加卡只提速，单卡显存一点没降''，右侧几根灰柱注着``另外 7
张卡各存一模一样的一份''；(4)ZeRO-2 把梯度段和优化器段各切成 1/8，降到
38 GB；(5)ZeRO-3 把三段全部切成 1/8，降到 26
GB。图下一行注明：斜纹那段是激活值，随批量与序列长度变，图上只画一个示意高度，其余三段按
16 字节/参数精确折算（分片按 8 卡算，与 4.4.6 节表里的公式一致）}

\end{figure}%

图上有两处正好解释了这条决策线为什么是这个顺序。\textbf{第一处是 (3)
那根柱子}：它和 (1) 一模一样高，因为 DDP
只是把数据分给多张卡，每张卡仍然要完整地放下参数、梯度和优化器状态------所以第
1
步只解决``慢''，不解决``放不下''，单卡放不下时加多少张卡都没用。\textbf{第二处是
(2) 和 (4) 的高低}：省显存四件套（31 GB）比 ZeRO-2（38
GB）还低，而且不需要多卡、不增加通信。这就是第 2 步要排在第 3
步前面的原因------最贵的那一段（优化器状态 84 GB）用 offload
直接搬走，比切成 8
份还划算。真正非分片不可的，是四件套之后仍然顶破容量线的那些情况。
...
\item
  ...
  三选一，把 4.2 节（DDP）与 4.4 节（ZeRO/FSDP）的原理对上代码；底层仍是
  torchrun 式多进程。
\item
  ...
  \texttt{-\/-mixed\_precision} 控；\textbf{学习率要手动调}，且要走
  \texttt{-\/-policy.optimizer\_lr} 而不是会被预置覆盖的
  \texttt{-\/-optimizer.lr}（有效 batch =
  batch×卡数；线性缩放只是最常被引用的起点，不是定律，最终要扫）。
\item
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1431\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.8569\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
维度 & 要点 \\
\midrule\noalign{}
...
混合精度 & 训练侧一律由 accelerate 控：\texttt{-\/-mixed\_\allowbreak precision=fp16/bf16}；\texttt{-\/-policy.use\_\allowbreak amp} 只作用于推理侧 \\
学习率缩放 & 不自动缩；有效 batch=batch×卡数。改学习率走 \texttt{-\/-policy.optimizer\_\allowbreak lr}（\texttt{-\/-optimizer.*} 会被策略预置覆盖）；线性缩放只是起点，要用验证集扫 \\
...
Hugging Face 的 \texttt{accelerate\ launch}（双卡
\texttt{-\/-multi\_gpu\ -\/-num\_processes=2\ \$(which\ lerobot-train)}，混合精度交给
accelerate、学习率得自己调且不能照搬线性缩放）；双卡数据并行买到的是近线性吞吐、买不到单卡显存，选型就按``放得下用
...
Sim2Real（从仿真到真实的策略迁移）讲透，再反过来看
Real2Sim（把真实世界重建成可仿真的世界）。前面三节微调出来的策略，最后都要过这一关。

\subsection{5.1
Sim2Real：从仿真到真实的策略迁移}\label{sim2realux4eceux4effux771fux5230ux771fux5b9eux7684ux7b56ux7565ux8fc1ux79fb}

\subsubsection{5.1.1 为什么需要
Sim2Real}\label{ux4e3aux4ec0ux4e48ux9700ux8981-sim2real}
...
\item
  ...
  让 Shadow Hand 学会解魔方（5.1.4
  节展开），靠的是在大规模仿真集群上堆出来的经验总量------那个量级不可能在真机上跑出来。
\item
...
但任何在仿真里训练好的策略，一旦放到真机就会出现性能下降，有时是小问题，有时是策略完全失效。这就是\textbf{现实差距}。

Aljalbout 等人的综述《The Reality Gap in Robotics: Challenges,
Solutions, and Best
Practices》{[}26{]}系统总结了这个问题：现实差距是视觉、动力学、传感器、执行器和软件栈多层误差共同叠加的结果。

Sim2Real
就是研究``如何让仿真训练好的策略在真实世界依然能工作''的子领域。它在过去十年里发展出了一整套方法论，并且在
VLA（Vision-Language-Action）时代继续演化。本节就是要把这套方法论梳理清楚。

\begin{quote}
小结：仿真是必需品；但要把仿真里学到的东西真正落地，必须正面应对现实差距。Sim2Real
这一整套方法就是为这一步存在的。
\end{quote}

\subsubsection{5.1.2
现实差距到底差在哪里}\label{ux73b0ux5b9eux5deeux8dddux5230ux5e95ux5deeux5728ux54eaux91cc}
...
\item
  接触模型大多基于硬约束（hard
  constraint）求解，无法精确刻画粘滑、变形、接触面随时间退化等现象
\item
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1364\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4489\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4147\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
差距类型 & 典型表现 & 主流应对策略 \\
\midrule\noalign{}
...
在深入展开每条路线之前，先给这些方法画一张分布图。Sim2Real
的方法大致可以划成六个家族：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3500\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3202\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3298\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
家族 & 核心思想（一句话） & 代表工作 \\
\midrule\noalign{}
...
Randomization）}\label{ux57dfux968fux673aux5316domain-randomization}
...
最早的两个代表作是 Sadeghi 与 Levine 的 \textbf{CAD2RL}{[}42{]}（arXiv
2016 年底、RSS 2017）和 Tobin 等人的 \textbf{Domain
Randomization}{[}27{]}（2017）。

CAD2RL 让无人机在大量随机化的 3D CAD
房间里学避障，全程不用一张真实图片，模型直接零样本部署到真机就能飞。Tobin
等人{[}27{]}把同样的思路用在桌面物体定位上：在仿真里随机化纹理、光照、相机位姿、干扰物，训练出来的检测器直接在真机相机上工作。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.78\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-005-tobin2017-domain-randomization-figure1-crop_arxiv-1703.06907.png}

}

\caption{左侧``训练''栏是九张仿真渲染图，同一张桌子和同一批几何体在每张里换了一套颜色、纹理、光照和相机角度，桌面从粉到绿到紫无所不有；右侧``测试''栏只有一张真机照片------普通木桌上摆着彩色积木。（图取自参考文献
{[}27{]}）}
...
Tobin 之后一年，\textbf{Peng
等人}{[}28{]}把同样的思路从视觉迁到了物理：随机化质量、摩擦、阻尼、关节限位、控制延迟。他们用一个
LSTM 策略在仿真中学会``推方块到目标位姿''，再直接部署到 Fetch
机械臂上。LSTM
的隐状态承担了一个隐式的``在线系统辨识''作用------模型从最近几步的运动看出当前真实物理，自适应地选择动作。

紧接着，\textbf{OpenAI 的 Dactyl}{[}29{]} 把动力学 DR 推到了极致。他们让
Shadow Dexterous Hand 把一个方块旋转到指定姿态，仿真中随机化了：
...
上。灵巧手自由度这么高、接触又这么密集，仍然靠``把变体撑宽到包住真实''这一条路走通了------这是本节开头那句直觉最极端的一次兑现。
...
OpenAI 在解魔方那项工作{[}30{]}里提出了
\textbf{自动化域随机化（Automatic Domain Randomization, ADR）}：
...
Sim2Real 历史上最复杂的成功演示之一。

\paragraph{（4）LLM
引导的域随机化}\label{llm-ux5f15ux5bfcux7684ux57dfux968fux673aux5316}

到 2024 年，\textbf{DrEureka}{[}31{]}
又走了一步：把``如何选随机化范围''这件事交给一个大语言模型（LLM）来想。GPT-4
阅读环境代码、阅读任务描述，自动生成奖励函数和域随机化分布，再迭代评估改进。在四足行走和灵巧操作两个任务上，DrEureka
自动找到的 Sim2Real 配置与人类专家手动调出的相当，甚至更好。
...
是我们设计的随机化分布，\(\pi_\theta\) 是参数为 \(\theta\) 的策略（这个
\(\pi\) 是强化学习里表示策略的通用记号，和本讲反复出现的模型名 \(\pi_0\)
没有关系），\(\tau\) 是它跑出来的一条轨迹，\(\mathcal{L}\)
是这条轨迹上的损失。模型求的是整个 \(\xi\)
分布上的\textbf{平均最优}。如果真实世界的参数被这个 \(p(\xi)\)
``覆盖''了，那么真机就只是其中一个采样。

\textbf{Chen 等人}{[}48{]}（ICLR
2022）给出了正式分析：把仿真器建模成一族参数可调的 MDP 之后，可以对
Sim2Real 差距------DR
返回的策略与真实世界最优策略之间的价值差------给出上界，并证明在温和条件下，一条真实样本都不用也能迁移成功。同一篇还点出一件容易被忽略的事：DR
里要让策略带上历史（也就是用依赖历史的记忆型策略），否则这套保证撑不住。

\paragraph{（6）DR
五种变体一张表}\label{dr-ux4e94ux79cdux53d8ux4f53ux4e00ux5f20ux8868}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2119\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4331\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3550\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
变体 & 代表 & 关键扩展 \\
\midrule\noalign{}
...
Adaptation）}\label{ux57dfux9002ux914ddomain-adaptation}
...
Bousmalis 等人的 GraspGAN{[}32{]} 是这条路线的开山作。他们把一个
GAN（生成对抗网络）接到抓取数据流水线里：
...
论文{[}32{]}报告，这样训练出来的抓取策略相比纯仿真训练，达到同等效果所需的真实样本量少了约一个数量级到
50 倍（视对照设置而定）。
...
\tightlist
\item
  \textbf{RL-CycleGAN}{[}44{]}（Rao 等人，CVPR 2020）：把 RL
  智能体的特征也作为约束训练 GAN，强迫翻译保留任务相关信息。
\item
  \textbf{RetinaGAN}{[}33{]}：进一步引入``物体级一致性''约束，让翻译前后物体的边界框检测结果一致。论文那张真机实例抓取对照表里，RetinaGAN
  是 80.0\%，同表的 CycleGAN 是 67.8\%、RL-CycleGAN 是
  68.9\%，而纯仿真训练只有 18.9\%。
\end{itemize}
...
{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-003-rcan-method-summary.png}

}

\caption{上下两行结构完全相同，差别只在最左边那一格。上行``训练''从高度随机化的仿真图出发，下行``测试''从真机照片出发；两者都先过同一个生成器
\(G\)，被翻译成同一套规范仿真外观（灰底、纯色积木、彩色机械臂），再送进同一个智能体输出动作。（图取自参考文献
{[}34{]}）}

\end{figure}%

RCAN 因此既享受了 DR 的鲁棒性（训练翻译器要看大量随机化图），又享受了 DA
的精确性（最后策略输入是同一套规范化的图），并且不需要任何真机数据。

\paragraph{（4）三种 DA
路线对比}\label{ux4e09ux79cd-da-ux8defux7ebfux5bf9ux6bd4}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3373\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2648\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3979\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
方法 & 翻译方向 & 关键贡献 \\
\midrule\noalign{}
...
Identification）}\label{ux7cfbux7edfux8fa8ux8bc6system-identification}

域随机化和域适配都从模型这边发力。系统辨识则反过来：\textbf{改进仿真，让仿真离真实更近}。

\paragraph{（1）闭环辨识：SimOpt}\label{ux95edux73afux8fa8ux8bc6simopt}

\textbf{SimOpt}{[}45{]}（Chebotar 等人，ICRA 2019）提出一个闭环：
...
\textbf{BayesSim}{[}46{]}（Ramos 等人，RSS 2019）把 SimOpt
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1852\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3785\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4363\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
组件 & 解决什么问题 & 风险 \\
\midrule\noalign{}
...
Networks）}\label{ux6e10ux8fdbux7f51ux7edcprogressive-networks}

\textbf{Rusu 等人}（DeepMind）先在 2016 年 6 月提出 Progressive
Networks{[}50{]} 这个架构本身：
...
\item
  第一列：在仿真中训练一个完整网络
\item
  第二列：冻结第一列，新加一列在真机上训练；新列每一层都和第一列对应层有``侧向连接（lateral
  connection）''
\item
  后续列继续叠加
\end{itemize}
...
把它用到 Sim2Real 上的是同一批人同年 10
月挂出的那篇{[}43{]}（后来正式发表在 CoRL
2017）：仿真里训练好像素级到达任务，加一列在真机上微调，只需要少量真机数据就能迁移。

\paragraph{（2）师生框架（Teacher-Student）}\label{ux5e08ux751fux6846ux67b6teacher-student}

师生是腿足机器人 Sim2Real 真正的主流方案，\textbf{RMA（Rapid Motor
Adaptation）}{[}35{]} 是最有代表性的工作之一。
...
更早的开山工作是 \textbf{Hwangbo 等人}{[}47{]}（Science Robotics
2019）：在 ANYmal
上用师生框架学敏捷动作。教师在仿真里享受准确动力学，学生再蒸馏到只看实际可观测的输入。它和
RMA 的分工正好互补：RMA
把重点放在部署后的在线估计，这一篇把重点放在``仿真里的准确动力学怎么蒸馏进一个只看得见本体的学生''。

\paragraph{（3）师生 vs 渐进 vs
DR}\label{ux5e08ux751f-vs-ux6e10ux8fdb-vs-dr}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1516\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3376\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5108\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
方法 & 关键机制 & 适合什么 \\
\midrule\noalign{}
...
\begin{quote}
小结：师生是腿足机器人 Sim2Real 的事实标准，因为腿足机器人天生有``特权
vs 实际可观测''的差距；DR
...
\item
  有的工作在做 \textbf{Real2Sim 参数对齐}。
\end{enumerate}

这几类做法针对的差距不一样。把它们分开看，反而更容易判断一篇论文到底贡献在哪里。

\paragraph{（1）扩散策略（Diffusion
Policy）}\label{ux6269ux6563ux7b56ux7565diffusion-policy}

Chi 等人提出的 Diffusion Policy{[}36{]} 把扩散模型（diffusion
model）用作机器人动作生成的策略类。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-003-diffusion-policy-overview_arxiv-2303.04137.png}

}

\caption{论文的方法总览，分三格。左格是输入输出示例------上排是机械臂的图像观测序列，下排三张图叠着彩色散点，画的是同一个观测下采出的多条候选动作序列。中格是通用形式：观测
\(\mathbf{O}_t\) 送进去噪网络
\(\varepsilon_\theta(\mathbf{O},\mathbf{A},k)\)，一次吐出预测时域
\(T_p\) 内的一整段动作
\(\mathbf{A}_t\)，只执行开头几步再重新预测。右两格是两种网络实现：CNN
版用 FiLM 把观测条件注入一维卷积，Transformer
版用交叉注意力。（图取自参考文献 {[}36{]}）}

\end{figure}%

左格那三张彩色散点图是这张图里最该看的：同一个观测下，动作散成几团------\textbf{``多模态''在这里是画得出来的}，而这正是下面三条特点里第一条要说的事。
...
\tightlist
\item
  \textbf{多模态动作分布}：同一观测可以有多种合理动作；扩散模型不会把它们``平均''成一个差的动作
\item
  \textbf{训练稳定}：本质上是回归损失，不像 GAN/VAE 那么挑超参
\item
  \textbf{高维连续动作友好}：扩散过程天然适配长序列的连续动作块
\end{itemize}
...
VLA
的核心想法是：把视觉-语言模型（VLM）接上机器人动作输出，让一个模型同时具备语义理解和动作生成能力。下面这些工作按时间排列，但不是一条严格的继承链；更准确地说，它们代表了几种不同的
VLA 设计取向。

在这些系统里，仿真通常用于数据增广、评估、具身适配或补足真实数据，很少是唯一训练来源。VLA
...
\tightlist
\item
  \textbf{RT-2}{[}6{]}：把动作离散化成文本 token，让 VLM
  直接生成动作；论文基于 PaLI-X / PaLM-E 等 VLM
  尺度做实验，展现出语义泛化（例如能理解``把灭绝的动物放到桌上''这种新颖指令）。
\item
  \textbf{OpenVLA}{[}7{]}：开源 7B VLA，用 DINOv2 + SigLIP 视觉编码器和
  LLaMA-2 骨干，在 Open X-Embodiment 970K 条数据上训练；用 LoRA
  高效适配新任务。
\end{itemize}

\begin{figure}[htbp]

{\centering \includegraphics[width=0.72\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-005-openvla-architecture-flattened_arxiv-2406.09246.png}

}

\caption{OpenVLA 的数据通路。左侧输入是一张相机图和一句语言指令 ``Put
eggplant in bowl''；图像经 DinoV2 与 SigLIP 两个编码器并联，再过 MLP
投影成视觉 token，指令经 Llama Tokenizer 成文本 token，两串 token
拼在一起送进 Llama 2 7B；输出端的动作 token 经 Action De-Tokenizer
还原成 \([\Delta x, \Delta\theta, \Delta\text{Grip}]\) 这样一个 7
维机器人动作。（图取自参考文献 {[}7{]}）}

\end{figure}%

图右端那个七维向量是理解这条路线的关键：动作在模型内部一直是\textbf{离散
token}，直到最后一步才被反 token 化成连续量------所以 OpenVLA
能直接复用语言模型的全部训练机制，代价是动作精度受限于 token 的分辨率。

\begin{itemize}
\tightlist
\item
  \textbf{\(\pi_0\)}{[}8{]}：在 VLM 之上挂 flow matching
  动作专家（action expert），输出连续动作块；预训练数据来自 7
  种机器人配置、68 个任务和超过 10,000 小时机器人数据，并补充 Open
  X-Embodiment 数据。相较于离散 action-token 路线，\(\pi_0\)
  更强调连续动作建模和长程真实任务。
\end{itemize}

\begin{figure}[htbp]

{\centering \includegraphics[width=0.72\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-005-pi0-framework-flattened_arxiv-2410.24164.png}

}

\caption{\(\pi_0\) 的框架。左端是训练数据来源------\(\pi\)
自采数据集、互联网预训练语料与 OXE（Open X-Embodiment）；中间是预训练
VLM，标注为 SigLIP（400M）+ Gemma（2.6B），多路相机图各过一个
ViT，与指令 ``fold shirt'' 一起进入；右接一个 300M
的动作专家，输入当前关节状态 \(q_t\) 和噪声，输出动作块
\(a_t, a_{t+1}, \dots, a_{t+H}\)；最右列出三类目标本体------14
自由度双臂、18 自由度移动操作、7 与 8 自由度单臂。（图取自参考文献
{[}8{]}）}

\end{figure}%

和上一张并排看差别就很清楚：OpenVLA 的动作从主干里以 token
形式出来，\(\pi_0\) 则把动作交给一个\textbf{挂在 VLM 之外的 300M
动作专家}，主干只负责理解、不负责产出连续量。右侧那三类本体也说明了参数量为什么要这么分------同一个
VLM 要面对自由度差一倍以上的机器人，可换的是动作专家那一头。

\begin{itemize}
\tightlist
\item
  \textbf{GR00T N1}{[}9{]}：人形机器人基础模型；双系统设计------System 1
...
\textbf{DrEureka}{[}31{]}
不直接提出一个新的机器人策略网络。它解决的是另一个更工程化的问题：奖励函数和域随机化的参数范围通常要靠人手调。
...
所以 DrEureka 属于``自动化 Sim2Real 配置''这一类。它和 VLA
的关系很弱：LLM 在这里是奖励函数与域随机化范围的设计助手。

\paragraph{（4）3DGS
场景桥接：SplatSim}\label{dgs-ux573aux666fux6865ux63a5splatsim}

\textbf{SplatSim}{[}37{]} 关心的是视觉差距。传统视觉 DR
是随机换纹理、光照、背景；SplatSim 换了一种思路：先用 3D Gaussian
Splatting（3DGS）把真实场景外观重建出来，再在这个更接近真实的仿真视觉里训练策略。
...
\item
  部署到真机时，让人类操作者在线介入纠正；
\item
  不直接覆盖基础策略，而是学习一个残差策略；
\item
  再训练一个门控模块，判断什么时候该让残差策略接管。
\end{enumerate}
...
\paragraph{（6）Real2Sim 对齐和人形灵巧操作：ASAP /
Humanoid-Dex}\label{real2sim-ux5bf9ux9f50ux548cux4ebaux5f62ux7075ux5de7ux64cdux4f5casap-humanoid-dex}
...
\item
  \textbf{ASAP}（He
  等人，2025）：先在仿真中预训练人形全身动作跟踪策略；再在真实机器人上采集少量数据，学习一个增量动作模型（delta
  action
  model），用它补偿仿真和真实动力学差异；最后把这个增量模型放回仿真中继续微调策略。
\item
  \textbf{Humanoid-Dex（Lin 等人，2025）}：做人形灵巧操作时，使用自动
  Real2Sim
  标定、通用的接触与物体奖励、分而治之的策略蒸馏，以及稀疏/稠密混合的物体表示。
\end{itemize}
...
接触是机器人物理仿真最薄弱的一环。摩擦、滑动、粘附、磨损、碰撞瞬态------绝大多数仿真器都用大幅简化的接触求解器。一旦任务依赖精细接触（夹精密件、摩擦触觉、装配公差），Sim2Real
就会显著恶化。
...
视觉那一侧已经攒下一整套可用的工具（域随机化、域适配、3DGS
渲染），\textbf{触觉 Sim2Real 却还在早期阶段}。GelSight、DIGIT
等基于光学的触觉传感器开始可仿真，力/力矩传感器的零飘和动力学耦合在仿真里还很难刻画。

\paragraph{（4）跨本体迁移}\label{ux8de8ux672cux4f53ux8fc1ux79fb}
...
不同论文用不同的真机平台、不同的成功率定义，结果很难严格比较。SIMPLER、LIBERO
等基准在缩小这个鸿沟，但远未统一。

\paragraph{（7）在线适应与持续学习}\label{ux5728ux7ebfux9002ux5e94ux4e0eux6301ux7eedux5b66ux4e60}

部署后的策略能不能在真实世界继续学？RMA
提供了``在线快速适应''的雏形（靠适应模块在线估计环境隐变量，见 5.1.7
节）；真机在线纠正这一路的代表是 TRANSIC{[}38{]}、DAgger 及 HIL-SERL
一类人在环方法（第16讲第 5
节会展开）。但持续学习不忘旧、避免奖励黑客仍是难题。

顺带把一个容易混的归类说清：\(\pi_{0.5}\)（第11讲第 5
节）\textbf{不属于}这一格。它的核心是\textbf{开放世界泛化、异构数据与知识迁移}（预训练
+
...
\tightlist
\item
  \textbf{现实差距是问题}。视觉、动力学、传感器/执行器、控制软件栈四类来源都在制造它。
\item
  \textbf{Sim2Real
  是一族方法}，不是一个算法。几大家族------DR、DA、SysID、渐进/师生、基础模型与生成式策略、混合训练------按``想消除哪个差距''来选用。
\item
  \textbf{DR + 师生}是腿足机器人 Sim2Real 的事实主流；\textbf{DA + DR}
  是视觉抓取的常见组合；\textbf{SimOpt + DR} 是接触敏感任务的稳妥做法。
\item
  ...
  真机微调''是当前常见的工程组合之一。
\item
  \textbf{开放问题集中在}：接触动力学、可形变物体、触觉、跨本体迁移、基础策略的真机精度，以及评估标准化。
\end{itemize}
...
\subsection{5.2
Real2Sim：从真实观测到可仿真的三维世界}\label{real2simux4eceux771fux5b9eux89c2ux6d4bux5230ux53efux4effux771fux7684ux4e09ux7ef4ux4e16ux754c}

\subsubsection{5.2.1 Real2Sim
的问题定义}\label{real2sim-ux7684ux95eeux9898ux5b9aux4e49}

Sim2Real 关心的是：策略已经在仿真里学好，怎样迁移到真实机器人。Real2Sim
关心的是更靠前的一步：真实世界本身怎样变成一个机器人仿真器能使用的世界。这里的目标是做一个可以被加载、碰撞、推动、控制、采样、评估的仿真环境。

一个厨房柜子如果只在渲染图里像柜子，还不够。机器人需要知道门板是哪一根连杆（link，模型里可以独立运动的刚性部件），铰链轴在哪里，关节限位是多少，碰撞几何是否稳定，摩擦和质量是否合理，场景能否写成
URDF / MJCF /
USD（机器人和仿真场景常用的模型描述格式）或仿真器原生格式。衣服、绳子、软包裹这类可形变物体还需要材料参数和变形模型。Real2Sim
的核心难点就在这里：\textbf{从真实观测恢复一个仿真就绪（simulation-ready，能被仿真器直接加载并稳定求解）的三维世界}。

Ye 等人的综述《3D Generation for Embodied AI and Robotic Simulation: A
Survey》{[}39{]}把这个问题概括得很清楚：面向具身智能的三维内容正在从``视觉真实''转向``交互可用''。该综述把相关工作分成三个角色------资产生成器（Data
Generator）管物体级资产从哪里来，仿真环境（Simulation
Environments）管这些资产怎么组成可交互的世界，Sim2Real 桥接（Sim2Real
Bridge）管真实观测怎么进仿真、仿真数据又怎么回到真实机器人。下面三个小节就按这三个角色展开。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-018-ye2026-fig1-outline_arxiv-2604.26509.png}

}

\caption{Ye 等人综述的全文外框，自上而下五层：预备知识（三维表示 /
生成模型 / 仿真平台与资产 /
具身智能基础）、三个角色（资产生成器、仿真环境、Sim2Real
桥接）、数据集与评估、挑战与未来方向。三个角色一层被虚线框单独圈出。（图出自参考文献
{[}39{]} 图 1）}

\end{figure}%

被虚线圈起来的那一层才是本节要讲的部分，它上下各压着一层：上面是三维表示与生成模型这些前置技术，下面是数据集与评估。这个夹层位置本身说明了
Real2Sim
的处境：它算不上一项独立技术，更像是把三维生成的能力接到机器人评估口径上的中间环节。三个角色之间也有依赖顺序：没有可仿真的资产，就没有可靠的数字孪生；没有可交互的场景，就不能训练或评估策略。
...
Splatting，用大量三维高斯点快速渲染场景的显式表示）可以让画面逼真，但不一定能碰撞；三角网格（mesh，物理引擎常用的表面几何）可以进物理引擎，但材质和纹理可能不真实；有
URDF
关节也不代表物理参数准确；有一个重建场景也不代表训练出来的策略能迁回真机。

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1437\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3861\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4702\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
层级 & 主要问题 & 典型输出 \\
\midrule\noalign{}
...
NeRF、3DGS、三角网格只是这条链里的表示选择，单独拿出来都撑不起完整的
Real2Sim。

\subsubsection{5.2.2
仿真就绪：比视觉真实更高的门槛}\label{ux4effux771fux5c31ux7eeaux6bd4ux89c6ux89c9ux771fux5b9eux66f4ux9ad8ux7684ux95e8ux69db}

传统三维生成常用 Chamfer
Distance（倒角距离，衡量点云或表面之间的几何差异）、FID（生成图像分布和真实图像分布的距离）、CLIP
Score（图像与文本的语义匹配分数）评估几何和外观。但机器人仿真还要回答另一个问题：这个资产能不能真的在仿真器里跑。该综述{[}39{]}把仿真就绪拆成四项要求：
...
\item
  \textbf{运动学可执行}：如果对象有关节，要有关节类型、关节轴、关节限位和运动约束。
\item
  \textbf{仿真器兼容}：输出能写成 URDF、MJCF、USD
  或目标仿真平台的原生格式。
\end{enumerate}

这四项把 Real2Sim 和普通三维重建区分开。一个漂亮的 3DGS
场景，如果没有稳定的碰撞网格（collision
mesh，专供接触计算用的简化几何），就不能直接训练接触策略；一个由多视角图片重建的三角网格，如果不是封闭无洞的（watertight，表面完全闭合，体积与碰撞计算才有定义），进入物理引擎后可能穿模、爆炸或产生错误接触；一个看起来正确的抽屉，如果铰链轴错了，机器人学到的开抽屉策略就没有意义。

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2323\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3981\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3696\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
3DGS 和 NeRF
属于``可渲染表示''这一层。它们能显著减少视觉差距，但不自动解决动力学差距。完整的
Real2Sim 系统通常采用混合表示：3DGS 或 NeRF 负责真实外观，三角网格 /
有符号距离场负责碰撞，URDF / MJCF / USD 负责运动学和物理属性。

\subsubsection{5.2.3
资产生成器：生成可仿真的物体资产}\label{ux8d44ux4ea7ux751fux6210ux5668ux751fux6210ux53efux4effux771fux7684ux7269ux4f53ux8d44ux4ea7}
...
可形变物体是 Real2Sim
最难的部分。布料、绳子、软玩具、包裹、食物没有低维刚体状态，接触和变形高度非线性。综述中把可形变资产单独列为一类，因为它们需要和有限元法（FEM）、物质点法（MPM）、基于位置的动力学（PBD）、弹簧-质点模型、布料求解器等模型接起来。

服装生成相对成熟一些，因为裁剪样片（sewing
pattern，衣服在裁剪台上摊平后的二维片形）可以充当仿真原生的中间表示：它是布料仿真器能直接理解的结构，超出了单纯的视觉网格。一般软体物体还没有类似统一的中间表示。PhysGaussian、PhysDreamer、GarmentDreamer、DressCode
...
这类工作代表了端到端方向：输入文本、图片或粗糙的三维内容，输出仿真器可加载的资产。

这类系统的关键不只是生成能力，还有质量控制。大规模生成一定会产生坏资产：非流形网格、自相交、错误尺度、漂浮部件、碰撞体缺失、关节轴错位。好的流水线需要自动验证和过滤，例如拓扑检查、语义一致性检查、物理稳定性检查、仿真器加载测试。

\textbf{小结}：资产生成器解决的是``物体资产的规模化来源''，为 Real2Sim
提供可补全、可替换、可扩展的资产基础。

\subsubsection{5.2.4
仿真环境：生成可交互场景}\label{ux4effux771fux73afux5883ux751fux6210ux53efux4ea4ux4e92ux573aux666f}
...
早期具身场景多来自程序化生成规则（procedural
rules）或已有的真实扫描数据。ProcTHOR、RoboCasa、OmniGibson、BEHAVIOR、Habitat
这类平台提供大量房间、厨房、家居任务。它们的价值在于场景可以被智能体走动、抓取、打开、移动和重置。

结构驱动方法通常依赖场景图（scene
graph，用节点和边表示物体及其关系）或布局先验。场景图里的节点是物体，边表示支撑、相邻、包含、可交互关系。例如``杯子在桌子上''\,``抽屉属于柜子''``盘子靠近水槽''。这种结构比单纯的三角网格更接近机器人任务。
...
\paragraph{（3）智能体式场景生成}\label{ux667aux80fdux4f53ux5f0fux573aux666fux751fux6210}

近年的场景生成开始引入由 LLM / VLM
驱动的智能体（自动化决策模块）：模型先理解任务，再调用分割、检索、生成、放置、物理检查等模块，迭代构造场景。Holodeck、SceneWeaver、SAGE、SceneSmith
代表了这种趋势。
...
\textbf{小结}：仿真环境这一层解决的是``场景如何可交互''。缺了它，Real2Sim
只能得到一堆孤立资产，撑不起长任务、导航、操作和策略评估。

\subsubsection{5.2.5 Sim2Real
桥接：从数字孪生到策略闭环}\label{sim2real-ux6865ux63a5ux4eceux6570ux5b57ux5b6aux751fux5230ux7b56ux7565ux95edux73af}

Sim2Real
桥接描述真实观测和机器人策略之间的闭环：真实观测先变成数字孪生；数字孪生再用于数据增强、任务生成和示教生成；训练出的策略回到真实机器人；真实执行反馈继续修正仿真域。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-018-ye2026-fig9-bridge_arxiv-2604.26509.png}

}

\caption{中间橙色框是生成式仿真域，内含三列------数字孪生列（自动场景重建
/ 运动学重建 / 物理感知与对齐）、数据增强列（视角与几何增强 /
动态与时序增强 / 物理一致性）、任务与示教生成列；左端真实观测（RGB-D
扫描、视频示教、遥操数据）经一支粗箭头进入，右端合成示教喂给真实机器人部署，底部一条虚线箭头从部署折返回真实观测。（图出自参考文献
{[}39{]} 图 9）}

\end{figure}%

图里最值得看的是那条底部虚线：中间三列之间都是单向往右推进的实线箭头，只有它从真实部署折回起点。它把这套流水线的设计意图交代清楚了------真机执行的失败要被当作下一轮重建的输入，而不是``扫一次、生成一批数据、训完就结束''。少了这条虚线，中间三列就退化成一个只跑一遍的离线数据工厂；5.2.9
节最后一条开放挑战说的正是这件事。

\paragraph{（1）生成式数字孪生}\label{ux751fux6210ux5f0fux6570ux5b57ux5b6aux751f}

数字孪生的目标是从真实观测自动构造一份仿真就绪的真实场景副本。输入可能是
RGB-D、视频、多视角图片、点云、机器人力矩传感器或接触轨迹；输出必须既视觉对齐，又物理可交互。

生成式数字孪生要解决三个层层递进的子问题，正对应上图数字孪生那一列的三格：\textbf{自动场景重建}（从
RGB-D
或多视角图像得到可交互的资产与场景）、\textbf{运动学重建}（恢复某个具体物体实例的关节类型、轴与限位）、\textbf{物理感知与视觉-动力学对齐}（恢复质量、惯量、摩擦、弹性、阻尼，让仿真推演的轨迹对得上真实观测）。三者的难度是递增的：几何最容易，关节次之，物理参数最难------因为前两者能从图像里看出来，物理参数看不出来。本节后面的两个案例正好各占一端：Scalable
Real2Sim{[}40{]} 做刚体的几何加惯性参数，PhysTwin{[}41{]}
做可形变物体的物理模型。

资产生成器多是生成``新的类别级资产''；数字孪生多是重建``某个真实实例''。两者可以共享技术，目标却不同：生成一个可能存在的抽屉，和重建眼前这个抽屉，是两件事。

\paragraph{（2）三维可控的数据增强}\label{ux4e09ux7ef4ux53efux63a7ux7684ux6570ux636eux589eux5f3a}

机器人示教很贵。Real2Sim
的一个直接用途是把少量真实示教转成可编辑的三维表示，然后从新视角、新物体摆放、新光照、新背景、新时间轨迹中合成更多训练数据。

传统二维图像增强容易破坏几何一致性：换背景、改颜色、贴干扰物，都不保证多视角深度和接触关系正确。NeRF
和 3DGS
把增强从像素层搬到三维层：相机可以移动，物体可以替换，遮挡关系和深度更容易保持一致。5.1.8
节讲过的 SplatSim{[}37{]} 就属于这一方向，同路的还有
RoboSplat、ExoGS、GAF、Splat-MOVER。
...
更进一步，系统可以不只增强已有示教，还能生成新的任务、新的场景、新的示教轨迹。MimicGen
从少量人类示教中重组出大量以物体为中心的轨迹；Gen2Sim 和 GenSim2 用 LLM
/ VLM 生成仿真任务、奖励和示教；R2R2R 从视频恢复物体运动，再把 3DGS
转成三角网格进入 IsaacLab 之类的并行仿真器。

这类工作把 Real2Sim 和基础模型规模的机器人学习连了起来。VLA
模型需要大量跨任务、跨对象、跨场景数据，真实采集成本过高。可仿真的三维世界可以成为数据发动机：真实观测提供锚点，生成和仿真提供规模。

\textbf{小结}：Sim2Real 桥接是 Real2Sim
的下游闭环。数字孪生做出来是要拿去用的------把真实数据转成可训练、可评估、可迁移的机器人学习资源，光留一个漂亮场景没有意义。

\subsubsection{5.2.6 案例一：Scalable
Real2Sim}\label{ux6848ux4f8bux4e00scalable-real2sim}

\textbf{Scalable Real2Sim}{[}40{]} 展示了刚体资产的主动式 Real2Sim
流水线。它瞄准的是产量------在一个标准的抓取-放置工作台上，把可仿真的物体资产一件件自动造出来，展示效果不在它的目标里。
...
\item
  \textbf{视觉几何}：带纹理的三角网格；
\item
  \textbf{碰撞几何}：适合物理引擎使用的凸分解碰撞体；
\item
  \textbf{物理属性}：质量、质心、转动惯量等惯性参数。
\end{itemize}

这篇论文的重要性在于它把 Real2Sim
变成了一个``物理扫描''过程。普通三维扫描只能得到形状；Scalable Real2Sim
通过机器人交互同时估计碰撞几何和惯性参数。论文还提出从 NeRF / 3DGS
这类只按图像亮度重建出来的表示里，抽取物体级三角网格的方法，处理了运动物体和夹爪遮挡的问题。

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2779\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.7221\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
流水线阶段 & 作用 \\
\midrule\noalign{}
...
这篇案例清楚地说明：决定 Real2Sim
成不成的，是有没有产出一份完整的、仿真器直接能用的资产；用没用 NeRF 或
3DGS
反倒是次要的。视觉重建只是中间步骤，碰撞几何和物理参数才决定它能否进入真实的操作仿真。

\subsubsection{5.2.7 案例二：PhysTwin}\label{ux6848ux4f8bux4e8cphystwin}

\textbf{PhysTwin}{[}41{]} 面向可形变物体的物理数字孪生。它从稀疏 RGB-D
视频构建可交互的物理数字孪生，目标对象包括绳子、布、软玩具和包裹。

\begin{figure}[htbp]

{\centering \includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/ref/figures/wri-003-phystwin-framework.png}

}

\caption{左下角方框是 PhysTwin
的核心表示------一只软玩具，放大处画出连接质点的弹簧拓扑（红点为控制点），旁边挂着刚度、接触和三维高斯三组参数；左上角两台相机拍下真实视频，经重建与跟踪得到上排的伪轨迹，下排是仿真渲染出的几何与运动；两排之间的代价函数由几何项、运动项、渲染项三项相加，红色虚线箭头是回传的优化方向。（图出自参考文献
{[}41{]}）}

\end{figure}%

图中间那个三项相加的代价函数，是 PhysTwin 与单纯的动态 3DGS
分野的地方。动态 3DGS 只需要渲染项------把画面拟合上就行；PhysTwin
另外两项要求仿真推出来的几何与运动也对得上观测，等于强迫弹簧刚度、接触参数这些物理量必须解释真实视频里的形变，而不只是渲染得像。左上角那两台相机也值得留一眼：视角只有两个，靠的是生成式形状先验把看不到的背面补出来。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1656\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6169\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2175\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
评估层级 & 常见指标 & 局限 \\
\midrule\noalign{}
...
这类评估基础设施为什么重要：它们重建场景是为了拿它当比较策略的公共考场，让
VLA、扩散策略、ACT 在更可复现的环境里横向比一次。Real2Sim
评估替代不了真机实验，但可以显著降低筛选策略和定位失败模式的成本。
...
需要把封闭无洞比例、无碰撞比例、稳定性测试、跨仿真器一致性作为基本检查。
...
一次前馈或扩散式的场景生成速度快，但任务约束和物理一致性弱；LLM / VLM
智能体式生成可控性强，但慢且容易在模块之间累积误差。一个务实方向是先用快速模型生成粗布局，再只对任务关键区域做智能体式精修和物理验证。
...
\paragraph{（6）Sim2Real
差距不会自动消失}\label{sim2real-ux5deeux8dddux4e0dux4f1aux81eaux52a8ux6d88ux5931}

Real2Sim
能把差距压小，但消不掉它。传感器噪声、接触模型、电机延迟、材质变化、隐藏状态、场景长尾都会造成残余误差。更可靠的方向是闭环：真实执行反馈持续更新数字孪生，而不是一次扫描后永远相信仿真。

\subsubsection{5.2.10 本节小结}\label{ux672cux8282ux5c0fux7ed3-4}

Real2Sim
要交付的是``把真实世界转换成机器人仿真器可执行的三维内容''------资产要同时满足几何有效、物理参数化、运动学可执行和格式兼容四项。Ye
等人{[}39{]}的三段式结构可以概括为：

\begin{itemize}
\tightlist
\item
  \textbf{资产生成器}提供可仿真的物体资产；
\item
  \textbf{仿真环境}把资产组织成可交互任务世界；
\item
  \textbf{Sim2Real
  桥接}把真实观测转成数字孪生，并用于数据增强、任务生成、策略训练和评估。
\end{itemize}
...
\item
  ...
  / 流水线并行各归其位，最后落到双卡 + LeRobot 的真实启动命令上。
\item
  \textbf{Sim2Real 与 Real2Sim}：现实差距来自视觉 / 动力学 /
  传感器执行器 /
  软件栈四类；域随机化、域适配、系统辨识、师生框架各有主攻的那一类差距，但真跑通的方案基本都是几种叠着用；Real2Sim
  则反过来把真实重建成可仿真的世界，两者互为闭环。
\end{itemize}
...
\begin{quote}
著录遵《GB/T 7714---2015》；arXiv 预印本按 {[}EB/OL{]}、正式会议论文按
{[}C{]}、期刊按 {[}J{]}。引用日期以各条方括号内标注为准。
\end{quote}
...
{[}2{]} DETTMERS T, PAGNONI A, HOLTZMAN A, et al.~QLoRA: efficient
finetuning of quantized LLMs{[}EB/OL{]}. (2023){[}2026-06-26{]}.
https://arxiv.org/abs/2305.14314.

{[}3{]} HOULSBY N, GIURGIU A, JASTRZEBSKI S, et al.~Parameter-efficient
...
{[}9{]} NVIDIA, BJORCK J, CASTAÑEDA F, et al.~GR00T N1: an open
foundation model for generalist humanoid robots{[}EB/OL{]}.
(2025){[}2026-06-26{]}. https://arxiv.org/abs/2503.14734.

{[}10{]} AGHAJANYAN A, ZETTLEMOYER L, GUPTA S. Intrinsic dimensionality
explains the effectiveness of language model fine-tuning{[}EB/OL{]}.
(2020){[}2026-06-26{]}. https://arxiv.org/abs/2012.13255.

{[}11{]} RASCHKA S. Practical tips for finetuning LLMs using LoRA
(low-rank adaptation){[}EB/OL{]}. (2023){[}2026-06-26{]}.
https://magazine.sebastianraschka.com/p/practical-tips-for-finetuning-llms.

{[}12{]} LIU S Y, WANG C Y, YIN H, et al.~DoRA: weight-decomposed
low-rank adaptation{[}EB/OL{]}. (2024){[}2026-06-26{]}.
https://arxiv.org/abs/2402.09353.

{[}13{]} ZHANG Q, CHEN M, BUKHARIN A, et al.~AdaLoRA: adaptive budget
allocation for parameter-efficient fine-tuning{[}EB/OL{]}.
(2023){[}2026-06-26{]}. https://arxiv.org/abs/2303.10512.

{[}14{]} HAYOU S, GHOSH N, YU B. LoRA+: efficient low rank adaptation of
large models{[}EB/OL{]}. (2024){[}2026-06-26{]}.
https://arxiv.org/abs/2402.12354.

{[}15{]} KALAJDZIEVSKI D. A rank stabilization scaling factor for
fine-tuning with LoRA{[}EB/OL{]}. (2023){[}2026-06-26{]}.
https://arxiv.org/abs/2312.03732.

{[}16{]} MENG F, WANG Z, ZHANG M. PiSSA: principal singular values and
singular vectors adaptation of large language models{[}EB/OL{]}.
(2024){[}2026-06-26{]}. https://arxiv.org/abs/2404.02948.

{[}17{]} RAJBHANDARI S, RASLEY J, RUWASE O, et al.~ZeRO: memory
optimizations toward training trillion parameter models{[}EB/OL{]}.
(2019){[}2026-06-26{]}. https://arxiv.org/abs/1910.02054.

{[}18{]} HPC-AI Tech. Colossal-AI documentation: paradigms of
parallelism{[}EB/OL{]}. (2024){[}2026-06-26{]}.
https://colossalai.org/docs/.

{[}19{]} LI S, ZHAO Y, VARMA R, et al.~PyTorch distributed: experiences
on accelerating data parallel training{[}EB/OL{]}.
(2020){[}2026-06-26{]}. https://arxiv.org/abs/2006.15704.

{[}20{]} Uber Engineering. Meet Horovod: Uber's open source distributed
deep learning framework{[}EB/OL{]}. (2017){[}2026-06-26{]}.
https://eng.uber.com/horovod/.

{[}21{]} Google Cloud. BFloat16: the secret to high performance on Cloud
TPUs{[}EB/OL{]}. (2019){[}2026-06-26{]}.
https://cloud.google.com/blog/products/ai-machine-learning/bfloat16-the-secret-to-high-performance-on-cloud-tpus.

{[}22{]} CHEN T, XU B, ZHANG C, et al.~Training deep nets with sublinear
memory cost{[}EB/OL{]}. (2016){[}2026-08-29{]}.
https://arxiv.org/abs/1604.06174.

{[}23{]} PyTorch Team. Getting started with fully sharded data parallel
(FSDP){[}EB/OL{]}. (2023){[}2026-06-26{]}.
https://pytorch.org/tutorials/intermediate/FSDP\_tutorial.html.

{[}24{]} SHOEYBI M, PATWARY M, PURI R, et al.~Megatron-LM: training
multi-billion parameter language models using model
parallelism{[}EB/OL{]}. (2019){[}2026-06-26{]}.
https://arxiv.org/abs/1909.08053.

{[}25{]} HUANG Y, CHENG Y, BAPNA A, et al.~GPipe: efficient training of
giant neural networks using pipeline parallelism{[}EB/OL{]}.
(2018){[}2026-06-26{]}. https://arxiv.org/abs/1811.06965.

{[}26{]} ALJALBOUT E, XING J, ROMERO Á, et al.~The reality gap in
robotics: challenges, solutions, and best practices{[}EB/OL{]}.
(2025){[}2026-06-26{]}. https://arxiv.org/abs/2510.20808.

{[}27{]} TOBIN J, FONG R, RAY A, et al.~Domain randomization for
transferring deep neural networks from simulation to the real
world{[}EB/OL{]}. (2017){[}2026-06-26{]}.
https://arxiv.org/abs/1703.06907.

{[}28{]} PENG X B, ANDRYCHOWICZ M, ZAREMBA W, et al.~Sim-to-real
transfer of robotic control with dynamics randomization{[}C{]}//ICRA
2018. https://doi.org/10.1109/ICRA.2018.8460528.

{[}29{]} OPENAI, ANDRYCHOWICZ M, BAKER B, et al.~Learning dexterous
in-hand manipulation{[}EB/OL{]}. (2018){[}2026-06-26{]}.
https://arxiv.org/abs/1808.00177.

{[}30{]} OPENAI, AKKAYA I, ANDRYCHOWICZ M, et al.~Solving Rubik's cube
with a robot hand{[}EB/OL{]}. (2019){[}2026-06-26{]}.
https://arxiv.org/abs/1910.07113.

{[}31{]} MA Y J, LIANG W, WANG H J, et al.~DrEureka: language model
guided sim-to-real transfer{[}C{]}//Robotics: Science and Systems XX
(RSS 2024), paper 094. https://doi.org/10.15607/RSS.2024.XX.094.

{[}32{]} BOUSMALIS K, IRPAN A, WOHLHART P, et al.~Using simulation and
domain adaptation to improve efficiency of deep robotic
grasping{[}C{]}//ICRA 2018. https://doi.org/10.1109/ICRA.2018.8460875.

{[}33{]} HO D, RAO K, XU Z, et al.~RetinaGAN: an object-aware approach
to sim-to-real transfer{[}C{]}//ICRA 2021.
https://arxiv.org/abs/2011.03148.

{[}34{]} JAMES S, WOHLHART P, KALAKRISHNAN M, et al.~Sim-to-real via
...
{[}35{]} KUMAR A, FU Z, PATHAK D, et al.~RMA: rapid motor adaptation for
legged robots{[}EB/OL{]}. (2021){[}2026-06-26{]}.
https://arxiv.org/abs/2107.04034.

{[}36{]} CHI C, XU Z, FENG S, et al.~Diffusion policy: visuomotor policy
learning via action diffusion{[}EB/OL{]}. (2023){[}2026-06-26{]}.
https://arxiv.org/abs/2303.04137.

{[}37{]} QURESHI M N, GARG S, YANDÚN F, et al.~SplatSim: zero-shot
...
{[}38{]} JIANG Y, WANG C, ZHANG R, et al.~TRANSIC: sim-to-real policy
transfer by learning from online correction{[}EB/OL{]}.
(2024){[}2026-06-26{]}. https://arxiv.org/abs/2405.10315.

{[}39{]} YE T, MAO Y, LIAO M, et al.~3D generation for embodied AI and
robotic simulation: a survey{[}EB/OL{]}. (2026){[}2026-06-26{]}.
https://arxiv.org/abs/2604.26509.

{[}40{]} PFAFF N, FU E, BINAGIA J P, et al.~Scalable real2sim:
physics-aware asset generation via robotic pick-and-place
setups{[}C{]}//IROS 2025.
https://doi.org/10.1109/IROS60139.2025.11246653.

{[}41{]} JIANG H, HSU H, ZHANG K, et al.~PhysTwin: physics-informed
reconstruction and simulation of deformable objects from
videos{[}C{]}//ICCV 2025. https://doi.org/10.1109/ICCV51701.2025.00678.

{[}42{]} SADEGHI F, LEVINE S. CAD2RL: real single-image flight without a
single real image{[}C{]}//Robotics: Science and Systems XIII (RSS 2017),
paper 34. https://www.roboticsproceedings.org/rss13/p34.pdf.

{[}43{]} RUSU A A, VECERIK M, ROTHÖRL T, et al.~Sim-to-real robot
learning from pixels with progressive nets{[}EB/OL{]}.
(2016){[}2026-08-29{]}. https://arxiv.org/abs/1610.04286.

{[}44{]} RAO K, HARRIS C, IRPAN A, et al.~RL-CycleGAN: reinforcement
learning aware simulation-to-real{[}C{]}//CVPR 2020.
https://arxiv.org/abs/2006.09001.

{[}45{]} CHEBOTAR Y, HANDA A, MAKOVIYCHUK V, et al.~Closing the
sim-to-real loop: adapting simulation randomization with real world
experience{[}C{]}//ICRA 2019: 8973-8979.
https://arxiv.org/abs/1810.05687.

{[}46{]} RAMOS F, POSSAS R C, FOX D. BayesSim: adaptive domain
randomization via probabilistic inference for robotics
simulators{[}C{]}//Robotics: Science and Systems XV (RSS 2019).
https://arxiv.org/abs/1906.01728.

{[}47{]} HWANGBO J, LEE J, DOSOVITSKIY A, et al.~Learning agile and
dynamic motor skills for legged robots{[}J{]}. Science Robotics, 2019,
4(26): eaau5872. https://doi.org/10.1126/scirobotics.aau5872.

{[}48{]} CHEN X, HU J, JIN C, et al.~Understanding domain randomization
for sim-to-real transfer{[}C{]}//ICLR 2022.
https://openreview.net/forum?id=T8vZHIRTrY.

{[}49{]} HARLAP A, NARAYANAN D, PHANISHAYEE A, et al.~PipeDream: fast
and efficient pipeline parallel DNN training{[}EB/OL{]}.
(2018){[}2026-08-29{]}. https://arxiv.org/abs/1806.03377.

{[}50{]} RUSU A A, RABINOWITZ N C, DESJARDINS G, et al.~Progressive
neural networks{[}EB/OL{]}. (2016){[}2026-08-29{]}.
