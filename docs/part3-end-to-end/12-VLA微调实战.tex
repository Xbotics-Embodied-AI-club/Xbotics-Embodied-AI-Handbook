...
的单卡容量线。①单卡基线 124
GB，高高在线上；②开省显存四件套后橙色的优化器状态整段消失（搬到卡外）、斜纹的激活段被梯度检查点压扁，落到
31 GB；③DDP 那根柱子仍标着 124 GB，一条双向箭头从 ① 拉到
③、上面写着''加卡只提速，单卡显存一点没降''，右侧几根灰柱注着''另外 7
张卡各存一模一样的一份''；④ZeRO-2 把梯度段和优化器段各切成 1/8，降到 38
GB；⑤ZeRO-3 把三段全部切成 1/8，降到 26
...
图上有两处正好解释了这条决策线为什么是这个顺序。\textbf{第一处是 ③
那根柱子}：它和 ① 一模一样高，因为 DDP
只是把数据分给多张卡，每张卡仍然要完整地放下参数、梯度和优化器状态------所以第
1
步只解决''慢''，不解决''放不下''，单卡放不下时加多少张卡都没用。\textbf{第二处是
② 和 ④ 的高低}：省显存四件套（31 GB）比 ZeRO-2（38
