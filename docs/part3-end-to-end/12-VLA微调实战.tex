...
\caption{图：一条从左到右的链------大规模 Web 数据经预训练得到 Base
...
\caption{图：同一个医学问题下基座模型与微调后模型的回答对照，左侧只答
...
{\centering \includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/httpssubstack-post-media.s3.amazonaws.compublicimagesfea460ea-84d5-4973-9bc7-dc0e53a13ae0_1340x680.png}

}

\caption{图：In-Context Learning 的 prompt 结构------先给两个 German →
English 的示例，再让模型翻译一句没见过的新句子}

\end{figure}%

做法很简单：在 prompt
里塞几个示例，模型就能''临时''学会新任务。注意''临时''二字------上图那两个
German → English
的示例只活在这一次对话里，模型权重一个字节都没变，下一次提问还得把它们重新贴一遍。

硬伤也就在这里：示例要占上下文窗口、对小模型效果差、知识不进权重所以无法持久化。这三条一旦顶到，就该微调上场了。差别有多大？同样问''为什么天空是蓝色的''，基座模型常常一句话打发，微调过的模型会顺着瑞利散射把道理讲下来------不是它临时被提示了，而是它本来就带着这套语境。
...
可以全量训练，也可以只训练 LoRA
适配器。它们不是互斥分类，而是可以组合的两个坐标轴------记住这一点，后面遇到''LoRA
...
\caption{图：一张三行三列的方法地图。列是参数更新范围------全模型全参数
/ 模块级全参数 /
...
GRPO；红框那格 LoRA SFT
标着''本讲主线''。三个空格不是不能组合，只是本讲没走那条路（本书自绘）}
...
\caption{图：不同层解冻方案的对照------上半部分是训练/验证/测试准确率，下半部分是训练时间，横轴从只训练最后一层排到解冻全部层}
...
\caption{图：从 Feature-based 到 Finetuning I、Finetuning II
三档的性能与成本权衡}
...
\caption{图：三个并排的紫色标题条把常见 PEFT 方法分成三栏。Prompt
...
适配器仍以较高精度训练。所以它不是和 LoRA 并列的另一种适配器结构，而是把
LoRA 挂在一个被量化加载的大模型上。
...
\caption{图：左侧是一个 Transformer 层，两个 Adapter
...
Adapter，切换任务就是切换 Adapter。原始论文{[}3{]}报告，在它测的那批 NLP
任务上，仅训练 3.6\% 的参数就能接近全量微调性能。
...
\caption{图：指令微调的数据格式------每条样本由
PROMPT（指令或任务描述）与 COMPLETION（希望模型学会的标准回答）配对组成}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0698\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1275\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5403\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2624\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
阶段 & 做什么 & 常见训练信号 & 关注点 \\
\midrule\noalign{}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1364\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3098\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5538\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
模型 & 动作表示 & 微调时要关注什么 \\
\midrule\noalign{}
...
SO-101 的那批仿真数据就是这么来的，第16讲 6.3 节会把这套''视觉 RL
生成数据''的流程从头讲透。
...
\subsection{2.3 实验：把 SmolVLA 全量微调到
SO-101}\label{ux5b9eux9a8cux628a-smolvla-ux5168ux91cfux5faeux8c03ux5230-so-101}

配套代码给了两条路线，一条不需要真机、一条用公开真机数据。

\textbf{路线一：纯仿真闭环}（\texttt{code/vla/5\_vla\_finetune/5\_2\_smolvla\_full\_sft/}）。数据是第16讲
6.3 节那条''视觉 RL 生成数据''链路的产物------SAC 专家在 SO-101
仿真里自己练成，再把成功推演转成 LeRobot 数据集；微调从社区预训练权重
...
这条线上真正卡人的不是训练本身，而是\textbf{观测特征对齐}：\texttt{smolvla\_base}
...
\textbf{路线二：公开真机数据}（\texttt{code/vla/5\_vla\_finetune/5\_4\_so101\_real\_sft/train\_so101\_real.sh}）。同一个脚本按
\texttt{MODEL} 变量在 \(\pi_0\) / SmolVLA / ACT 之间切换，对一批公开的
SO-101
遥操作数据做全参微调；\texttt{code/vla/5\_vla\_finetune/5\_4\_so101\_real\_sft/verify\_checkpoint.py}
负责在每个存点上验一遍''这个 checkpoint
到底学没学到东西''。它的判据值得单独看一眼：不用
\texttt{corr(pred,\ action)}，因为这批数据里 \texttt{action}
是主臂目标位姿、\texttt{state}
是从臂实测位姿，两者天然贴在一起------什么都不学、原样照抄
\texttt{state}，逐关节相关系数就能到 0.9 以上。换成''模型误差 /
照抄误差''这个比值，照抄基线恒为 1，模型必须明显低于 1 才算学到了东西。

\textbf{该盯什么。} loss
下降只说明拟合在进行，不说明任务做得成；先看评测里的
\texttt{pc\_success}，再看上面那个验收脚本报出的比值有没有稳定压到 1
以下。但这个比值本身也有边界，脚本的说明里写得很清楚：它做的是\textbf{开环逐帧比对}------每一帧都先把策略状态清空再独立预测一次。真机上动作是连着下发的，误差会一步步累积，所以比值压到
1
以下只证明模型学到了非平凡的东西，\textbf{不等于它在真机上能把任务做完}；闭环那一步得有真机才能验。仿真那条线还有一个前提要先知道：训练数据所在的
ReachCube 任务在 \texttt{so101\_sim}
重构后已经下线，\texttt{smolvla\_eval.sh} 只能把这个 checkpoint
放进现存的 PickPlaceCube40
场景里评，属于任务不匹配，\texttt{pc\_success} 接近 0
是预期结果、不代表微调失败------脚本注释与 \texttt{5\_2} 的 README
都写明了这一点。想拿到一个真能读的成功率，得让训练与评测落在同一个现存任务上。
...
\caption{图：左半标注''训练中''------输入 \(x\) 兵分两路，一路过冻结的
\(W \in \mathbb{R}^{d \times d}\)，一路经下投影 \(A\)（图上标
\(\mathcal{N}(0,\sigma^2)\)）压到 \(r\) 维、再由上投影 \(B\)（图上标
\(B=0\)）抬回原维度，两路输出相加；右半标注''训练后''------\(W\) 与
\(BA\) 已合并成单块 \(W_{merged}\)，旁路不再出现}
...
{\centering \includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/lora_transformer_targets.png}

}

\caption{图：一个 Transformer 层里可以挂 LoRA
旁路的线性层------注意力侧的 Query / Key / Value 与输出投影，以及
MLP/FFN 侧的各个投影矩阵}
...
用 LoRA
微调一个模型一共五步：加载并冻结基座、注入旁路、训练、保存适配器、推理时合并回去。
...
\NormalTok{model }\OperatorTok{=}\NormalTok{ AutoModelForCausalLM.from\_pretrained(}\StringTok{"meta{-}llama/Llama{-}2{-}7b{-}hf"}\NormalTok{)}
...
\item
  Rank 不是越大越稳定地更好。QLoRA 论文{[}2{]}报告：当 LoRA
...
{\centering \includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/qlora_architecture.png}

}

\caption{图：输入 \(x\) 分成两条支路------上支是冻结基座 \(W_0\)，标注为
NF4 / 4-bit 存储、计算时反量化到 BF16；下支是 LoRA 旁路，标注为
BF16/FP16 的 \(A\)、\(B\)
矩阵，并注明''只有这里接收梯度''；两支输出相加得到
...
\NormalTok{model }\OperatorTok{=}\NormalTok{ AutoModelForCausalLM.from\_pretrained(}
    \StringTok{"meta{-}llama/Llama{-}2{-}70b{-}hf"}\NormalTok{,}
...
{\centering \includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture12/files/qlora_memory.png}

}

\caption{图：基座模型权重显存的粗略对比，四根柱子分两组------7B 在 fp16
下约 14 GB、压到 4-bit 后约 3.5 GB；65B 在 fp16 下约 130 GB、压到 4-bit
后约 32.5 GB（本书自绘；四个数的来源是按 fp16 每参数 2 字节、4-bit
每参数 0.5 字节直接折算）}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1267\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2877\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2963\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2893\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
65B/70B & \textsubscript{780}840 GB & \textasciitilde140 GB & \textasciitilde36 GB \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1378\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2031\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3125\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3466\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
量级、甚至更大的 VLA，单卡立刻开始吃紧------2.3
节那个真机微调脚本默认还是单卡在训 \(\pi_0\)，代价是权重压到
bf16、批量只敢给
...
parallelism）}{[}18{]}。当一份模型副本本身就装不进一张卡时，数据并行无能为力，这时只能把\textbf{模型本身切开}，不同部分放到不同卡上。
...
4.4
节的主角，这里只先把它在地图上的位置钉住：\textbf{它不是第三条轴，而是数据并行轴上的省显存升级。}
...
\begin{quote}
备注：ZeRO 论文{[}17{]}给的直觉例子------一个 7.5B 参数的模型，混合精度
Adam 下基线每卡约
...
Data Parallel，完全分片数据并行）}{[}23{]}。名字里的''Fully
Sharded''就是 ZeRO Stage 3
的精神：参数、梯度、优化器状态\textbf{全部分片}，每张卡只存 1/N。
...
\caption{图：Megatron 把 MLP 子层做张量并行的切法。第一个权重矩阵 \(A\)
...
\texttt{-\/-policy.optimizer\_lr}（2.3
节那个真机微调脚本用的就是它）。ACT 的预置学习率是
...
\caption{图：一根 7B 模型的单卡显存柱，在五种方案下怎么变。柱子按 4.1.2
...
16 字节/参数精确折算（本书自绘；分片按 8 卡算，与 4.4.6
节表里的公式一致）}
...
Solutions, and Best
Practices》{[}26{]}系统总结了这个问题：现实差距不是一个单点误差，而是视觉、动力学、传感器、执行器和软件栈多层误差共同叠加的结果。
...
\begin{quote}
小结：仿真不是奢侈品而是必需品；但要把仿真里学到的东西真正落地，必须正面应对现实差距。Sim2Real
这一整套方法就是为这一步存在的。
\end{quote}
...
很多失败的 Sim2Real 不是算法问题，而是软件栈对不齐。
...
\begin{quote}
小结：现实差距不是单一问题，而是四类差距的叠加。本节后面讲的每种方法，都可以放到这张表上对号入座。
\end{quote}
...
\caption{图：左侧''训练''栏是九张仿真渲染图，同一张桌子和同一批几何体在每张里换了一套颜色、纹理、光照和相机角度，桌面从粉到绿到紫无所不有；右侧''测试''栏只有一张真机照片------普通木桌上摆着彩色积木。（图取自参考文献
{[}27{]}）}
...
是这条轨迹上的损失。模型不是在某一个仿真上最优，而是在整个 \(\xi\)
...
\caption{图：上下两行结构完全相同，差别只在最左边那一格。上行''训练''从高度随机化的仿真图出发，下行''测试''从真机照片出发；两者都先过同一个生成器
...
\caption{图：论文的方法总览，分三格。左格是输入输出示例------上排是机械臂的图像观测序列，下排三张图叠着彩色散点，画的是同一个观测下采出的多条候选动作序列。中格是通用形式：观测
\(\mathbf{O}_t\) 送进去噪网络
...
左格那三张彩色散点图是这张图里最该看的：同一个观测下，动作不是收敛到一个点，而是散成几团------\textbf{``多模态''在这里是画得出来的}，而这正是下面三条特点里第一条要说的事。
...
\caption{图：OpenVLA 的数据通路。左侧输入是一张相机图和一句语言指令
``Put eggplant in bowl''；图像经 DinoV2 与 SigLIP 两个编码器并联，再过
MLP 投影成视觉 token，指令经 Llama Tokenizer 成文本 token，两串 token
...
\caption{图：\(\pi_0\) 的框架。左端是训练数据来源------\(\pi\)
...
\caption{图：Ye 等人综述的全文外框，自上而下五层：预备知识（三维表示 /
生成模型 / 仿真平台与资产 /
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1092\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4015\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4892\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
层级 & 主要问题 & 典型输出 \\
\midrule\noalign{}
...
pattern，衣服在裁剪台上摊平后的二维片形）可以充当仿真原生的中间表示：它不是单纯的视觉网格，而是布料仿真器能直接理解的结构。一般软体物体还没有类似统一的中间表示。PhysGaussian、PhysDreamer、GarmentDreamer、DressCode
...
这类平台提供大量房间、厨房、家居任务。它们的价值在于：场景不是静态图片，而是可以被智能体走动、抓取、打开、移动和重置。
...
\caption{图：中间橙色框是生成式仿真域，内含三列------数字孪生列（自动场景重建
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0833\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6771\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2396\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
评估层级 & 常见指标 & 局限 \\
\midrule\noalign{}
...
Real2Sim
要交付的不是''把真实世界重建得很像''，而是''把真实世界转换成机器人仿真器可执行的三维内容''------资产要同时满足几何有效、物理参数化、运动学可执行和格式兼容四项。Ye
等人{[}39{]}的三段式结构可以概括为：
