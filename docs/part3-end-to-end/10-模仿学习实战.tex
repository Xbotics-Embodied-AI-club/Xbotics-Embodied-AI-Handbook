...
在机器人模仿学习里，生成模型扮演的角色首先是一个''动作分布建模器''：它要做的不是生成一张好看的图片，而是把多个峰都表达出来，让''从左绕''和''从右绕''各自保留成一个模式，而不是被平均成中间那个谁也做不到的动作。这就是生成模型进入机器人策略学习的入口。
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/fig10-2-act-architecture_arxiv-2304.13705.png}}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/fig10-2-act-step3-dataflow.png}
...
\(k\) 步动作块。两处标注按 LeRobot 默认写：特征通道数 512、decoder 1
层（本书自绘）}
...
728、再线性投影到 512，与本节按 ResNet18 讲的 512 也对不上------ResNet18
的 \texttt{layer4} 输出就是 512 通道，所以图上直接标
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/fig10-2-chunking-vs-temporal-ensemble.png}
...
\(w_i \propto \exp(-mi)\) 在 \(m=0.01\) 时的真实取值（本书自绘）}
...
关键在于：\textbf{loss 下降不等于任务成功}（第8讲 3.1
...
0------这不是训练坏了，而是这个任务要求''抓起碗、移过去、放到盘子上''三步全对才算成功，前面若干次评估里策略往往只做对了前两步，成功率是离散的
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/fig10-3-libero-success-curve.png}

}

\caption{这条曲线来自本仓库真跑过的一次 LIBERO ACT
...
\tightlist
\item
  \textbf{有真机}：用第9讲 4 节采的 SO-101
  主从臂遥操作数据（例如''抓方块放盘子''），同样用
  \texttt{lerobot-train} 训 ACT，训练命令封在
...
尾部那批样本的监督就会被越界部分污染。也就是说，补位掩码不是边角细节，而是动作块监督能正常工作的必要条件。
...
是因为 ACT
也支持没有本体状态、只有图像的数据集------那种情况下序列里少一个
token，长度变成 \(S+1\)。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1190\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2488\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6322\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/fig10-4-diffusion-policy-arch_arxiv-2303.04137.jpg}}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1679\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.7131\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1190\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 典型值 \\
\midrule\noalign{}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/fig10-5-flow-matching-path.png}
...
时格子的轮廓已经显形（本书自绘：直接按插值式采样画出解析的概率路径，未训练模型）}

\end{figure}%

要说清楚这五格画的到底是什么：它们不是某个训练好的模型跑出来的采样轨迹，而是\textbf{概率路径本身}------按
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/fig10-5-linear-interpolation-path.png}

}

\caption{左列是采到的噪声样本，右列是数据集里的样本（画成双峰）。同一下标的两个点配成一条\textbf{条件路径}，图里画了
7
条：每个噪声点各自连向一个数据点，端点是独立采样再配对的，所以这些直线彼此交叉。红色那条被高亮出来，标出
\(z_0\)、\(z_1\) 与它们之间的插值点 \(z_t\)（本书自绘）}
...
个控制时刻''占住了，再用 \(t\) 兼指流的时间就会撞车------第 4
节把扩散步记成 \(i\)
是出于同一个理由。下面写的是一个教学版、最常见的条件化 Flow Matching
...
\begin{quote}
第 5 节的二维棋盘格流动过程图由本书的 Flow Matching 示例脚本渲染得到。
\end{quote}
