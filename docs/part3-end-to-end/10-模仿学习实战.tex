...
上一讲我们采到了一份能训练的数据集：每条轨迹都是人类示教的一串
\((o_t, a_t)\) 配对。这一讲开始，从数据里真正把策略学出来。

最朴素、也最直接的做法就是\textbf{行为克隆（Behavioral Cloning,
BC）}：把示教数据当成监督学习的训练集，输入观测、回归动作。它离线、安全、不需要
reward，是最能落地的起点。但朴素 BC
很快会撞上两堵墙，这两堵墙正是后面所有生成式策略（ACT、Diffusion
Policy、Flow Matching）出现的根本原因。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0777\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5887\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3336\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 典型维度 \\
\midrule\noalign{}
...
就偏离专家轨迹，进入训练集从未覆盖的
OOD（out-of-distribution）区域。模型在 OOD
状态下更容易出错，误差沿时间轴滚雪球：
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/bc_covariate_shift.jpg}

}

\caption{point-wise policy 在 covariate shift 下如何逐步失控}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/bc_multimodal.jpg}

}

\caption{双峰示教被平均后，策略输出会变得犹豫甚至不安全}

\end{figure}%

\subsection{1.4
要学的不是一个动作，而是一个分布}\label{ux8981ux5b66ux7684ux4e0dux662fux4e00ux4e2aux52a8ux4f5cux800cux662fux4e00ux4e2aux5206ux5e03}
...
换句话说，在机器人模仿学习里，生成模型首先是一个''动作分布建模器''：它的任务不是生成图片，而是表达多峰动作分布、保留不同示教风格、而不是强行平均。这就是生成模型进入机器人策略学习的入口。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1762\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2791\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1713\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3734\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
方法 & 核心想法 & 推理方式 & 在机器人里的意义 \\
\midrule\noalign{}
\textbf{VAE / CVAE} & 用 latent variable 表达隐含模式 & 一次采样 + 一次解码 & 适合 chunk-level 动作生成，直接通向 ACT \\
\textbf{Diffusion} & 从噪声开始，逐步去噪得到动作 & 多步迭代 & 更强的多模态建模，直接通向 Diffusion Policy \\
\textbf{Flow Matching} & 学习从简单分布到目标分布的连续流 & ODE 积分，通常步数更少 & 是新一代 VLA action head 的关键基础 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
\begin{quote}
论文：\emph{Learning Fine-Grained Bimanual Manipulation with Low-Cost
Hardware}，Tony Z. Zhao et al., 2023 arXiv: 2304.13705
\end{quote}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1490\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2425\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6084\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
问题 & ACT 的解法 & 核心思想 \\
\midrule\noalign{}
...
\begin{quote}
备注：Action Chunking
是\textbf{缓解}误差累积，不是消除分布漂移（covariate
shift）。它把''每步都重新决策''换成''每 \(k\)
...
\begin{quote}
备注：原论文里还重点讨论了 \textbf{temporal ensembling}
这一执行方式；本文主线以 LeRobot
默认实现（\texttt{temporal\_ensemble\_coeff\ =\ None}）为准，论文设定作为补充说明。
\end{quote}

ACT 不是简单的回归器，而是一个\textbf{条件变分自编码器（CVAE）}。在讲
ACT 的具体结构之前，先把 CVAE 的来龙去脉讲清楚。
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/01_cvae.png}}
...
简单类比：KL
散度就像一根弹簧，把编码器的输出往原点拉。弹簧太松（\(\beta\)
小），隐空间散乱，采样质量差；弹簧太紧（\(\beta\)
大），所有数据被压到同一个点，丢失多样性。好的 \(\beta\)
在两者之间取平衡。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0601\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1969\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.7431\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 在 ACT 中对应 \\
\midrule\noalign{}
...
\((s_t, \mathbf{a}_{t:t+k})\) 去近似后验；图像是在右侧 policy decoder
中使用的。这是一个很容易和''概念上的条件变量 \(o_t\)``混淆的点。
\end{quote}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/structure.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/02_training1.png}}
...
VAE Encoder 是一个 BERT 风格的 Transformer Encoder。它的输入是一个 token
序列，每个 token 都是 \(d_{\text{model}} = 512\) 维的向量：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0792\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3229\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1367\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3604\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1008\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/02_training2.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/02_training3.png}}

}

\caption{训练过程 Step 3：Transformer 预测动作序列}
...
每路相机图像先经过 \textbf{ResNet18} backbone（ImageNet 预训练权重），取
\texttt{layer4} 的输出特征图，再经过 \(1 \times 1\) 卷积投影到
\(d_{\text{model}} = 512\) 维，最后展平空间维度变成 token 序列：
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3917\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1526\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2903\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1653\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
LeRobot 中 \texttt{n\_decoder\_layers\ =\ 1}。虽然原始 ACT
代码配置里写的是 7 层，但社区发现其 released implementation
实际只用到了第 1 层；LeRobot 因而直接把默认值设为 1
以对齐这一行为。Decoder 输出经过线性 action head 投影到动作空间：
...
逐元素 L1，对 padding 位置做 mask
后取均值------直觉上就是''预测的每一步每一个关节，和真实值的绝对误差''：

\[
\mathcal{L}_{\text{L1}} = \operatorname{mean}\!\left(\,|\mathbf{a} - \hat{\mathbf{a}}| \odot \mathbf{1}_{\text{not pad}}\right)
\]

其中 \(\mathbf{1}_{\text{not pad}}\) 是非 padding 位置的掩码。当一段
chunk 落在 episode 尾部、未来动作不足 \(k\) 步时，超出部分会被 padding
填上，再被这个掩码屏蔽掉，不参与损失。
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture10/ref/output.png}

}

\caption{Action Chunking 与 Temporal Ensemble}
...
先用 LeRobot 的 ACT 在 LIBERO 数据集上训练一个抓取策略（例如
\texttt{libero\_goal}
套件里''把碗放到盘子上''这条任务）。流程是标准三步：
...
关键在于：\textbf{loss
下降不等于任务成功}（这一点在第8讲已经强调过），所以必须靠仿真器内的闭环
rollout 来真正判断策略好不好。LIBERO
这类带成功判定的仿真环境，正好提供了这条闭环验证的标尺。

\subsection{3.2 在 SO-101 主从臂上训练
ACT（无真机用仿真器替代）}\label{ux5728-so-101-ux4e3bux4eceux81c2ux4e0aux8badux7ec3-actux65e0ux771fux673aux7528ux4effux771fux5668ux66ffux4ee3}
...
\tightlist
\item
  \textbf{有真机}：用上一讲采的 SO-101
  主从臂遥操作数据（例如''抓方块放盘子''），同样用
  \texttt{lerobot-train} 训 ACT，再把训练好的策略部署回 SO-101
  做闭环执行；动作维度 \(d_a\)、状态维度
...
在 LeRobot 里的实现按数据流逐层拆开。代码基于 LeRobot 源码，训练脚本以
3.1 节的 LIBERO 训练为例、使用默认 \texttt{ACTConfig}。
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. 从 lerobot/libero 数据集中筛选 "put the bowl on the plate" 的轨迹}
\NormalTok{metadata }\OperatorTok{=}\NormalTok{ LeRobotDatasetMetadata(}\StringTok{"lerobot/libero"}\NormalTok{)}
\NormalTok{episodes }\OperatorTok{=}\NormalTok{ get\_task\_episodes(metadata, }\StringTok{"put the bowl on the plate"}\NormalTok{)}

\CommentTok{\# 2. 配置 ACT 策略（使用默认超参）}
\NormalTok{policy\_cfg }\OperatorTok{=}\NormalTok{ ACTConfig(device}\OperatorTok{=}\StringTok{"cuda"}\NormalTok{, push\_to\_hub}\OperatorTok{=}\VariableTok{False}\NormalTok{)}

\CommentTok{\# 3. 配置数据集（只用筛选出的 episode）}
\NormalTok{dataset\_cfg }\OperatorTok{=}\NormalTok{ DatasetConfig(repo\_id}\OperatorTok{=}\StringTok{"lerobot/libero"}\NormalTok{, episodes}\OperatorTok{=}\NormalTok{episodes)}

\CommentTok{\# 4. 配置 LIBERO 评估环境（libero\_goal suite, task 8）}
\NormalTok{env\_cfg }\OperatorTok{=}\NormalTok{ LiberoEnvConfig(task}\OperatorTok{=}\StringTok{"libero\_goal"}\NormalTok{, task\_ids}\OperatorTok{=}\NormalTok{[}\DecValTok{8}\NormalTok{],}
\NormalTok{                          observation\_height}\OperatorTok{=}\DecValTok{256}\NormalTok{, observation\_width}\OperatorTok{=}\DecValTok{256}\NormalTok{)}

\CommentTok{\# 5. 组装 pipeline 并训练}
\NormalTok{cfg }\OperatorTok{=}\NormalTok{ TrainPipelineConfig(dataset}\OperatorTok{=}\NormalTok{dataset\_cfg, env}\OperatorTok{=}\NormalTok{env\_cfg, policy}\OperatorTok{=}\NormalTok{policy\_cfg, ...)}
\NormalTok{train(cfg)}
\end{Highlighting}
\end{Shaded}

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2656\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1250\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3750\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
训练参数 & 值 & 含义 \\
\midrule\noalign{}
\texttt{TRAIN\_\allowbreak STEPS} & 100000 & 总训练步数 \\
\texttt{BATCH\_\allowbreak SIZE} & 256 & 批大小 \\
\texttt{EVAL\_\allowbreak EPISODES} & 10 & 每次评估的 episode 数 \\
\texttt{EPOCHS\_\allowbreak PER\_\allowbreak EVAL} & 2 & 每 2 个 epoch 评估一次 \\
\texttt{LOG\_\allowbreak FREQ} & 5 & 每 5 步记录一次日志 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
首先要注意：\textbf{ACT 的 chunk supervision 是 dataset
层生成的，不是模型内部现拼出来的。}
...
文件位置 \texttt{policies/act/configuration\_act.py}，核心类：
...
它继承自
\texttt{PreTrainedConfig}（\texttt{configs/policies.py}），天然具备：\texttt{input\_features}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2612\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3940\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1166\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2282\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
参数 & 含义 & 默认值 & 训练脚本中的值 \\
\midrule\noalign{}
...
注意 \texttt{n\_decoder\_layers\ =\ 1}。原始 ACT 论文代码写了 7 层，但有
bug 导致实际只用了第 1 层。LeRobot 直接设为 1 以匹配原始行为（见
\texttt{configuration\_act.py} 的相关注释）。
...
文件位置
\texttt{datasets/lerobot\_dataset.py}、\texttt{datasets/factory.py}（\texttt{resolve\_delta\_timestamps}
函数）。
...
具体代码在 \texttt{datasets/factory.py}：

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ resolve\_delta\_timestamps(cfg: PreTrainedConfig, ds\_meta: LeRobotDatasetMetadata):}
...
\NormalTok{            delta\_timestamps[key] }\OperatorTok{=}\NormalTok{ [i }\OperatorTok{/}\NormalTok{ ds\_meta.fps }\ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in}\NormalTok{ cfg.action\_delta\_indices]}
        \CommentTok{\# ... 类似处理 observation 和 reward}
    \ControlFlowTok{return}\NormalTok{ delta\_timestamps }\ControlFlowTok{if} \BuiltInTok{len}\NormalTok{(delta\_timestamps) }\OperatorTok{\textgreater{}} \DecValTok{0} \ControlFlowTok{else} \VariableTok{None}
\end{Highlighting}
\end{Shaded}
...
一起取出来。这也说明：\textbf{ACT 的 chunk supervision
是数据层根据配置主动生成的。}

在
\texttt{LeRobotDataset.\_\_getitem\_\_()}（\texttt{lerobot\_dataset.py}）里：先取当前帧
\texttt{item\ =\ self.hf\_dataset{[}idx{]}}；如果配置了
\texttt{delta\_indices}，调用
\texttt{\_get\_query\_indices(abs\_idx,\ ep\_idx)} 和
\texttt{\_query\_hf\_dataset(query\_indices)}，把未来若干时刻的 action
拼进来，同时生成 padding mask
\texttt{action\_is\_pad}。\texttt{\_get\_query\_indices}（\texttt{lerobot\_dataset.py}）的核心逻辑：
...
\NormalTok{        [(abs\_idx }\OperatorTok{+}\NormalTok{ delta }\OperatorTok{\textless{}}\NormalTok{ ep\_start) }\OperatorTok{|}\NormalTok{ (abs\_idx }\OperatorTok{+}\NormalTok{ delta }\OperatorTok{\textgreater{}=}\NormalTok{ ep\_end)}
         \ControlFlowTok{for}\NormalTok{ delta }\KeywordTok{in}\NormalTok{ delta\_idx]}
\NormalTok{    )}
    \ControlFlowTok{for}\NormalTok{ key, delta\_idx }\KeywordTok{in} \VariableTok{self}\NormalTok{.delta\_indices.items()}
\NormalTok{\}}
\end{Highlighting}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2344\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2188\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2188\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
键 & 形状 & 含义 \\
\midrule\noalign{}
\texttt{action} & \((k, d_a) = (100, 7)\) & 动作块 \\
\texttt{action\_\allowbreak is\_\allowbreak pad} & \((k,) = (100,)\) & padding 掩码 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

如果没有 \texttt{action\_is\_pad}，episode
尾部那批样本的监督就会被越界部分污染。也就是说，padding mask
不是边角细节，而是 chunk supervision 能正常工作的必要条件。
...
\item
  ...
  下并不是这个扁平结构，而是嵌套的
  \texttt{robot\_state/eef/...}、\texttt{robot\_state/gripper/...}
\item
...
当前 LeRobot
的环境预处理逻辑（\texttt{processor/env\_processor.py}）里，这个压平过程是：
...
工厂入口 \texttt{policies/factory.py}，对 \texttt{ACTConfig} 最终走到
\texttt{policies/act/processor\_act.py} 的
...
实现其实分工非常清楚：dataset 负责把样本组织成 chunk-level
监督、processor 负责归一化和设备搬运、policy / model
负责神经网络前向。postprocessor 的步骤：
...
文件位置 \texttt{policies/act/modeling\_act.py}，外层类
...
\NormalTok{    config.validate\_features()}
    \VariableTok{self}\NormalTok{.model }\OperatorTok{=}\NormalTok{ ACT(config)                    }\CommentTok{\# 核心神经网络}
...
\NormalTok{        mean\_kld }\OperatorTok{=}\NormalTok{ (}\OperatorTok{{-}}\FloatTok{0.5} \OperatorTok{*}\NormalTok{ (}\DecValTok{1} \OperatorTok{+}\NormalTok{ log\_sigma\_x2\_hat }\OperatorTok{{-}}\NormalTok{ mu\_hat.}\BuiltInTok{pow}\NormalTok{(}\DecValTok{2}\NormalTok{)}
                    \OperatorTok{{-}}\NormalTok{ log\_sigma\_x2\_hat.exp())).}\BuiltInTok{sum}\NormalTok{(}\OperatorTok{{-}}\DecValTok{1}\NormalTok{).mean()}
...
\item
  ...
  \((B, k, d_a) = (B, 100, 7)\)
\item
  监督信号是 dataset 提供的 chunk-level
  \texttt{batch{[}ACTION{]}}，形状同样是 \((B, 100, 7)\)，并通过
  \texttt{action\_is\_pad} 屏蔽越界位置
\end{enumerate}
...
\NormalTok{    actions }\OperatorTok{=} \VariableTok{self}\NormalTok{.model(batch)[}\DecValTok{0}\NormalTok{]}
    \ControlFlowTok{return}\NormalTok{ actions  }\CommentTok{\# (B, chunk\_size, action\_dim) = (B, 100, 7)}
\end{Highlighting}
\end{Shaded}

注意：\texttt{batch{[}OBS\_IMAGES{]}} 被整理成了一个
list，而不是保留原始的多个键。底层 \texttt{ACT}
模型期待的是图像列表，不是分散的键。
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{self}\NormalTok{.vae\_encoder }\OperatorTok{=}\NormalTok{ ACTEncoder(config, is\_vae\_encoder}\OperatorTok{=}\VariableTok{True}\NormalTok{)        }\CommentTok{\# 4 层 Transformer Encoder}
\VariableTok{self}\NormalTok{.vae\_encoder\_cls\_embed }\OperatorTok{=}\NormalTok{ nn.Embedding(}\DecValTok{1}\NormalTok{, config.dim\_model)    }\CommentTok{\# [CLS] token}
\VariableTok{self}\NormalTok{.vae\_encoder\_robot\_state\_input\_proj }\OperatorTok{=}\NormalTok{ nn.Linear(d\_s, dim\_model)   }\CommentTok{\# 状态投影}
\VariableTok{self}\NormalTok{.vae\_encoder\_action\_input\_proj }\OperatorTok{=}\NormalTok{ nn.Linear(d\_a, dim\_model)        }\CommentTok{\# 动作投影}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.4139\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1255\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1358\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3248\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
组件 & 输入维度 & 输出维度 & 含义 \\
\midrule\noalign{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{backbone\_model }\OperatorTok{=}\NormalTok{ torchvision.models.resnet18(}
...
取 ResNet18 的 \texttt{layer4} 输出。对于 \(256 \times 256\) 输入图像：
...
主干结构可以概括为：\texttt{VAE\ encoder\ +\ 图像\ backbone\ +\ 主\ encoder/decoder\ +\ 动作回归头}。\texttt{Transformer}
子模块、位置编码和 \texttt{Temporal\ Ensembler} 的实现细节这里不再展开。
...
\NormalTok{cls\_embed }\OperatorTok{=}\NormalTok{ einops.repeat(}
    \VariableTok{self}\NormalTok{.vae\_encoder\_cls\_embed.weight, }\StringTok{"1 d {-}\textgreater{} b 1 d"}\NormalTok{, b}\OperatorTok{=}\NormalTok{batch\_size)             }\CommentTok{\# (B, 1, 512)}
...
\NormalTok{action\_embed }\OperatorTok{=} \VariableTok{self}\NormalTok{.vae\_encoder\_action\_input\_proj(batch[ACTION])               }\CommentTok{\# (B, 100, 512)}
\NormalTok{vae\_encoder\_input }\OperatorTok{=}\NormalTok{ torch.cat(}
\NormalTok{    [cls\_embed, robot\_state\_embed, action\_embed], axis}\OperatorTok{=}\DecValTok{1}\NormalTok{)                      }\CommentTok{\# (B, 102, 512)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1513\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5007\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1669\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1811\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
推理时没有未来动作 \(\mathbf{a}_{t:t+k}\)，无法运行 VAE
Encoder，当前实现会直接令
\(z = \mathbf{0} \in \mathbb{R}^{32}\)，因为：(1) 训练时 KL 项把后验
\(q(z \mid o_t, \mathbf{a})\) 约束到 \(\mathcal{N}(0, I)\) 附近；(2)
\(z = \mathbf{0}\) 是 \(\mathcal{N}(0, I)\) 的均值、概率密度最高；(3)
Decoder 在训练时见过大量 \(z \approx \mathbf{0}\) 的样本（因为
\(\beta = 10\) 的强 KL 约束）。\textbf{VAE encoder 只在训练时出现。}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{encoder\_in\_tokens }\OperatorTok{=}\NormalTok{ [}\VariableTok{self}\NormalTok{.encoder\_latent\_input\_proj(latent\_sample)]  }\CommentTok{\# latent token}
\NormalTok{encoder\_in\_tokens.append(}\VariableTok{self}\NormalTok{.encoder\_robot\_state\_input\_proj(batch[OBS\_STATE]))  }\CommentTok{\# robot state token}
\ControlFlowTok{for}\NormalTok{ img }\KeywordTok{in}\NormalTok{ batch[OBS\_IMAGES]:}
\NormalTok{    cam\_features }\OperatorTok{=} \VariableTok{self}\NormalTok{.backbone(img)[}\StringTok{"feature\_map"}\NormalTok{]              }\CommentTok{\# (B, 512, 8, 8)}
\NormalTok{    cam\_features }\OperatorTok{=} \VariableTok{self}\NormalTok{.encoder\_img\_feat\_input\_proj(cam\_features) }\CommentTok{\# (B, 512, 8, 8)}
...
\NormalTok{encoder\_in\_tokens }\OperatorTok{=}\NormalTok{ torch.stack(encoder\_in\_tokens, axis}\OperatorTok{=}\DecValTok{0}\NormalTok{)       }\CommentTok{\# (N\_enc, B, 512)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3464\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2892\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1569\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2074\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
Token 类型 & 来源 & 数量 & 投影方式 \\
\midrule\noalign{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{decoder\_in }\OperatorTok{=}\NormalTok{ torch.zeros((chunk\_size, batch\_size, dim\_model))  }\CommentTok{\# (100, B, 512)}
\NormalTok{decoder\_out }\OperatorTok{=} \VariableTok{self}\NormalTok{.decoder(}
\NormalTok{    decoder\_in, encoder\_out,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{decoder\_out }\OperatorTok{=}\NormalTok{ decoder\_out.transpose(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{)  }\CommentTok{\# (B, 100, 512)}
\NormalTok{actions }\OperatorTok{=} \VariableTok{self}\NormalTok{.action\_head(decoder\_out)     }\CommentTok{\# (B, 100, 7)}
...
\item
  ...
  \(\in \{0,1\}^{k}\)（padding 掩码，\((100,)\)）
\item
  dataset 根据 \texttt{ACTConfig.action\_delta\_indices} 生成
  chunk-level 监督
\item
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3241\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1371\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1179\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4210\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
超参数 & 符号 & 默认值 & 含义 \\
\midrule\noalign{}
\texttt{chunk\_\allowbreak size} & \(k\) & 100 & 一次预测的动作步数 \\
...
Transformer 输出 chunk-level 连续动作，最后在推理端把完整 chunk
放进动作队列并逐步消费成一步动作。
\end{quote}
...
\item
  ...
  做标准化、postprocessor
  反标准化（第8讲的安全边界一节已经强调）；统计量缺失会让动作尺度整体出错。
\item
...
参考：Ho, Jain \& Abbeel (2020) \emph{Denoising Diffusion Probabilistic
Models}（NeurIPS 2020）；Chi et al.~(2023) \emph{Diffusion Policy:
Visuomotor Policy Learning via Action Diffusion}（RSS 2023）；Zhao et
al.~(2023) \emph{Learning Fine-Grained Bimanual Manipulation with
...
也就是说，第 \(t\) 步的数据并不是''完全重画一遍''，而是在第 \(t-1\)
步的基础上做一个很小的扰动：先把原有信号稍微缩小一点，再叠加一小份随机噪声。

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1094\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4375\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3906\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 典型值 \\
\midrule\noalign{}
...
\(z_t\) 走向更干净的 \(z_{t-1}\)。在数学上，当 \(\beta_t\)
足够小时，反向过程也是高斯的：
...
问题在于：\textbf{推理时我们并不知道真正的 \(\epsilon\)
是多少}。\(\epsilon\) 必须由神经网络预测（即
...
\tightlist
\item
  当 \(t\) 很小（图像几乎清晰）时，网络能比较准确地预测噪声，直接反推
  \(z_0\) 效果尚可。
\item
  当 \(t\)
...
更稳定''的实践原因。换句话说，公式给出的是一条数学上的''捷径''；而扩散模型真正学到的是：在预测存在误差的现实条件下，怎样沿着这条捷径\textbf{稳扎稳打地一点点往回走}。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1277\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2687\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6036\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
Chi et al.~(2023) 提出的 Diffusion Policy 把 DDPM
的框架直接搬到了机器人策略学习中。核心改动是：\textbf{不建模完整的联合分布
\(p(o, a)\)，而是建模条件分布 \(p(a \mid o)\)。}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/diffusion_policy_arch.jpg}}
...
上图展示的是 Diffusion Policy
的一种典型条件去噪实现：左侧是带噪声的动作序列，下方是历史观测，中间的条件网络把两者融合起来，预测当前时间步上的噪声。经过
\(T\) 次迭代去噪后，右侧得到可执行的动作序列。图中的 \texttt{1DConv}
对应论文里的 CNN 型时序去噪网络；论文中也讨论了 Transformer 型实现。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1581\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4684\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3735\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
符号 & 含义 & 典型值 \\
\midrule\noalign{}
...
下图展示了一个 2D 棋盘格数据集上的 Flow Matching 过程。从 \(t=0\)
的标准正态噪声出发，分布沿着学到的向量场逐步流动，最终在 \(t=1\)
时形成清晰的棋盘格结构：

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/2d_flow_matching_visualize_path.png}}

}

\caption{2D Flow Matching：从噪声到棋盘格的流动过程}
...
Matching 直接学习驱动连续归一化流（CNF）的向量场，通常用 ODE
求解器采样。
...
条件路径时，才配得上''OT 路径''这个名字------那是 OT Flow Matching
的做法。
\end{quote}

下图展示了 1D 情况下的线性插值路径。每条线连接一个噪声点
\(z_0\)（左侧）和一个数据点 \(z_1\)（右侧），中间的 \(z_t\)
就是两者的线性混合：

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{../../assets/figures/lecture10/ref/1D.png}}

}

\caption{1D Flow Matching：噪声到数据的线性插值路径}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1883\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4227\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3889\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
求解设置怎么配，这些旋钮都还在------有些 Flow Matching
变体反而比一个标准 DDPM
实现更复杂。工程复杂度要按具体实现比，不按方法名比。
...
明白了机制，再看数字时就要注意一件事：\textbf{该比的量是
NFE（网络前向次数）和端到端时延，不是''步数''}------每步的代价并不相同。下表是各家论文与实现里常见的量级，属示例范围、不是通用结论：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3904\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2064\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4032\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
方法 & 常见步数量级 & 每步网络前向 \\
\midrule\noalign{}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1144\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4081\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4775\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
是沿连续向量场做积分。在常见设定下，后者往往能用更少的网络前向完成采样------但具体快多少，要在同一模型和硬件上实测。}
\end{quote}

一种常见的理解方式是：在合适的条件概率路径和目标向量场定义下，diffusion
可以被放进更一般的 flow-based
学习框架里来理解。也正因为这个统一视角，Flow Matching 常被看作比经典
diffusion 表述更一般的一类框架。
...
Policy 里的 \(T_p\) 与
\(T_o\)，只是换了字母。下面写的是一个教学版、最常见的条件化 Flow
Matching
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1460\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.5371\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3169\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
这条演进线最终汇入
VLA：连续动作生成不再只停留在单步回归，而是逐步走向更强的分布建模。Flow
Matching
是其中一种很自然的落点，因为它既保留了生成式动作建模的表达力，又更容易把采样过程写成少步数的
ODE 积分。以 \(\pi_0\) 为代表的一类系统，就把这种思路和视觉-语言
backbone 结合在了一起------\(\pi_0\)
的具体架构和训练细节将在第11讲展开。
...
看这三个方法，最好别按''第一代、第二代、第三代''来记------它们不是一条互相替代的世代链，而是在\textbf{两个正交的维度}上各做了选择：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1562\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.6250\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
维度 & 可选项 \\
\midrule\noalign{}
时序结构 & 单步动作 / 动作块（chunk）/ 滚动重规划 \\
分布建模 & 确定性回归 / CVAE / diffusion / flow \\
\bottomrule\noalign{}
\end{tabular}
\end{center}

ACT 选的是「chunk × CVAE」，还额外带上了 Transformer 架构与 temporal
ensemble；Diffusion Policy 是「chunk + 滚动重规划 × diffusion」；Flow
Matching 作为动作头是「chunk ×
flow」。三者的网络结构和训练目标也可以互换搭配，所以实际优劣要看任务、时延预算和数据，而不是看它''属于第几代''。

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1201\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2985\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2887\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2928\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
\begin{quote}
一句话总结：模仿学习从''看图回归一个动作''走向''看图生成一段动作分布''------ACT
用 CVAE 建模多模态、用 action chunking 做短时规划，Diffusion
...
这条线最终汇入
VLA：连续动作生成不再停留在单步回归，而是逐步走向更强的分布建模。下一讲（第11讲）就从
OpenVLA 讲起，看 VLA
如何把图像、语言、机器人状态统一进一个大模型，并把本讲的 flow matching
作为 \(\pi_0\) 家族的动作头------动作怎么被''流''出来。
...
{[}2{]} CHI C, XU Z, FENG S, et al.~Diffusion policy: visuomotor policy
...
{[}4{]} CAPUANO F, PASCAL C, ZOUITINE A, et al.~Robot learning: a
tutorial{[}EB/OL{]}. (2025-10-14){[}2026-06-26{]}.
https://arxiv.org/abs/2510.12403.

{[}5{]} LIPMAN Y, CHEN R T Q, BEN-HAMU H, et al.~Flow matching for
generative modeling{[}EB/OL{]}. (2022-10-06){[}2026-06-26{]}.
https://arxiv.org/abs/2210.02747.

{[}6{]} BLACK K, BROWN N, DRIESS D, et al.~\(\pi_0\): a
vision-language-action flow model for general robot control{[}EB/OL{]}.
(2024-10-31){[}2026-06-26{]}. https://arxiv.org/abs/2410.24164.

\begin{quote}
本讲的 AE→VAE→CVAE 演进图、ACT 训练流程示意图、Flow Matching
二维棋盘格与一维线性插值可视化图为本课程教学自制（本课程实验 EAI-wri-003
渲染），不引外部来源。
\end{quote}
