...
> 备注：别把这条 KL 和上一讲 PPO 里的 KL 混了。第14讲 7.3 节那版 PPO 也量新旧策略的 KL，但那只是用来**自适应调学习率**的旁路，真正约束单步步长的是 clip；GRPO 新增的这条 KL 则全程锚定在同一个冻结的参考模型上，防的是“训着训着整个人变了”。一个盯“这一步迈太大”（clip）、一个盯“整场训练跑偏”（KL 锚）——两件事各管各的。这条 KL 锚该不该留，第 6.3.5 节还会再算一次账。
...
![多模态 GRPO 训练管线：策略对同一输入采样多条带推理过程的回答（图中上下两例分别演示检测的 IoU 奖励与分类的正确性奖励，用的是两个不同的问题），可验证奖励逐条打分，组内比较后更新策略](../../assets/figures/lecture15/ref/fig153-visual-rft-framework_arxiv-2503.01785.png)
...
![视觉指令微调与视觉强化微调的对比：(a) 指令微调依赖大量标注数据教模型模仿；(b) 强化微调只需少量样本，用可验证奖励驱动模型自己试错变好；下方 (c) 列出 Visual-RFT 的三类应用——检测、指代定位、细粒度分类，各配一个带推理过程的样例](../../assets/figures/lecture15/ref/fig153-visual-rft-overview_arxiv-2503.01785.png)
...
![VLM 数数后训练前后对照：数数准确率从 0.095 提升到 0.44，答案格式合规率从 0.22 提升到 0.79（SuperCLEVR 固定 200 题；数据来源是 `code/rl/2_grpo_posttraining/result/2_1_grpo_vlm_counting/base_vs_adapter_summary.json`）](../../assets/figures/lecture15/ref/fig154-vlm-counting-before-after.png){width=75%}
...
把它和第14讲 7.2 节的 clip 目标并排放，会发现只有一处不同：优势换成了组相对优势。少掉的那个价值网络，连带它的梯度与优化器状态一起省了；而组内比较算出来的优势，天然适合结果奖励（成功/失败）这种只有 0 和 1 的场景。
...
![动态采样的消融实验（LIBERO-Long）：开启动态采样（红）的平台比关闭（蓝）高出图上标注的 15 个百分点（那 15 是两条水平参考虚线之间的差；在横轴终点 300 步处两条曲线实测差约 18 个百分点），且绝大部分区间爬升更快](../../assets/figures/lecture15/ref/fig-simplevla-ablation_arxiv-2509.09674.png){width=50%}
...
实验设置：每个任务套件的 10 个任务中，9 个用于训练，1 个保留为未见任务进行分布外评估。**这样的 9/1 划分每个套件做了三组**，各留一个不同的任务当未见任务，所以论文的图 4 每个套件是三张子图（Unseen Task 1/2/3），下面的结论是在三组划分上都成立的，不是单个样本的观察。

![泛化分析示例，Goal 套件三组划分里的一组，未见任务是“把酒瓶放到柜子顶上”：横轴是已见训练任务的成功率（即上面说的那 9 个，图上只标 Seen Tasks SR，任务数出自论文的实验设置）、纵轴是这个未见任务的成功率。红线（RL）全程在 70% 上下起伏、中途最高到 83%，始终没有塌陷；蓝线（SFT）在训练任务刚过 65% 时就一头栽到 0，此后再没起来](../../assets/figures/lecture15/ref/fig-simplevla-unseen-goal_arxiv-2509.09674.png){width=50%}
...
- **LIBERO-Goal 最具挑战性**：SFT 在训练开始时就立即在三个未见任务上全部降至 0%，因为 Goal 任务涉及多样化的物体和操作策略，跨任务可迁移的成分很少；而 RL 不仅避免了性能下降，还实现了 5%-15% 的提升
...
![Pushcut 现象（RoboTwin 2.0 的 Place A2B Right 任务）：上排 SFT 模型照示教先把方块抓起来再搬（图中蓝色 Grasp 箭头），下排 RL 训练后的模型直接把方块推向目标位（红色 Push 箭头），两排各取动作前后两帧；两排取自该任务下不同的初始布置，桌面与周边物件因此并不相同](../../assets/figures/lecture15/ref/fig-simplevla-pushcut_arxiv-2509.09674.png){width=50%}
...
![RLinf-VLA 总览，自上而下三层：统一接口层把仿真器、VLA 模型、RL 算法三个维度都做成可插拔的；中间一层并排比较三种 GPU 分配编排，右半的吞吐柱状图给出混合模式相对基线 2.27× 的提速；最下一层是三个基准上的结果面板（LIBERO 与 RoboTwin 是成功率条形图，中间的 ManiSkill 是 GRPO 与 PPO 的训练曲线）](../../assets/figures/lecture15/ref/fig-rlinf-overview_arxiv-2510.06710.png)
...
图上那个大大的 **2.27×** 是混合模式相对 SimpleVLA-RL 那套实现的吞吐加速上限（OpenVLA-OFT 在 LIBERO 与 RoboTwin 这两个 CPU 并行仿真器上，随 GPU 数变化，最低一档是 1.34 倍）；论文摘要另给了一个口径：混合式细粒度流水线相对**分置模式**提速 1.61 到 1.88 倍——这个数的出处是 §5.2 的 ManiSkill + OpenVLA，8 卡时 1.88 倍，扩到更多卡后仍保持 1.61 到 1.69 倍。**两个数的分母不同**：2.27× 比的是另一套框架，1.88× 比的是本框架自己的分置编排。底层的性能面板给出算法侧的结果：同一套系统训练出的**单一统一模型**，在 130 个 LIBERO 任务上取得 98.11% 成功率、25 个 ManiSkill 任务上 97.66%、六个 RoboTwin 任务平均 84.63%。面板里每根柱子的实心段是 SFT 起点、斜线段是 RL 补上的增量，不同任务的增量宽窄差得很远——这本身就是 5.3 节那条规律的又一次印证：起点越低，RL 能补的越多。
...
![RECAP 总览：左侧三类进料（机器人数据、子任务指令、多模态网络数据）分别汇入上方的 $\pi^{*}_{0.6}$ 策略与下方的价值函数；中间的 Recap 圆环把两者接成闭环——策略部署出去跑 rollout，人在旁边接管纠偏并打标，数据回流去更新价值函数，价值函数给出的 advantage 再回来条件化训练策略；右侧是它要做的三件真事：组装箱子、做浓缩咖啡、叠各式衣物](../../assets/figures/lecture15/ref/fig-pi06-teaser_arxiv-2511.14759.png)
...
**价值函数架构**。价值函数沿用与 VLA 相同的架构设计，但换了个小得多的骨干：VLA 那边的基座是 Gemma 3 4B，价值函数这边是一个约 670M 参数、同样从 Gemma 3 初始化的 VLM。它接收与 VLA 相同的输入（图像、语言指令），输出价值分布。（论文正文只给到“670M 的 VLM 骨干”这一层；下面那张架构图上另标了两边各自的构成——策略是 SigLIP 400M 配 Gemma 4B，价值函数是 SigLIP 400M 配 Gemma 270M，两项相加正好约 670M。图比正文细，引用时按各自的出处读。）为防止过拟合，价值函数也会在少量多模态网络数据上联合训练。
...
> **关于那个 30%**：论文自己的两处写法并不一致——正文 §5.4 说的是把 $\epsilon_\ell$ 取成价值函数对该任务预测值的**30% 分位**，附录 F 说的是把阈值选到**约 30% 的示教数据 advantage 为正**。两种取法在分布上并不等价。上表取的是附录那个口径，因为它更贴近这里要表达的意思：$\epsilon_\ell$ 控制的是有多少数据被标成 positive。表里 40% 与 10% 两个数也都出自附录 F。另外，10% 那一档论文给的理由是这个任务的高质量示教会训出成功率高但动作慢的策略，所以要把阈值调高、只留最好的一成——不是因为它难。
...
论文在 T 恤与短裤折叠这个任务上，把 advantage conditioning 与 AWR、PPO 摆在一起比。（这是三个叠衣变体里较容易的一个——最难的是下一节要说的“多样叠衣”，那才是唯一没能进 90% 区间的任务。这组对比挑的是容易的那个，所以它比的是三种方法谁把同一个任务做得更好、更快，不是谁扛得住最难的场面。）结论不必记完整图表：advantage conditioning 最稳——它把 positive 和 negative 数据一起用上，既提成功率、又不牺牲执行速度，吞吐量因此最高。AWR 输在速度上：它会过滤或降权大量低 advantage 的数据，学出来的策略偏保守，吞吐量跟不上。PPO 则受限于 8.4.1 节说过的那个矛盾——为了在 off-policy 数据上稳住，信赖域必须收得很小，而信赖域一小，改进幅度也就锁死了。
...
![实验任务与各自的成功判据：五列依次是叠 T 恤短裤、多样叠衣、叠衣消融（判据多一条“领口朝上”）、组装箱子、制作浓缩咖啡；每列上两行是该任务过程中的两帧画面（第三列取的是摊开与叠好的前后对照，其余几列两帧同属一次执行、不必读成严格的开始与完成），最下一行的色块逐条列出“算成功”要满足什么](../../assets/figures/lecture15/ref/fig-pi06-tasks_arxiv-2511.14759.png){width=100%}
...
![四个任务上的吞吐量对比（纵轴为每小时成功完成数，柱子带误差棒）：四张子图依次是叠 T 恤/短裤、最难的多样叠衣服、制作浓缩咖啡、组装箱子。每组柱子从左到右按训练配方由弱到强排列——$\pi_{0.6}$ 预训练、$\pi^{*}_{0.6}$ 离线 RL 预训练、$\pi^{*}_{0.6}$ 离线 RL + SFT，最右的黄柱是加了真机经验的完整 RECAP；配方变强不等于每一档都往上走，多样叠衣与组装箱子这两张子图的中间档位都有回落。前两组另在最左多一根 $\pi_{0.5}$ 预训练作为更早一代的参照，其中多样叠衣那组的 $\pi_{0.5}$ 柱高基本为零](../../assets/figures/lecture15/ref/fig-pi06-throughput_arxiv-2511.14759.png){width=98%}
...
Advantage conditioning 并非 RECAP 首创。把 RL 问题改写成“以某个期望回报为条件的序列建模”，这条思路早有前身：Decision Transformer[18] 以 return-to-go 为条件，Upside-Down RL[19] 直接把期望回报当输入条件训练策略。RECAP 自己在论文里把 advantage conditioning 这个做法本身归给了 Kumar 等人的 Reward-Conditioned Policies[20]，说自己用的是它的一个变体；而具体公式形式上最接近的是 CFGRL[16]，它把 classifier-free guidance 引进 RL——那是 RECAP 最直接的理论基础。
...
[20] KUMAR A, PENG X B, LEVINE S. Reward-conditioned policies[EB/OL]. (2019-12-31)[2026-08-29]. https://arxiv.org/abs/1912.13465.
