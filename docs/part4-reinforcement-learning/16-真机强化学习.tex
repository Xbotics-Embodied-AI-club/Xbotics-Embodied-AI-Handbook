...
万步也没稳住，滑动均值前 8 万步一直压在 100 分以下，之后在 100～350
分之间大幅起落、收在 193 分；v2 DQN 约 3 万步就打出第一个 500
分满分回合，5 万步收工时末 50 回合里已有 31 个满分。原始记录见
...
仿真（\texttt{code/platform/so101\_sim/}）。这里先记住一个贯穿全讲的事实：\textbf{强化学习训练用的环境，和
VLA
...
\begin{quote}
关于复现：下面四级的曲线与成功率，都是在\textbf{单相机版的 ReachCube
场景}上跑出来的，那个场景后来随 \texttt{so101\_sim}
重构下线了。四个训练脚本（\texttt{code/rl/3\_offpolicy/3\_2\_so101\_offpolicy/train\_v3\_ddpg.py}、\texttt{code/rl/3\_offpolicy/3\_2\_so101\_offpolicy/train\_v4\_td3.py}、\texttt{code/rl/3\_offpolicy/3\_2\_so101\_offpolicy/train\_v5\_sac.py}、\texttt{code/rl/3\_offpolicy/3\_2\_so101\_offpolicy/train\_v6\_squint.py}）里的任务名现在指向仓库里还在的双相机抓放场景，\textbf{照着跑能跑通整条代码路径，但复现不出下面这组数值}------任务换了，奖励尺度和难度都不一样。要对着看的是''每一级改了什么、曲线形状怎么变''，不是这几个具体数字。模块
README 里也记着这条。
\end{quote}
...
SO-101
仿真场景，本身就是从它的开源任务集分叉来的（\texttt{code/platform/so101\_sim/so101\_sim/tasks/UPSTREAM.md}
记着这条来路）。
...
\begin{quote}
注意：\textbf{这一级现在还跑不起来。} 文件里的 \texttt{CNNEncoder} 和
\texttt{ReplayBuffer} 是按单相机 3
通道图像写的，而仓库里现存的三个场景都出双相机（顶视 + 腕视）6
通道观测，直接跑会因通道数不匹配报错。要跑通得先把编码器和回放池改成吃 6
通道------这本身是一道不错的练手题，前五级都是可以直接跑的。
\end{quote}
...
里，前三个文件各管一步，第四个把三步串起来：

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.6673\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3327\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
文件 & 这一步干什么 \\
\midrule\noalign{}
\texttt{code/rl/3\_\allowbreak offpolicy/3\_\allowbreak 2\_\allowbreak so101\_\allowbreak offpolicy/datagen/rollout.py} & 加载训好的策略，跑出成功轨迹 \\
\texttt{code/rl/3\_\allowbreak offpolicy/3\_\allowbreak 2\_\allowbreak so101\_\allowbreak offpolicy/datagen/replay.py} & 把每条轨迹换个外观（黑臂、白底）重渲一遍 \\
\texttt{code/rl/3\_\allowbreak offpolicy/3\_\allowbreak 2\_\allowbreak so101\_\allowbreak offpolicy/datagen/to\_\allowbreak lerobot.py} & 转成 LeRobot 数据集格式 \\
\texttt{code/rl/3\_\allowbreak offpolicy/3\_\allowbreak 2\_\allowbreak so101\_\allowbreak offpolicy/datagen/gen\_\allowbreak dataset.py} & 一条命令把上面三步串起来 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
16×16
像素（贴近真机部署形态，又不让高分辨率拖垮回放池和编码器）。它从零学会
ReachCube 成功率 0.99，训好的策略再顺手 rollout + 换色生成 VLA
微调数据集，回收了第9讲 2.6 节、第12讲 3.2 节''数据是视觉 RL
生成''的伏笔，把这一讲和 VLA 课闭成一个''先用后造''的完整数据闭环。
