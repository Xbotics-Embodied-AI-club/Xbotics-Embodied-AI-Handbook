...
确实，但那是\textbf{小范围、短窗口}的复用（第14讲 7.3 节讲过）：PPO 的
ratio 和 clip
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1875\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3125\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.4688\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
\caption{一个终点奖励沿转移链回传成价值坡道：抽到丙记住成功，再抽到乙、甲时价值按
\(\gamma\) 折扣逐格向起点传播。\(\gamma=0.9\)，起点三个 \(Q\) 全为 0}
...
先看这个环境自己渲出来是什么样，三帧分别对应上面说的那两道终止线：

\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{../../assets/figures/lecture16/ref/fig16-cartpole-real.png}

}

\caption{真实渲出的 CartPole
三帧（环境自带渲染器，非示意图）：黑色方块是小车，浅色细杆铰在车顶，贯穿画面的横线是轨道。左：杆基本竖直，回合继续。中：杆偏到
11.5°，再歪半度就撞上 12° 那道线。右：小车滑到 2.2，再滑一点就出 ±2.4
的轨道范围}

\end{figure}%

画面里看不到那两道终止线------它们是判定用的数值，不是场景里的物体。下面这张示意图把它们和四个观测量一起标出来：
...
回合里已有 31 个满分。两次训练的原始回报记录都在
\texttt{code/rl/3\_offpolicy/result/} 下，这张图由它们重画而成}

\end{figure}%

\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0788\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2132\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1777\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1413\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1852\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2038\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
版本 & 方法 & Q 的表示 & 环境步数 & 末 50 回合平均回报 & 其中满分回合 \\
\midrule\noalign{}
v1 & 表格 Q-learning & 查表，状态手工分桶 & 20.1 万 & 193 & 0 / 50 \\
\textbf{v2} & \textbf{DQN} & \textbf{网络，吃原始状态} & \textbf{5.0 万} & \textbf{396} & \textbf{31 / 50} \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
ratio，即每采一步数据做几次梯度更新，治更新过猛）------下一节起会逐个登场，每一件都对着死亡三角的某一条边。它们不是装饰，本讲后面的方法（DDPG、TD3、SAC、HIL-SERL）都带着这样一串``稳定性配件''。
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.2288\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3730\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.3982\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
 & 第15讲 & 第16讲 \\
\midrule\noalign{}
场景 & GRPO 后训练 & 真机 RL \\
...
选动作，迈不进连续控制；而三者一凑齐，死亡三角带来的稳定性风险也一并到场。接下来一节把连续动作这道坎补上：DDPG、TD3、SAC
一级一级往上走------这也是为什么入门先学 on-policy，上真机才换
off-policy。

\section{3 连续动作的 off-policy：DDPG → TD3 →
SAC}\label{ux8fdeux7eedux52a8ux4f5cux7684-off-policyddpg-td3-sac}

前面 CartPole 上的 DQN 有一个天花板：它靠``对每个动作的 Q 值取
argmax''来选动作，这只在动作是\textbf{有限几个离散选项}时才做得到。机械臂的动作是六个关节各自转多少的连续量（每一步给每个关节一个目标角度增量），把连续区间切成格子再
argmax，格子数随维度指数爆炸------和上一节表格 Q
撞的是同一堵墙。要在连续动作空间里做
off-policy，得换一个思路：不再``评分选最高''，而是\textbf{直接让一个网络吐出动作}。

这一节把连续动作的 off-policy 一次讲完：DDPG 打地基，TD3
补它高估的毛病，SAC 换掉它太弱的探索，最后再看一个把 Critic
从``一个数''升级成``一条分布''的做法。\textbf{每一级只补上一级的一个短板}，这个递进关系本身就是要记住的东西。

\begin{quote}
这一节只讲机制，不带实验曲线。原先这里挂着一组
SO-101``够到方块''的四级对照实验，可那个单相机场景随仿真器重构下线了，图里的数既无法复现也无法逐点核对，所以整组撤下------留一张核不了的截图，不如先不留。等仿真器上的任务重建完，会带着可复现的原始记录重新补回来。\textbf{这一讲能动手跑、且有记录可核对的实验在上一节（CartPole）和下一节（G1
行走）。}
\end{quote}

\subsection{3.1 DDPG：Actor 出动作，Critic
打分}\label{ddpgactor-ux51faux52a8ux4f5ccritic-ux6253ux5206}
...
靠在输出动作上\textbf{外加一点高斯噪声}来试探周围。记住这个``外加噪声''------它后面会成为这条阶梯的命门。
...
此刻给出的下一动作
\(\mu'(s')\)------两个网络互为对方的老师。下面这个毛病，根子就在这个闭环上。

DDPG 的毛病是它只有\textbf{一个} Critic。确定性策略梯度让 Actor
...
\begin{quote}
备注：拿原始观测直接喂这个 Actor
有一个很容易撞上的坑。观测里某些分量的数值可能大到几十上百（位姿读数一类），逐层放大到末端
\texttt{tanh} 时早已\textbf{深度饱和}，而 \texttt{tanh}
在饱和区梯度几乎为零------梯度在这里被掐断，Actor
就冻在初始值上，表现是``平均奖励一条死平线、参数一位小数都不动''。修法是标准动作：\textbf{观测归一化}，在线统计每一维的均值方差，归到零均值单位方差再喂网络。第14讲那套
G1 代码里就带着它------第14讲 4.5 节的代码地图指出的那个共用
\texttt{model.py}，三个版本的归一化都在里面。这条对\textbf{任何}拿原始观测喂
MLP 的连续控制都成立；有些实现改用 \texttt{LayerNorm}
把归一化直接长进网络结构里，效果一样。诊断这类``怎么训都不涨''的顺序值得记住：先排除训练预算，再定位是哪个网络没在动，再顺着零梯度找数值根因。
\end{quote}

\subsection{3.2 TD3：用两个 Critic
...
\item
  ...
  Critic
  可能虚高，但两个网络在同一个动作上\textbf{同时}虚高的概率低得多，取
...
但三个补丁动的全是 \textbf{Critic 那一侧}。TD3 的 Actor
依然是\textbf{确定性}的，探索依然靠\textbf{外加的那点高斯噪声}------确定性策略加一点小噪声，在六维动作空间里翻来覆去就那么大范围。\textbf{双
Critic 不是万能药，它只解决高估这一类问题；探索的天花板还在那儿。} 那是
SAC 的地盘。

\subsection{3.3
SAC：把探索变成一条原则}\label{sacux628aux63a2ux7d22ux53d8ux6210ux4e00ux6761ux539fux5219}

DDPG 和 TD3 卡在同一个地方------确定性策略 +
外加噪声，探索太弱。SAC（Soft
Actor-Critic，柔性演员-评论员）{[}6{]}从根上换掉这套探索：它让策略重新变成\textbf{随机}的，并且把``保持随机、多探索''写成训练目标的一部分。

普通强化学习的目标只有一条：回报最大化。SAC
在它上面加了一项------\textbf{策略熵}最大化。熵衡量策略的随机程度：熵高，策略在多个动作间保持犹豫、乐于尝试；熵低，策略趋于孤注一掷。SAC
的目标变成``\textbf{在拿高回报的同时，尽量保持随机}''。这一项的作用正是治探索：它不让策略过早锁死到某一套确定动作上，而是逼着它持续在动作空间里铺开试探。DDPG/TD3
是``选定一个动作 + 抖一抖''，SAC
...
\item
  \textbf{最大熵目标}：回报项里减去一个``熵的代价''，等价于奖励策略保持随机。
\item
  \textbf{自动温度
...
自动温度''探索机制。前两级在 Critic 那侧修修补补，SAC
终于动了\textbf{策略}这一侧。

这条阶梯想让你带走的判断是：\textbf{在需要精细试探才能碰到成功的任务上，卡住确定性策略的往往不是高估、也不是网络容量，而是探索本身}。把距离一点点磨小和真正跨过``做成''那道坎，是两件事------前者靠
Critic 修得更准，后者要靠探索方式的质变。这个判断在下一节 G1
行走的实验里会以另一种形式再出现一次；专门支撑它的那组操作任务对照实验，等仿真器重建后补。

\subsection{3.4 再往前一步：把 Critic
从一个数换成一条分布}\label{ux518dux5f80ux524dux4e00ux6b65ux628a-critic-ux4eceux4e00ux4e2aux6570ux6362ux6210ux4e00ux6761ux5206ux5e03}

上面三级的 Critic 都是估\textbf{一个}标量 Q
值。还有一路做法换了个思路{[}7{]}：不估``预期回报是多少''，而是估``回报\textbf{落在各个取值上的概率分布}长什么样''------把可能的回报切成一排固定的档位，网络输出的是``落在每一档的概率''，训练时用分类交叉熵去拟合，而不是对一个数做均方误差。这类
Critic 一般叫\textbf{分布式 Critic}，档位数是个超参。

直觉上，回报本身是有不确定性的------同一个状态动作，运气好回报高、运气差回报低。硬压成一个平均数会丢掉这层结构；让网络直接去学整条分布，每一步更新提供的训练信号更丰富，训练往往也更稳。它不是这条阶梯的必修课，但值得认识：下一节那套把
off-policy 搬进大规模并行仿真的配方，论文里就带着分布式 Critic（4.3
节会说到，我们的课堂版刻意没上它）。

\subsection{3.5 本节小结}\label{ux672cux8282ux5c0fux7ed3-2}

这一节把连续动作的 off-policy 讲完了。DDPG 用一个确定性 Actor
直接吐连续动作、一个 Critic 打分，靠确定性策略梯度让 Actor 沿 Critic
的梯度往上爬，把 off-policy 从离散动作推进到了连续控制；代价是单个
Critic 会系统性高估，Actor 追着虚高的分反复横跳。TD3 打三个补丁------双
Critic 取 min、延迟策略更新、目标策略平滑------治住高估，但动的全是
Critic 那一侧，探索仍是``确定性动作加噪声''。SAC 换掉探索本身：随机策略
+ 熵最大化 +
自动温度，逼着策略持续铺开试探，这才动到策略那一侧。再往前还有一路把标量
Critic 换成分布式 Critic 的做法，下一节的配方里会用到。这几件零件（双 Q
取小、目标网络软更新、随机抽样、更新采样比）就是 2.5
节死亡三角那笔账要付的工程开销，下一节起会在真实训练里逐个现形。

\section{4 同一个 G1，换 off-policy 再走一遍：15
分钟学会走路}\label{ux540cux4e00ux4e2a-g1ux6362-off-policy-ux518dux8d70ux4e00ux904d15-ux5206ux949fux5b66ux4f1aux8d70ux8def}

连续动作的 off-policy 讲完了。现在把镜头拉回第14讲的老地方 G1
行走，看看业界怎么把同一套 off-policy
规模化：它真能在大规模并行仿真里顶替 on-policy 吗？2025
年恰好有一对工作把这件事做成了标杆。

\subsection{4.1 FastTD3 与 FastSAC：把 off-policy
...
4096 环境 + PPO）一直是 on-policy 的天下。\textbf{FastTD3}{[}8{]}
...
行走和全身动作跟踪上的回报-墙钟时间曲线（横轴是墙钟时间，纵轴是任务回报），上排为
sim-to-real 部署到真机的画面}
...
那张上它只到两条彩线的四成出头，T1
那张上追到了七成，差距没那么悬殊。右图是更难的全身动作跟踪（和第14讲 8.2
...
\subsection{4.3 动手实验：给 G1
行走加第四个版本}\label{ux52a8ux624bux5b9eux9a8cux7ed9-g1-ux884cux8d70ux52a0ux7b2cux56dbux4e2aux7248ux672c}

第14讲的 G1
行走模块里躺着三个训练脚本：\texttt{code/rl/1\_rl\_basics/1\_1\_g1\_walk\_rl/train\_v1\_reinforce.py}、\texttt{code/rl/1\_rl\_basics/1\_1\_g1\_walk\_rl/train\_v2\_a2c.py}、\texttt{code/rl/1\_rl\_basics/1\_1\_g1\_walk\_rl/train\_v3\_ppo.py}------同一个任务，算法由简到繁。这里再补一个动手实验：给第14讲这条
G1 阶梯加\textbf{第四级}------off-policy 的 SAC
...
reward 只有 0.026，是 v3 PPO 峰值（0.098）的四分之一出头。
...
在速度指令下持续行走、全程保持直立。四格取自 G1 行走模块 result 目录下的
rollout 视频 \texttt{g1-walk-sac.mp4}}
...
3000 次迭代，SAC 用了约 15 分钟，PPO 约 103 分钟。
...
万步、0.026），此后总体下行、中途数次回升；PPO 的第一个存档点要到 2000
万步，此后一路爬到 0.098 的平台。数据来源是 G1 行走模块 result 目录下的
\texttt{compare.json}}
...
\caption{按墙钟时间对齐的 reward
曲线，横轴换成训练用掉的分钟数，两条线仍是 SAC 与 PPO：跑满 3000
次迭代，SAC 约 15 分钟、PPO 约 103 分钟；SAC 那条从第一个存档点（约 1.3
分钟）起就是全程最高，剩下的十几分钟一路往下走}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.0661\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.0661\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1599\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1174\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1599\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1599\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.1755\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.0952\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
版本 & 算法 & 类别 & 最好的一档 reward & 该档摔倒率 & 该档交互量 & 该档墙钟 & 跑满 3000 迭代总墙钟 \\
\midrule\noalign{}
v3 & PPO & on-policy & 0.098 & 0\% & 2.16 亿步 & 约 76 分钟 & 约 103 分钟 \\
...
同样要说清楚的是，\textbf{这个 15 分钟和 4.2 节论文那个 15
...
\item
  ...
  放大十倍之后，任务信号才压得过熵。回头看 3.3 节的目标函数：reward
  和熵是相加的两项，谁的量级大谁说了算。
\item
  \textbf{温度失守}。自动温度（3.3 节的第四个零件）会自己往下调
...
工作 & 年份 & 在前一步上补了什么 \\
\midrule\noalign{}
SAC{[}6{]} & 2018 & 连续动作 off-policy 的算法底座（第 3 节） \\
RLPD{[}12{]} & 2023 & \textbf{先验数据进池子}：每个 batch 一半采自在线经验、一半采自离线示教；配上高 UTD 和 LayerNorm 稳住 critic \\
SERL{[}13{]} & 2024 & \textbf{软件套件}：actor 与 learner 分进程解耦、阻抗控制器保护硬件、免复位训练支持 \\
HIL-SERL{[}10{]} & 2024 & \textbf{人在环}：训练中人工干预 + 视觉奖励分类器，通吃接触型任务 \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
这与模仿学习里的 DAgger 家族形成一组重要对照：HG-DAgger{[}14{]}
同样让人纠偏，但把纠偏数据拿去做\textbf{监督学习}------策略的上限被钉死在``学得像示范者''；HIL-SERL
把同样的数据交给\textbf{强化学习}------干预只是引路，策略最终按任务奖励自我优化，可以\textbf{超过}示范者（5.8
节与 HG-DAgger
的对照曲线会印证这一点）。这是对第三道坎（探索闯祸）的回答：人挡住最坏情况，策略保留自我提升空间。这一件套的操作接口就是
5.3 节说的 leader 臂 + space 键，具体命令并进下一节的在线训练里一起给。
...
\item
  \textbf{干预率一路降到
  0}------人从``全程陪练''逐渐退成``旁观者''，这是系统在自我提升的最直接体征。要注意降的过程并不平顺（5.10
...
万步以内就到顶。图例三条都写作 \texttt{default}，哪条对应哪次实验依
LeRobot 官方文档的说明判定}

\end{figure}%

橙色那条（无干预）在头一万八千步里贴着 0
一动不动，之后才开始爬，到三万步上下才追平；有人干预的两条（粉、绿）不到一万步就已接近满分、曲线在那里就结束了------\textbf{有干预的两次都在一万步内到顶，无干预那次要到三万步上下}。这一组每种条件各只跑了一次，量级参考即可，不宜再往下乘出倍数。方向是清楚的：干预不是锦上添花，而是真机
RL 能在小时级完成的关键。
...
步附近一度归零；随后有一段回升到 0.3 上下、最高约
0.4，并持续约一千步，4.7k 步后才稳定归零（截图自 LeRobot 官方文档）}

\end{figure}%

这是``健康训练''的模样：\textbf{总体趋势向下}。注意它并不单调------3.3k
步附近先归零，接着又回升到 0.3 上下、最高约
0.4，拉锯了近一千步，最后才真正稳定在
...
HIL-SERL 论文与 LeRobot 官方文档。你在自己那台 SO-101 上按 5.3 到 5.7
走完一遍，拿到的训练前后成功率对照、干预率曲线和 rollout
视频，就是属于你自己的那份证据链。

\subsection{5.11 本节小结}\label{ux672cux8282ux5c0fux7ed3-4}
...
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.1951\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2846\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2429\linewidth - 2\tabcolsep\relax} >{\raggedright\arraybackslash}p{\dimexpr 0.2775\linewidth - 2\tabcolsep\relax}@{}}
\toprule\noalign{}
 & 第14讲 & 第15讲 & 第16讲 \\
\midrule\noalign{}
...
本讲演示阶梯 & REINFORCE→A2C→PPO & 单一 GRPO & 表格Q→DQN（CartPole）＋ SAC（G1 第四版） \\
\bottomrule\noalign{}
\end{tabular}
\end{center}
...
\subsection{7.1 主线回顾}\label{ux4e3bux7ebfux56deux987e}
...
DQN。随后把 off-policy 推进到连续动作：DDPG 用确定性 Actor
直接吐动作，代价是单 Critic 高估；TD3 用双 Critic
取小治住高估，却仍卡在探索；SAC
把最大熵写进训练目标，才动到策略那一侧。再往前还有一路把标量 Critic
换成分布式的做法。接着回到 G1 行走验证规模化：FastSAC/FastTD3 单卡 15
分钟练成能上真机的行走。最后 HIL-SERL 把 off-policy
带上真机------示教打底、视觉分类器发奖、人在环纠偏、自动复位撑连续训练，1
到 2.5 小时把接触型任务练到接近 100\%，落到 SO-101 上就是一整套 LeRobot
工作流，干预率下降就是训练健康的体征。

\subsection{7.2 方法演进链}\label{ux65b9ux6cd5ux6f14ux8fdbux94fe}

这一讲织了两条演进链。一条是\textbf{算法阶梯}，每一级只补上一级的一个短板：

\[\text{表格 Q} \;\to\; \text{DQN} \;\to\; \text{DDPG} \;\to\; \text{TD3} \;\to\; \text{SAC}\]

分桶到函数近似、离散到连续、单 Critic 到双
Critic、外加噪声到最大熵------一步一个知识点。另一条是把 SAC
带上真机的\textbf{工程血缘链}：
...
{[}7{]} BELLEMARE M G, DABNEY W, MUNOS R. A distributional perspective
on reinforcement learning{[}EB/OL{]}. (2017-07-21).
https://arxiv.org/abs/1707.06887.

{[}8{]} SEO Y, SFERRAZZA C, GENG H, et al.~FastTD3: Simple, fast, and
capable reinforcement learning for humanoid control{[}EB/OL{]}.
(2025-05-28). https://arxiv.org/abs/2505.22642.

{[}9{]} SEO Y, SFERRAZZA C, CHEN J, et al.~Learning sim-to-real humanoid
locomotion in 15 minutes{[}EB/OL{]}. (2025-12-01).
https://arxiv.org/abs/2512.01996.

{[}10{]} LUO J, XU C, WU J, et al.~Precise and dexterous robotic
