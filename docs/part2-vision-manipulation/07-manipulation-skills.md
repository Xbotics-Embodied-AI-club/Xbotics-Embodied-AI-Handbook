...
第 6 讲已经完成手眼标定，并通过坐标变换得到目标物体在机器人基座坐标系中的位姿。机器人已经能够回答“物体在哪里”，接下来似乎只要让机械臂去抓就可以了。

但真正执行时，问题才刚刚开始。机械臂按计划到达 A 区，夹爪闭合、抬升、移动，最后在 B 区打开。控制台没有报错，整段轨迹也执行完了。可回头一看，方块还留在 A 区——夹爪从一开始就是空的。

这是机器人操作里很典型的一类失败：**动作执行完了，任务却没有完成。** 本讲就在第 6 讲的位姿结果上继续向前：把物体位姿转换成夹爪动作，决定怎样接近、什么时候闭合、如何确认物体已经离桌，以及失败后该改什么。

我们用两个难度递增的任务贯穿全讲：先把红色方块从 A 区搬到 B 区，跑通最小闭环；再从侧面抓起竖直瓶子并放入收纳盒，处理滑落、盒沿碰撞和姿态约束。
...
物体位姿是一个空间坐标，抓取却是一段与环境发生接触的过程。把方块中心直接发送给机械臂，夹爪可能从错误方向撞上去，也可能在尚未对准时闭合。即使末端顺利到位，程序仍不知道两指之间有没有物体。

一套可用的操作流程至少要做四件事：生成一组有安全间隔的动作位姿；按顺序执行；在抓取、抬升和放置后检查物理结果；失败时停止原流程并选择恢复动作。
...
方块形状规则、重心稳定，适合从正上方抓取。瓶子沿用同一套流程，却把接近方向改成水平侧向，并把放置目标从平面区域换成有边界的收纳盒。程序骨架可以不变，但抓取姿态、夹爪宽度、放置高度和成功条件必须随任务变化。
...
| 夹持位置 | 方块中心附近 | 瓶身中部 |
| 放置目标 | 桌面 B 区 | 收纳盒内部 |
| 主要难点 | 抓偏、抓空、放置越界 | 下滑、碰盒沿、释放后倾倒 |

## 1.2 为什么要把任务拆成几个状态

如果把抓取、抬升和搬运命令一次性发完，方块没有夹住，机械臂也会照样前往 B 区。状态机在动作之间插入检查点：预抓取状态只判断是否到位，夹爪状态只负责闭合，抬升验证状态再判断物体有没有随夹爪离桌。检查不通过，后续搬运便不会发生。
...
```math
\mathbf{p}_{lift}=\mathbf{p}_{g}+h_{lift}\mathbf{e}_z
```
...
![方块任务的关键动作位姿](../../assets/figures/lecture07/fig-07-3-cube-key-poses.png)

**图 7-3 方块任务的关键动作位姿**

![瓶子任务的关键动作位姿](../../assets/figures/lecture07/fig-07-4-bottle-side-grasp.png)
...
## 3.4 接触以后，每一步都要等物理世界给出回答

从夹爪碰到物体开始，程序不能再只看控制器是否返回成功。正确的顺序是：闭合夹爪，短距离抬升，确认物体跟随；搬到目标上方，低速下降，确认物体获得支撑；最后打开夹爪，等待物体稳定，再退出观察区域。

这几个停顿看似拖慢了流程，实际是在降低返工成本。空抓若在抬升阶段就被发现，机器人不必带着空夹爪走完整段搬运轨迹；瓶子若在盒沿处受到阻挡，也不应继续下降或强行释放。具体采用哪些信号判断，将在第 4 节展开。

## 3.5 执行前校验：看起来合理，还要真的能到
...
只保存 `success=False`，对调试几乎没有帮助。看一段典型的失败时间线：夹爪闭合后保留了合理开口，初步检测认为可能夹到物体；末端随后抬高，但视觉中的物体高度没有变化。最早出现矛盾的是抬升验证，因此这次失败应先检查抓取点和夹爪接触，而不是去调 B 区放置位置。

日志至少要把这条推理链还原出来：五个动作位姿、状态时间戳、末端与夹爪反馈、物体观测、阶段检测结果，以及第一次失败时的代码和关键帧。任务编号和统一时间基准把这些信息对齐，失败分析才不会停留在猜测上。
...
实验按三层推进：先用 Mock 检查状态流和日志，再用 ManiSkill 检查控制与碰撞，最后才接入 SO-101。每切换一层，只替换机器人后端，不改任务配置、检测逻辑和状态机。这样出现问题时，可以判断错误来自上层流程，还是来自仿真或硬件接口。
...
这里有一条清晰的数据流：`TaskConfig` 描述“要完成什么任务”，`ActionTargets` 给出“五步分别去哪里”，`Observation` 返回“机器人和物体现在怎样”，`MotionResult` 则说明“一条运动命令是否执行成功”。状态机只依赖这些统一对象，因此不需要知道底层连接的是 Mock、ManiSkill 还是真机。

配置文件完整内容见 [`config.py`](../../code/lecture07/robot_pick_place/config.py)。`TaskConfig` 字段很多，对照两个任务时先看接近方向、抓取偏移、预抓取距离和夹爪宽度：
...
动作位姿生成函数先将接近方向归一化，再根据目标物体位姿、抓取偏移和抓取姿态得到抓取位姿。预抓取位姿沿接近方向反向偏移；抬升位姿在抓取位姿基础上提高；退出位姿位于目标区域上方。

代码将 place_pose 解释为物体期望到达的位置，因此生成机械臂放置位姿时仍需保留抓取偏移。这样，方块中心最终落在 B 区目标点，瓶身中心最终落在收纳盒内部，而不是把末端参考点误当成物体中心。

完整实现见 [`pose_generator.py`](../../code/lecture07/robot_pick_place/pose_generator.py)。核心不是辅助函数 `_normalize()`，而是五个位姿如何由两个参考位姿派生：
...
| **运行前检查** 打印五个位姿并在 RViz、Open3D 或仿真窗口中可视化。若抓取位姿方向、预抓取偏移或放置高度明显错误，应在动作执行前修正。 |
|-------------------------------------------------------------------------------------------------------------------------------------|
...
恢复函数根据失败代码修改下一次任务参数。运动或碰撞失败时增加预抓取距离并降低接近速度；抓取或抬升失败时微调抓取偏移、夹爪闭合宽度和接近速度；放置失败时调整放置高度和退出高度。此处调整量是教学示例，真机实验应依据日志和安全范围逐步确定。
...
```python
def adjust_task(task: TaskConfig, failure: FailureCode) -> TaskConfig:
...
完整状态机见 [`state_machine.py`](../../code/lecture07/robot_pick_place/state_machine.py)。`run_attempt()` 负责一次尝试，每个状态都遵循“进入—执行—检查—失败即返回”的模式：
...
def run_attempt(self) -> AttemptResult:
    ...
    # 搬运、下降、释放和放置检测使用相同模式，见完整源码。
```

跨尝试的恢复由 `run()` 管理。失败后先停止，再判断是否达到重试上限；仍可恢复时才修改任务参数：
...
这样，单次状态流与重试策略彼此独立，恢复逻辑不会散落在每一个动作状态中。
...
| 分阶段试运行     | 先空载到达预抓取、抬升和放置位姿，再加入物体并启用夹爪动作                 |
...
实验按六个检查点推进，每一步都要留下可验证的输出：

1. **Mock 空跑**：分别运行 `cube` 和 `bottle`，确认状态机到达 `DONE`，并生成 `events.jsonl`。
2. **位姿可视化**：画出五个动作位姿。方向、高度或偏移不合理时，在这一阶段修正，不要等真机碰撞后再查。
3. **仿真闭环**：在 ManiSkill 中检查控制器动作、碰撞、物体是否随夹爪运动，以及视频和轨迹能否保存。
4. **真机空载**：只执行预抓取、抬升和放置位姿，低速确认逆运动学、轨迹、桌面间隙和急停路径。
5. **逐级加任务**：先运行方块，再运行瓶子；每次只增加一种新约束，不同时修改多组参数。
6. **制造一次可恢复失败**：在仿真中改变接近偏移或夹爪宽度，检查失败代码、参数调整和第二次尝试是否都写入日志。真机不得通过故意碰撞完成这一项。

## 6.11 怎么看结果

实验结果需要同时保留空间、时间和任务三个层面的证据。空间层面显示目标物体位姿、五个动作位姿和机械臂末端轨迹；时间层面显示状态机进入与退出时刻、夹爪命令和检测结果；任务层面统计成功率、耗时、重试次数和失败类型。

![](../../assets/figures/lecture07/fig-07-11-log-to-report_song.png)

**图 7-9 从运行日志到实验报告和数据回流的处理链路**

**表 7-3 第 6 节实验的可视化与分析输出**

| **输出文件或图表** | **必须包含的内容**                                       | **用于回答的问题**                     |
|--------------------|----------------------------------------------------------|----------------------------------------|
| 动作位姿图         | 物体位姿、预抓取、抓取、抬升、放置和退出位姿             | 目标生成是否符合任务几何关系           |
...
假设瓶子最终倒在收纳盒外。不要从最后一帧开始猜，而是沿状态时间线向前找：如果瓶子在 `VERIFY_LIFT` 时已经相对夹爪下滑，盒外倾倒只是后果，第一修改项应是夹持位置、闭合宽度或抬升速度；只有瓶子在搬运阶段始终稳定、进入 `MOVE_PLACE` 后才碰到盒沿，才应该调整放置位姿和运动路径。

一次合格的复盘写四行就够：最早异常状态是什么；当时直接观察到了什么；哪些证据支持原因判断；下一轮只改哪个条件。把事实和推断分开，通常比列出十种“可能原因”更有用。

## 6.13 让 AI 帮忙分析，但别把判断交出去
...
## 6.14 Vibe Coding：让 AI 补完日志分析，再逐项核对
...
**表 7-4 完整实验结果的评价参考（满分 100 分）**

| **检查项**            | **看到什么才算完成**                                                   | **参考分值** |
|-----------------------|------------------------------------------------------------------------|----------|
| 工程代码              | 模块结构清楚，方块和瓶子任务均可配置；状态机、检测、恢复和日志能够运行 | 20       |
| 动作位姿              | 展示五个关键位姿，说明方块竖直抓取和瓶子水平抓取的差异                 | 15       |
...
开篇那次空抓之所以会一路运行到 B 区，是因为程序只检查了命令是否执行，没有检查物体是否跟随。现在，五个动作位姿给出路径，状态机在关键位置停下来，夹爪与视觉提供物理证据，失败代码则把流程送回正确的恢复入口。

![从目标位姿到真实操作闭环](../../assets/figures/lecture07/fig-07-12-closed-loop-summary_song.png)

**图 7-10 从目标位姿到真实操作闭环**

方块搬运和瓶子入盒表面上是两个不同的任务，它们在底层其实是同一套“生成—执行—检测—恢复”闭环。只要其中一环缺少可靠证据，程序就不应宣布任务成功。

到这里，第 7 讲也为传统模块化机器人操作画上了句号。从前面的感知、位姿估计，到本讲的抓取、放置、结果检测和失败恢复，我们搭建的是一条典型的模块化流水线：
...
第 8 讲开始，课程会换一种方法回答这个问题：从感知、规划和控制分段设计的模块化 pipeline，走向直接学习 observation 到 action 映射的端到端 policy。规则不会从系统里消失，但机器人的核心行为将不再只靠工程师逐条写出，而要从任务数据中学习。
