# 开源项目链接汇总

按讲次组织；正文请链接到本文件，避免重复维护 URL。

---

## Lecture 01 — 具身智能导论 {#lecture-01}

| 项目 | URL |
|------|-----|
| Xbotics-Embodied-Guide | https://github.com/Xbotics-Embodied-AI-club/Xbotics-Embodied-Guide |
| LeRobot | https://github.com/huggingface/lerobot |
| Awesome-LLM-Robotics | https://github.com/GT-RIPL/Awesome-LLM-Robotics |
| Awesome-World-Models | https://github.com/knightnemo/Awesome-World-Models |
| Awesome-Humanoid-Robot-Learning | https://github.com/YanjieZe/awesome-humanoid-robot-learning |

## Lecture 02 — 系统架构 {#lecture-02}

| 项目 | URL |
|------|-----|
| LeRobot | https://github.com/huggingface/lerobot |
| LeRobot Hugging Face | https://huggingface.co/lerobot |
| lerobot-ros | https://github.com/ycheng517/lerobot-ros |
| so101_ros2 | https://github.com/msf4-0/so101_ros2 |
| so101-ros-physical-ai | https://github.com/legalaspro/so101-ros-physical-ai |

## Lecture 03 — 本体与控制 {#lecture-03}

| 项目 | URL |
|------|-----|
| MoveIt2 tutorials | https://github.com/moveit/moveit2_tutorials |
| MoveIt2 | https://github.com/moveit/moveit2 |
| MuJoCo Menagerie | https://github.com/google-deepmind/mujoco_menagerie |
| ManiSkill | https://github.com/haosulab/ManiSkill |
| Isaac Lab | https://github.com/isaac-sim/IsaacLab |
| unitree_sdk2_python | https://github.com/unitreerobotics/unitree_sdk2_python |
| unitree_rl_gym | https://github.com/unitreerobotics/unitree_rl_gym |

## Lecture 04 — 传感器与坐标系 {#lecture-04}

| 项目 | URL |
|------|-----|
| Open3D | https://github.com/isl-org/Open3D |
| pytransform3d | https://github.com/dfki-ric/pytransform3d |
| ROS2 examples | https://github.com/ros2/examples |
| ROS2 geometry2 / tf2 | https://github.com/ros2/geometry2 |
| realsense-ros | https://github.com/IntelRealSense/realsense-ros |
| Kalibr | https://github.com/ethz-asl/kalibr |

## Lecture 05 — 仿真 {#lecture-05}

| 项目 | URL |
|------|-----|
| MuJoCo | https://github.com/google-deepmind/mujoco |
| Isaac Lab docs | https://isaac-sim.github.io/IsaacLab/main/index.html |
| unitree_rl_lab | https://github.com/unitreerobotics/unitree_rl_lab |
| unitree_sim_isaaclab | https://github.com/unitreerobotics/unitree_sim_isaaclab |
| RoboCasa | https://github.com/robocasa/robocasa |
| robosuite | https://github.com/ARISE-Initiative/robosuite |

## Lecture 06 — 位姿估计 {#lecture-06}

| 项目 | URL |
|------|-----|
| Ultralytics YOLO | https://github.com/ultralytics/ultralytics |
| Segment Anything | https://github.com/facebookresearch/segment-anything |
| SAM2 | https://github.com/facebookresearch/sam2 |
| Grounded-Segment-Anything | https://github.com/IDEA-Research/Grounded-Segment-Anything |
| FoundationPose | https://github.com/NVlabs/FoundationPose |
| SAM-6D | https://github.com/JiehongLin/SAM-6D |
| AnyGrasp | https://github.com/graspnet/anygrasp_sdk |

## Lecture 07 — 操作技能 {#lecture-07}

| 项目 | URL |
|------|-----|
| MoveIt 2 Documentation | https://moveit.picknik.ai/ |
| MoveIt 2 Tutorials | https://moveit.picknik.ai/main/doc/tutorials/tutorials.html |
| ManiSkill | https://github.com/haosulab/ManiSkill |
| ManiSkill Docs | https://maniskill.readthedocs.io/ |
| robosuite | https://robosuite.ai/docs/ |
| MuJoCo Menagerie | https://github.com/google-deepmind/mujoco_menagerie |
| Isaac Lab | https://isaac-sim.github.io/IsaacLab/ |
| AnyGrasp SDK | https://github.com/graspnet/anygrasp_sdk |
| GraspNet Baseline | https://github.com/graspnet/graspnet-baseline |
| GraspNet-1Billion | https://graspnet.net/ |
| Open3D | https://github.com/isl-org/Open3D |

## Lecture 08 — 端到端导论 {#lecture-08}

| 项目 | URL |
|------|-----|
| robomimic | https://github.com/ARISE-Initiative/robomimic |
| ACT | https://github.com/tonyzhaozh/act |
| Diffusion Policy | https://github.com/real-stanford/diffusion_policy |
| OpenVLA | https://github.com/openvla/openvla |
| Gymnasium Robotics | https://github.com/Farama-Foundation/Gymnasium-Robotics |

## Lecture 09 — 数据采集 {#lecture-09}

| 项目 | URL |
|------|-----|
| RLDS | https://github.com/google-research/rlds |
| Universal Manipulation Interface | https://github.com/real-stanford/universal_manipulation_interface |
| UMI Project | https://umi-gripper.github.io/ |
| Open X-Embodiment | https://robotics-transformer-x.github.io/ |
| Ego4D | https://ego4d-data.org/ |
| EgoDex | https://github.com/apple/ml-egodex |

## Lecture 10 — 模仿学习 {#lecture-10}

LeRobot、robomimic、ACT++、ALOHA — 见 Lecture 02/08。

## Lecture 11 — Diffusion Policy {#lecture-11}

Diffusion Policy、LeRobot、robomimic — 见 Lecture 08/02。

## Lecture 12 — VLA 理论 {#lecture-12}

| 项目 | URL |
|------|-----|
| Open X-Embodiment | https://github.com/google-deepmind/open_x_embodiment |
| Octo | https://github.com/octo-models/octo |
| OpenPI | https://github.com/Physical-Intelligence/openpi |
| unitree_lerobot | https://github.com/unitreerobotics/unitree_lerobot |
| π0 | https://www.physicalintelligence.company/blog/pi0 |
| NVIDIA GR00T | https://developer.nvidia.com/isaac/groot |

## Lecture 13 — VLA 实操 {#lecture-13}

OpenVLA-OFT、LIBERO — https://github.com/moojink/openvla-oft ，https://github.com/Lifelong-Robot-Learning/LIBERO

## Lecture 14–16 — 强化学习 {#lecture-14}

| 项目 | URL |
|------|-----|
| Stable-Baselines3 | https://github.com/DLR-RM/stable-baselines3 |
| rl-baselines3-zoo | https://github.com/DLR-RM/rl-baselines3-zoo |
| MuJoCo Playground | https://github.com/google-deepmind/mujoco_playground |
| unitree_rl_mjlab | https://github.com/unitreerobotics/unitree_rl_mjlab |

## Lecture 17 — VLN 理论 {#lecture-17}

| 项目 | URL |
|------|-----|
| Habitat-Lab | https://github.com/facebookresearch/habitat-lab |
| Habitat-Sim | https://github.com/facebookresearch/habitat-sim |
| VLN-CE | https://github.com/jacobkrantz/VLN-CE |
| Matterport3D | https://niessner.github.io/Matterport/ |

## Lecture 18 — VLN 实操 {#lecture-18}

| 项目 | URL |
|------|-----|
| AllenAct | https://github.com/allenai/allenact |
| Habitat-Lab VLN baselines | https://github.com/facebookresearch/habitat-lab/tree/main/habitat-baselines |

## Lecture 19 — Embodied Agent {#lecture-19}

| 项目 | URL |
|------|-----|
| LLM Powered Autonomous Agents | https://lilianweng.github.io/posts/2023-06-23-agent/ |
| Harness Engineering for Self-Improvement | https://lilianweng.github.io/posts/2026-07-04-harness/ |
| OpenAI Structured Outputs | https://openai.com/index/introducing-structured-outputs-in-the-api/ |
| OpenAI Function Calling | https://help.openai.com/en/articles/8555517 |
| ReAct | https://arxiv.org/abs/2210.03629 |
| Generative Agents | https://arxiv.org/abs/2304.03442 |
| Mem0 | https://arxiv.org/abs/2504.19413 |
| Introducing the Model Context Protocol | https://www.anthropic.com/news/model-context-protocol |
| Agent Skills | https://www.anthropic.com/engineering/equipping-agents-for-the-real-world-with-agent-skills |
| MCP Specification | https://modelcontextprotocol.io/ |
| MCP Architecture | https://modelcontextprotocol.io/docs/learn/architecture |
| MCP Server Features | https://modelcontextprotocol.io/specification/2025-11-25/server |
| MCP Security Best Practices | https://modelcontextprotocol.io/docs/tutorials/security/security_best_practices |
| Code as Policies | https://arxiv.org/abs/2209.07753 |
| Hi Robot | https://arxiv.org/abs/2502.19417 |
| What Matters in Orchestrating Robot Policies | https://arxiv.org/abs/2606.10267 |
| Harness VLA | https://arxiv.org/abs/2607.08448 |
| π0.7 | https://arxiv.org/abs/2604.15483 |
| MEM | https://arxiv.org/abs/2603.03596 |
| LimX COSA | https://www.limxdynamics.com/zh/news/BK000054 |
| Hey Robot | https://github.com/Xbotics-Embodied-AI-club/Xbotics-Hey-Robot |
| PhyAgentOS | https://github.com/PhyAgentOS/PhyAgentOS |
| Dimos | https://github.com/dimensionalOS/dimos |

## Lecture 20 — 世界模型 {#lecture-20}

| 项目 | URL |
|------|-----|
| World Models for Robot Learning: A Comprehensive Survey | https://arxiv.org/abs/2605.00080 |
| Auto-Encoding Variational Bayes | https://arxiv.org/abs/1312.6114 |
| DINOv2 | https://arxiv.org/abs/2304.07193 |
| DINOv3 | https://arxiv.org/abs/2508.10104 |
| DINO-WM | https://dino-wm.github.io/ |
| DreamerV3 | https://github.com/danijar/dreamerv3 |
| DreamerV3 PyTorch | https://github.com/burchim/DreamerV3-PyTorch |
| AVDC | https://arxiv.org/abs/2310.08576 |
| VidMan | https://arxiv.org/abs/2411.09153 |
| WorldVLA | https://arxiv.org/abs/2506.21539 |
| Vidar | https://arxiv.org/abs/2507.12898 |
| VLA-JEPA | https://arxiv.org/abs/2602.10098 |
| JEPA-VLA | https://arxiv.org/abs/2602.11832 |
| DreamZero | https://arxiv.org/abs/2602.15922 |
| Ctrl-World | https://ctrl-world.github.io/ |
| TesserAct | https://tesseractworld.github.io/ |
| Fast-WAM 论文 | https://arxiv.org/abs/2603.16666 |
| Fast-WAM 项目页 | https://yuantianyuan01.github.io/FastWAM/ |
| Fast-WAM 官方仓库 | https://github.com/yuantianyuan01/FastWAM |

## Lecture 21 — 前沿进展 {#lecture-21}

| 项目 | URL |
|------|-----|
| OpenPI | https://github.com/Physical-Intelligence/openpi |
| OpenVLA | https://github.com/openvla/openvla |
| Xbotics-Embodied-Guide 前沿 | https://github.com/Xbotics-Embodied-AI-club/Xbotics-Embodied-Guide/tree/main/docs/5-sota |

---

## 数据集与社区

- LeRobot Dataset Hub: https://huggingface.co/lerobot
- UMI Data Community: https://umi-data.github.io/

## 维护说明

新增链接请按讲次追加；勿在多个 Markdown 中硬编码同一 URL。
