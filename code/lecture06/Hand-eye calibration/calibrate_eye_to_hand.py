...
def build_motion_pairs_eye_to_hand(robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray]):
    a_rot, a_trans = [], []
    b_rot, b_trans = [], []
    n = len(robot_mats)
    for i in range(n - 1):
        for j in range(i + 1, n):
            A = robot_mats[j] @ invert_transform(robot_mats[i])
...
def solve_with_opencv(robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray], method: str) -> np.ndarray:
    a_rot, a_trans, b_rot, b_trans = build_motion_pairs_eye_to_hand(robot_mats, camera_mats)
    if len(a_rot) < 3:
    ...
    T_base_camera[:3, 3] = np.asarray(t_cam2gripper, dtype=np.float64).reshape(3)
    return T_base_camera


def evaluate_eye_to_hand(robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray], T_base_camera: np.ndarray):
    base_board = [T_base_camera @ camera for camera in camera_mats]
    ref = base_board[0]
...
def main() -> None:
    ...
    residual_summary = evaluate_eye_to_hand(robot_mats, camera_mats, T_base_camera)
