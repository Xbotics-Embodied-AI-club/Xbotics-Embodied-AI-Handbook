"""Eye-to-Hand hand-eye calibration demo for lecture 6."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Solve Eye-to-Hand hand-eye calibration.")
...
def build_motion_pairs_eye_to_hand(sample_ids: Sequence[str], robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray]):
    forward_robot = [invert_transform(T) for T in robot_mats]
    return build_relative_motion_pairs(sample_ids, forward_robot, camera_mats)
...
def solve_with_opencv(robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray], method: str) -> np.ndarray:
    ...
    gripper_to_base = [invert_transform(item) for item in robot_mats]
    gripper_R = [item[:3, :3] for item in gripper_to_base]
    gripper_t = [item[:3, 3].reshape(3, 1) for item in gripper_to_base]
    ...
    T_base_camera[:3, 3] = np.asarray(t_cam2base, dtype=np.float64).reshape(3)
    return T_base_camera


def evaluate_eye_to_hand(sample_ids: Sequence[str], robot_mats: Sequence[np.ndarray], camera_mats: Sequence[np.ndarray], T_base_camera: np.ndarray):
    end_board = [invert_transform(robot) @ T_base_camera @ camera for robot, camera in zip(robot_mats, camera_mats)]
    return summarize_transform_constancy(sample_ids, end_board)
...
def main() -> None:
    ...
    motion_pairs = build_motion_pairs_eye_to_hand(sample_ids, robot_mats, camera_mats)
    ...
    if args.solver == "opencv":
        T_base_camera = solve_with_opencv(robot_mats, camera_mats, args.method)
        solver_name = f"opencv_calibrateHandEye:{args.method}"
    else:
        T_base_camera, teaching_diagnostics = solve_hand_eye_park(motion_pairs)
        solver_name = "park_teaching"

    validation = evaluate_eye_to_hand(sample_ids, robot_mats, camera_mats, T_base_camera)
    residual_summary = validation["summary"]

    metadata = {
        "robot_matrix_key": args.robot_matrix_key,
        "camera_matrix_key": args.camera_matrix_key,
        "kinematic_chain": "T_base_end · T_end_board = T_base_camera · T_cam_board",
        ...
        "implied_T_end_board_reference": validation["reference_transform"],
    }
    if weak_pairs:
    ...
    if args.report_output:
        report = {
            ...
            "diagnostics": diagnostics,
            "end_board_constancy": validation,
        }
        if teaching_diagnostics is not None:
    ...
    print("  chain: T_base_end · T_end_board = T_base_camera · T_cam_board")
