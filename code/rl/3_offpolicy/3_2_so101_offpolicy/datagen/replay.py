...
import so101_sim  # 注册任务 + vendored squint 上 sys.path

# ── 后置可改的旋钮（默认=squint 原始外观/相机；真机标定后改这里）─────────────────
APPEARANCE = "black_white"          # "default"（原样）/ "black_white"（黑臂+白底，贴近真机）
ROBOT_COLOR = (0.0, 0.0, 0.0)       # 黑臂
BACKGROUND = 255                    # 白底（overlay 灰度值）
# 相机位姿 / FOV 留作真机标定接口：None=沿用 squint 默认（tune_camera 对齐后在此填真机值）
CAMERA_EYE = None
CAMERA_TARGET = None
CAMERA_FOV = None
...
def replay(task: str, in_h5: Path, out_h5: Path, work: Path) -> Path:
    ...
    with h5py.File(out_h5, "a") as f:
        for traj in f:
            g = f[traj]
            so101 = g["env_states/articulations/so101"][:]
            actors = {k: g[f"env_states/actors/{k}"][:] for k in g["env_states/actors"].keys()}
            rgb_ds = g["obs/sensor_data/base_camera/rgb"]
            for i in range(rgb_ds.shape[0]):
                sd = env.unwrapped.get_state_dict()
                sd["articulations"]["so101"] = torch.tensor(so101[i:i + 1], device="cuda")
