...
## 第四版对照：off-policy SAC vs on-policy PPO（讲16 §4.3）
...
- **绝对高度**：SAC 停在 10⁻² 量级（和 v1/v2 评测同档），**没追上 PPO 的 0.097**。这套奖励是给 PPO 调的（讲义 §4.2："原样搬不一定最优"），课堂版又只保留最核心三件（回放 + 双 Q + 自动温度），没上 FastSAC 的 n-step / 分布式 critic。
- **过训**：SAC 后期动作幅度慢慢顶大、reward 从峰值回落（off-policy 常见不稳），故取"会走"的最佳 checkpoint（iter 900）作证据。

**诚实结论**：教学版 SAC 在 G1 上**能稳定走起来、起步比 PPO 省交互**，印证 off-policy 的样本效率方向；但绝对指标要压过精调的 PPO 需要 FastSAC 完整配方。这正是讲16 §4 要立的对照。
