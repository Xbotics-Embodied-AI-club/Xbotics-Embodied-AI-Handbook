"""把 squint 的 ManiSkill3 SO101 任务包装成 lerobot 评测用的单环境 gym.Env。
...
这是本模块与仿真器之间**唯一的耦合点**：只有本文件依赖 vendored squint / ManiSkill，
...
class So101SimEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # squint 任务本身已注册 max_episode_steps=50，这里与之对齐。
        self._max_episode_steps = episode_length

        # 每个包装实例内部持有一个 num_envs 份的 ManiSkill 环境（GPU 后端、无头 RGB 渲染）。
        # squint 的 8 个 SO101 任务已在 import so101_sim 时注册；task 即其 id。
        ...
        # 观测空间：base_camera 一路 RGB（+ 可选关节位置）。关节维度取自单环境动作/状态。
        ...
        if obs_type == "pixels":
            self.observation_space = spaces.Dict({"pixels": spaces.Dict({"base_camera": image_space})})
        elif obs_type == "pixels_agent_pos":
            self.observation_space = spaces.Dict(
                {
                    "pixels": spaces.Dict({"base_camera": image_space}),
    ...
    def _format_raw_obs(self, raw_obs: dict) -> dict:
        # ManiSkill 观测：sensor_data.base_camera.rgb (N,H,W,3) / agent.noisy_qpos (N,dof)。
        # num_envs=1 时对首维取 [0]，还原成之前的单环境形状；否则整批返回。
        rgb = _to_numpy(raw_obs["sensor_data"]["base_camera"]["rgb"]).astype(np.uint8)
        images = {"base_camera": rgb[0] if self.num_envs == 1 else rgb}
