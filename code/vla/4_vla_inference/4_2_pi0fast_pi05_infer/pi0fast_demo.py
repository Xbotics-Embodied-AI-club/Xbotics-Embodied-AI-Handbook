"""π0-FAST 在 LIBERO 上闭环推理一次，把整段 rollout 写成 mp4。

第11讲 4.5 节的配套代码。和同为离散路线的 OpenVLA 对照着跑：FAST 把一整块动作压成一小串
token，于是从"每一步都等模型解码 7 个 token"变成"每隔一个动作块才等一次"。
"""
...
# π0-FAST：把连续动作块经 FAST（DCT+BPE）压缩成离散 token，让 VLM 自回归地
# “说出”动作，再解码回连续动作块。与 π0 的 flow matching 动作头形成对照。
POLICY_PATH = "lerobot/pi0fast-libero"
...
SEED = 7
OUT_PATH = Path("vla/4_vla_inference/4_2_pi0fast_pi05_infer/output/pi0fast_libero_success.mp4")
...
def main() -> None:
    ...
    try:
        ...
        for _ in range(MAX_STEPS):
            ...
            # π0-FAST 每次自回归生成一串 FAST 动作 token 并解码成动作块，
